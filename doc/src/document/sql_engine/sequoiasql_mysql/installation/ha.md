...
+ 同步工具中配置的 MySQL 用户，应仅用于该工具，不应在外部操作、应用程序中使用，否则可能造成实例间元数据不一致
...
至此，即完成了元数据同步工具的安装及配置。由于同步工具使用审计日志文件的 inode 值来记录同步进度，因此在正常运行期间，不要对审计日志进行编辑等任何可能导致其 inode 发生变化的操作，否则会造成同步中断。
