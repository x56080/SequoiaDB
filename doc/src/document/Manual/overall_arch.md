...
数据库实例模块则作为协议与语法的适配层，用户可根据需要创建包括 MySQL、PostgreSQL与 SparkSQL 在内的结构化数据实例；以及 JSON 和 S3 对象存储的非结构化实例。
...
SequoiaDB 巨杉数据库支持多种类型的数据库实例。至当前版本为止，SequoiaDB 巨杉数据库支持 5 种不同的实例类型。

| 实例类型 | 实例分类 | 描述 |
|----|----|----|
| MySQL | 结构化数据 | 适用于纯实时处理场景，与 MySQL 保持协议级兼容 |
| PostgreSQL | 结构化数据 | 适用于实时处理场景与中小量数据的分析类场景，与 PostgreSQL 基本保持兼容 |
| SparkSQL | 结构化数据 | 适用于海量数据的统计分析类场景，与 SparkSQL 保持协议级兼容 |
| JSON API | 半结构化数据 | 适用于基于 JSON 数据类型的实时处理场景 |
| S3 对象存储 | 非结构化数据 | 适用于对象存储类的实时处理与归档类场景，与 S3 保持协议级兼容 |
...
在当前版本中，SequoiaDB 巨杉数据库支持多达 5 种不同数据服务实例，包括针对结构化数据的 MySQL、PostgreSQL 与 SparkSQL 实例；针对半结构化数据的 JSON 实例；以及针对非结构化数据的 S3 对象存储实例。
