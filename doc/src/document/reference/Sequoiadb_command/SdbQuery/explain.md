...
| Location       | BSON 对象        | 表示对访问计划的结果按照数据组进行过滤，使用命令位置参数项。<br>Location 选项仅支持 "GroupID" 和 "GroupName" 选项。<br>详细见 **[命令位置参数](reference/Sequoiadb_command/location.md)**。<br>如果 Location 选项显式设置，Detail 选项自动设置为 true。 | 空 |
...
请参考：[详细的访问计划](database_management/access_plans/detailed_access_plan/overview.md)。

##访问计划的搜索过程##

Search 选项为 true 时，将会展示查询优化器搜索过的访问计划，并查看查询优化器选择的结果。

请参考：[访问计划的搜索过程](database_management/access_plans/search_paths/overview.md)。
...
```lang-javascript
> db.foo.bar.find( { a : { $gt : 100 } } ).explain( { Run : true } )
{
  "NodeName": "hostname:11810",
  "GroupName": "group1",
  "Role": "data",
...
```lang-javascript
> db.foo.bar.find( { a : { $gt : 100 } } ).explain( { Detail : true, Search : true } )
{
  "NodeName": "hostname:11800",
  "GroupName": "SYSCoord",
  "Role": "coord",
