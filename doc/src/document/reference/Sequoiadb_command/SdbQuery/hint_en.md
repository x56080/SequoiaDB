...
```lang-javascript
> db.foo.bar.find( {age: 100 } ).hint( { "": null } )
```

* Use the index ageIndex to iterate through the records of the age field in the collection bar and return.
...
```lang-javascript
> db.foo.bar.find( {age: 10 } ).hint( { "1": "aIndex", "2": "bIndex", "3":"cIndex" } )
```