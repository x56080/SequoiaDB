...
* Select the record that the age field value greater than (with using [$gt](reference/operator/match_operator/gt.md)) 10 under the collection, bar, and add (with using [$inc](reference/operator/update_operator/inc.md)) one to the age field.

```lang-javascript
> db.foo.bar.find( { age: { $gt: 10 } } ).update( { $inc: { age: 1 } }, true )
{
  "_id": {
