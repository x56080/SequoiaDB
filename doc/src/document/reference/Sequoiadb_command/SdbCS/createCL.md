...
- name（ *string，必填* ）
    ...
    - 在同一个集合空间中，集合名必须唯一。
    - 有关集合与集合命名的限制可参考[限制](reference/Sequoiadb_limitation.md#集合)。
...
- options（ *object，选填* ）
    ...
    - AutoIncrement（ *object* ）：自增字段，功能介绍可参考[自增字段](data_model/auto_increment.md)
    ...
    - Mapping（ *string* ）：所映射的集合名称
        ...
        >**Note:**
        >
        > 参数 DataSource 和 Mapping 的具体使用场景可参考[数据源](infrastructure/datasource.md)。
