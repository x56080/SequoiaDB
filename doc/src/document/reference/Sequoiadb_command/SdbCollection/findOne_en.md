...
This function is used to return a record that meets the query conditions, it has the same usage as the [find()](reference/Sequoiadb_command/SdbCollection/find.md) method.

##PARAMETERS##

For parameter description, refer to [find()](reference/Sequoiadb_command/SdbCollection/find.md) method.
...
- Return the records in the collection "employee" whose "age" field value is greater than 20 (for example, use [$gt](reference/operator/match_operator/gt.md) query) and set to only return the "name" and "age" fields of the records, and sort the age and name fields in ascending order of the "age" field value.
