...
- flags ( *object, optional* )
    ...
    Specify the flag bit to traverse the result set. For specific usage user can refer to [flags()](reference/Sequoiadb_command/SdbQuery/flags.md).
...
>**Note：**
>
> * The parameter 'sel' is an object, the values of it's fields are empty string, database only care about the name of the fields.
> * The parameter 'hint' is an object, the name of it's fields can be any unique string, database onlu care about the value of the fileds.
...
- select records from collection 'foo.bar' and return records which field "age" is greater
...
- select records from collection 'foo.bar' and return records which filed 'age' is greater
...
- Find records which contain "rock climbing" in collection "bar" by using [full text search](basic_operation/text_search/text_search_grammer.md).
