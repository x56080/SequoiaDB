...
该函数用于返回符合查询条件的一条记录，与 [find()](reference/Sequoiadb_command/SdbCollection/find.md) 方法用法相同。

##参数##

参数说明可参考 [find()](reference/Sequoiadb_command/SdbCollection/find.md) 方法。
...
- 返回集合 employee 中 age 字段值大于 20 的记录（如使用 [$gt](reference/operator/match_operator/gt.md) 查询），设置只返回记录的 name 和 age 字段，并按 age 字段值的升序排序
