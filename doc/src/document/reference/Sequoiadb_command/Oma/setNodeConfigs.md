...
* 覆盖后，配置文件中原来的内容会丢失。
* 使用 [reloadConf()](reference/Sequoiadb_command/Sdb/reloadConf.md) 重载配置文件。
* 使用 [updateConf()](reference/Sequoiadb_command/Sdb/updateConf.md) 和 [deleteConf()] (reference/Sequoiadb_command/Sdb/deleteConf.md) 在线修改配置。
...
* `config` ( *Object*， *必填* )
	...
	节点配置信息，如配置日志大小，是否打开事务等，具体可参考[数据库配置](database_management/database_configuration/configuration_parameters.md)。
