...
- options ( *object，选填* )
    ...
    - showalone ( *boolean* )：是否显示以 [standalone 模式](reference/Sequoiadb_command/Oma/start.md)启动的 sdbcm 节点信息，默认为 false
...
- filter ( *object，选填* )
    ...
    指定筛选节点信息的条件，支持通过[匹配符](reference/operator/match_operator/overview.md) $and、$or、$not 或精确匹配检索节点信息
