...
>         * The configuration item "instanceid" of modifying data node cannot take effect dynamically, and the data node needs to be stopped and started manually. After restarting, users also need to manually call [Sdb.invalidateCache()](reference/Sequoiadb_command/Sdb/invalidateCache.md) to clear the cache of each coordination node.
...
> * Get session attributes refers to [Sdb.getSessionAttr()](reference/Sequoiadb_command/Sdb/getSessionAttr.md).
