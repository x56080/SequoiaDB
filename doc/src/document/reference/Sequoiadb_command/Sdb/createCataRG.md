...
该函数用于新建编目分区组，同时创建并启动一个编目节点。同一集群中只能创建一个编目分区组，且组名默认为“SYSCatalogGroup”。
...
- config（ *object，选填* ）
 ...
 指定需要配置的细节参数，细节参数可参考[数据库配置](database_management/database_configuration/configuration_parameters.md)一节
...
在名为"hostname1"的主机上创建一个编目分区组，指定服务端口为 11800，指定数据文件存放路径为 `/opt/sequoiadb/database/cata/11800`，并配置日志文件大小为 64MB
