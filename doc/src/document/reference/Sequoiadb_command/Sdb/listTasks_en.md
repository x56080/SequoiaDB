##NAME##

listTasks - Enumerate background tasks

##SYNOPSIS##

**db.listTasks( [cond], [sel], [sort], [hint] )**

##CATEGORY##

Sdb

##DESCRIPTION##

This function is used to enumerate all background tasks in the database.

##PARAMETERS##

| Name    | Type   | Description    | Required or Not |
|---------|--------|----------------|-----------------|
| cond   | Json  | Filter conditions of tasks | Not  |
| sel 	 | Json  | Selection field of tasks | Not 		|
| sort   | Json   | Sort the returned records by the selected field. 1 is ascending and -1 is descending.        	| Not 	    |
| hint 	 | Json  | Reserved item 														| Not 	    |

##RETURN VALUE##

When the function executes successfully, it will return a detailed list of collections through the cursor.Users can refer to [SYSTASKS collection](infrastructure/catalog_node/SYSTASKS.md) to get the returned field information.

When the function fails, an exception will be thrown and an error message will be printed.

##ERRORS##

When the exception happens，use [getLastErrMsg()](reference/Sequoiadb_command/Global/getLastErrMsg.md) to get the error message or use [getLastError()](reference/Sequoiadb_command/Global/getLastError.md) to get the error code. For more details, refer to [Troubleshooting](troubleshooting/general/general_guide.md).

##VERSION##

v2.0 and above

##EXAMPLES##

* List all background tasks of the system

	```lang-javascript
	> db.listTasks()
	```