...
- 数值类型，如下例：
  ...
  { $inc: { age: 1 } }
  ```
- 通过[字段操作符](reference/operator/field_operator/field.md)指定的原始记录中某数值类型的字段，如下例：
...
- 对象类型，支持扩展功能，其中在 Value 字段的值中，也可以使用[字段操作符](reference/operator/field_operator/field.md)，如下例：
  ...
  对于使用[字段操作符](reference/operator/field_operator/field.md)的方式，如果 "fieldName" 字段在原始记录中不存在，则不做任何操作；如果存在但不是数值类型，则报错。
