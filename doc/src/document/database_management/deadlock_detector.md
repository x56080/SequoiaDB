...
![常见死锁场景](database_management/DeadlockDetector/sscDeadlock0.png)
...
用户可通过[事务死锁检测快照](database_management/monitoring/snapshot/SDB_SNAP_TRANSDEADLOCK.md)检测数据库中是否存在死锁。如果发现死锁，则输出每个死锁关联的事务信息；如果未发现死锁，则无输出。

假设集群中存在如下锁等待关系：

![关联死锁集](database_management/DeadlockDetector/sscDeadlock1.png)
...
以上述输出信息为例，应选择终止事务 T3 所在的会话。用户可通过 [forceSession()](reference/Sequoiadb_command/Sdb/forceSession.md) 终止该会话，以解除死锁。具体执行命令如下：
...
![极端死锁场景](database_management/DeadlockDetector/sscDeadlock2.png)
