...
>**Note:**
>
> 创建数据源的详细参数说明可参考 [createDataSource()](reference/Sequoiadb_command/Sdb/createDataSource.md)。

##查看数据源##

用户可通过 [listDataSources()](reference/Sequoiadb_command/Sdb/listDataSources.md) 或[数据源列表](database_management/monitoring/list/SDB_LIST_DATASOURCES.md)查看数据源的元数据信息。
...
| 操作                | 说明             |
| -------------       |------------      |
| [db.dropCS()](reference/Sequoiadb_command/Sdb/dropCS.md) | 删除映射集合空间<br>只会删除本地集群的元数据，不会删除作为数据源集群中的集合空间 | 
| [db.collectionspace.dropCL()](reference/Sequoiadb_command/SdbCS/dropCL.md) | 删除映射集合<br>只会删除本地集群的元数据，不会删除作为数据源集群中的集合 |
| [datasource.alter()](reference/Sequoiadb_command/SdbDataSource/alter.md) | 修改数据源的元数据信息 |
| [db.dropDataSource()](reference/Sequoiadb_command/Sdb/dropDataSource.md) | 删除指定数据源 |
