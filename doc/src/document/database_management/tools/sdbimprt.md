sdbimprt 是 SequoiaDB 数据导入工具，可以将 JSON 格式或 CSV 格式的数据导入到 SequoiaDB 数据库中。
...
| string        |         | 字符串，可以指定最小长度和最大长度，最大长度如果为 0，表示不设限制。<br>指定最大长度，如：```string(128)``` 最大长度128个字符，超出则截断<br> 指定最小长度，如：```string(10, 0)``` 最小长度10个字符，少于10个字符则用默认值代替，如果没有默认值则用 ```null``` 代替 <br>指定最小长度和最大长度，如：```string(10, 128)``` 最小长度10个字符，并且最大长度128个字符 |
| null          |         | 空值 |
...
| --csname    | -c   | 指定集合空间的名字 |
| --clname    | -l   | 指定集合的名字 |
| --errorstop |      | 如果遇到解析错误就停止，默认值为 false，解析错误后不停止 |
...
| --force        |      | 如果数据中有非 UTF-8 的字符，强制导入数据，默认为 false，不强制导入 |
...
| 选项           | 缩写 | 说明 | 
| -------------- | ---- | ---- | 
| --unicode      |      | 是否转义 Unicode 字符编码（\uXXXX），默认为 true，自动转移 Unicode 字符编码 | 
| --decimalto    |      | decimal 类型强制转换，默认 ""。<br>""：不转换，保留 decimal 类型。<br>double：强制转换成 double 类型，可能会发生精度丢失。<br>string：强制转换成 string 类型。 |
...
| --headerline   |      | 指定导入数据首行是否作为字段名。默认值为 false，导入数据首行不作为字段名 | 
| --sparse       |      | 指定导入数据中，数据的列数比字段的列数多，是否自动添加字段名。默认值为 true，字段名按 filed1、field2 顺序增加 |
| --extra        |      | 指定导入数据中，字段的列数比数据的列数多，是否自动添加 null 值。默认值为 false，不自动添加 |
| --cast         |      | 指定是否允许数值类型转换时丢失精度或数值溢出。默认值为 false，不允许丢失精度或者数值溢出 | 
...
* 导入格式是 csv，文件名是`test.csv`，导入至集合空间 sample 的集合 employee 中
...
* 导入格式是 csv，导入文件是目录`../data`中的所有文件，导入至集合空间 sample 的集合 employee 中
...
* 导入格式是 csv，导入文件是目录`../data`中的所有文件以及`./sample_employee_data.csv`，导入至集合空间 sample 的集合 employee 中，有 11810 和 11910 两个协调节点，记录中时间戳类型的数据类似于“2015-10-01 T 12.31.15.123 T”，使用两个连接同时导入
...
* 导入格式是 json，通过管道从其它工具 other 获取数据，导入至集合空间 sample 的集合 employee 中
