...
在下面的例子，sdbtop 使用配置文件为“/opt/sequoiadb/conf/sample/sdbtop.xml”，监控主机名为 hostname1，端口服务名为 11810，用户名为 test，密码为 test 的数据库集群中的一个节点。

```lang-javascript
$ sdbtop -c /opt/sequoiadb/conf/sample/sdbtop.xml -i hostname1 -s 11810 -u test -p test
```
