...
用户需搭建 Python 开发环境，具体可参考[Python 驱动开发](driver/python_driver/python_environment_to_build.md)章节。
...
| --help      |  -h   | 返回帮助信息                                      |    否    |
| --version   |  -v   | 返回版本信息                                      |    否    |
| --type      |  -t   | 监控类型，可选参数为 sdb、mysql                   |    是    |
| --inst      |       | 实例名，监控类型为 mysql 时可以指定               |    否    |
| --path      |       | 所监控对象的安装路径                              |    否    |
...
|instname|string|实例名，添加监控类型为 mysql 时自动生成|是|
