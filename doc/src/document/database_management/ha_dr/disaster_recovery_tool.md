SequoiaDB 巨杉数据库对于容灾处理提供了 split/merge 工具。split 工具的基本工作原理是使某些副本从原有集群中分裂出来，成为一个新的集群，单独提供读写服务；其余副本成为一个新的集群，仅提供读服务。merge 工具的基本工作原理是将分裂出去的副本重新合并到原有的集群中，恢复到集群最初的状态。关于工具的更多信息可参考[容灾切换合并工具](database_management/tools/split_merge.md)章节。 
...
在[同城双中心架构](database_management/ha_dr/twodatacenter.md)下，将机器划分为两个子网 SUB1（sdbserver1、sdbserver2）和 SUB2（sdbserver3），并在 sdbserver1 和 sdbserver3 上进行配置的修改。
...
在[同城三中心架构](database_management/ha_dr/threedatacenter.md)下，初始化时将机器划分为两个子网 SUB1（sdbserver1）和 SUB2（sdbserver2、 sdbserver3）。
...
[两地三中心](database_management/ha_dr/twocity_threedatacenter.md)架构下容灾工具的使用，可以参考[同城双中心](database_management/ha_dr/disaster_recovery_tool.md#同城双中心)。
...
### 灾备环境初始化
在[三地五中心架构](database_management/ha_dr/threecity_fivedatacenter.md)下，初始化时将机器划分为两个子网 SUB1（sdbserver1）和 SUB2（sdbserver2、sdbserver3、sdbserver4、sdbserver5)。
