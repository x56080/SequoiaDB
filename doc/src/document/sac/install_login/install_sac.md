...
- ###安装步骤###
  ...
  - 运行安装程序
     ...
     ```lang-javascript
     $ ./sequoiadb-2.0-linux-x86_64-installer.run --SMS true
     ```
  ...
  安装完成后，OM 会自动启动并开启8000端口的 web 服务，用户可以通过浏览器登陆 SAC，并进行集群的部署。假设安装 OM 的机器 IP 为192.168.1.100，则在浏览器键入 http://192.168.1.100:8000 ，访问 SAC 服务。
