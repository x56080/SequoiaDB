...
- insert 和 truncate 在某些情况下可能死锁
- 并发创建索引时在某些极端情况下导致内存破坏
- or 条件中同时包含分区键和非分区键字段查询返回结果错误
- 分区表使用特定条件查询，匹配数据不完整
- 在 insert/query 操作抛异常后，dropIndex 操作时卡死
- split 与 alter 并发时，导致数据组源节点集合在切分清理数据阶段被删除
- 插入 LOB 的同时切分并数据源主节点断网，丢失一个 LOB
- 在批量创建集合的过程中，强杀一备节点，导致最终备节点上丢失一个集合
- insert相同数据反复停启主节点导致主备数据不一致
- 使用 $exists 选择集合中嵌套数组中的元素，匹配记录不正确
- 使用内置SQL插入timestamp类型记录，1970年前的时间戳记录异常
- 选择符elemMatch只对数组生效，对嵌套obj不生效
- 并发事务在某种场景下更新 _id 字段，节点崩溃或者主备节点数据不一致
