...
| update bar set a=a+2 where b=-1               | db.foo.bar.update({$set:{a:2}},{b:-1})              | cl = db.get_collection("foo.bar")<br>condition = { "b":1 }<br>rule = { "$set":{"a":2} }<br>cl.update ( rule, condition=condition ) |
