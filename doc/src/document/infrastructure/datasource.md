...
![数据源架构图](infrastructure/datasource.png)
...
用户可通过 [alter()](reference/Sequoiadb_command/SdbDataSource/alter.md) 修改数据源的名称、地址列表、同步策略等元数据信息。
...
1. 获取数据源 datasource 的引用
   ...
   >**Note:**
   >
   > [getDataSource()](reference/Sequoiadb_command/Sdb/getDataSource.md) 用于获取数据源的引用。
...
用户可通过 [dropDataSource()](reference/Sequoiadb_command/Sdb/dropDataSource.md) 删除指定数据源。删除数据源时，需要确保该数据源不关联任何集合空间或集合。
