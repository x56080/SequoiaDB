...
- 结构化数据：指表单类型的数据存储结构，如银行核心交易等传统业务使用的数据
...
数据存储
----
图 1 的数模型中，数据文件最终都需要在磁盘文件中进行持久化存储，与之相关的三个概念如下：
...
- 实例（Instance）：一份完整的集合记录所涉及的数据节点称为一个实例。在多副本的情况下，复制组中存在多份完整的集合记录，也就拥有了多个数据节点实例。在业务场景中，用户可以访问所有的实例，以充分利用冗余的多副本数据。
...
[^_^]:
    本文使用的所有链接及引用
[data_mode_architecture_diagram]:images/infrastructure/Data_Model/data_mode_architecture_diagram.png
[data_mode_cluster_manager_concept]:images/infrastructure/Data_Model/replica_group_concept.png

[data_mode_collection_space]:manual/infrastructure/Data_Model/collection_space.md
[data_mode_collection]:manual/infrastructure/Data_Model/collection.md
[data_mode_document]:manual/infrastructure/Data_Model/document.md
[data_mode_lob]:manual/infrastructure/Data_Model/lob.md
[data_mode_oid]:manual/reference/Sequoiadb_command/SpecialObjects/OID.md

[data_mode_index]:manual/infrastructure/Data_Model/index.md
[data_mode_text_index]:manual/infrastructure/Data_Model/text_index.md
[data_mode_sequence]:manual/infrastructure/Data_Model/sequence.md

[data_mode_domain]:manual/infrastructure/domain.md
[data_mode_group]:manual/infrastructure/Sharding/architecture.md
[data_mode_node]:manual/infrastructure/Node/readme.md
[data_mode_replication]:manual/infrastructure/Replication/Readme.md
[data_mode_sharding]:manual/infrastructure/Sharding/Readme.md
