...
2. 创建子集合 bill.201905 ，分区键为帐号 id 字段，分区方式为散列分区（hash），hash 值总数为 4096 个, 集合所在复制组为 group1
...
3. 执行切分命令，将集合 bill.201905 中，帐号 id 字段 hash 值范围在[2048, 4096)的记录，从复制组 group1 切分到复制组 group2 

   ```lang-bash
   > db.bill.201905.split( "group1", "group2", { Partition: 2048 }, { Partition: 4096 } )
   ```

4. 通过[挂载操作][attach_command]，将主集合 main.bill 和子集合 bill.201905 进行关联

   ```lang-bash
   > db.main.bill.attachCL( "bill.201905", { LowBound: { create_date: "201905" }, UpBound: { create_date: "201906" } } )
   ```

[^_^]:
    本文使用到的所有链接及引用。
[intro_sharding_multiple]:images/infrastructure/Sharding/intro_sharding_multiple.png
