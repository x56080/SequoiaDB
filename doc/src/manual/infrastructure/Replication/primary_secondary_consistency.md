...
在核心交易型业务中，为了保证数据安全性，同时可以牺牲写入性能时，推荐使用强一致性策略。
...
为了尽量保证数据的安全性，又兼顾高可用性，用户可以将 ReplSize 设为大多数。对数据一致性要求较高的业务，如影像内容管理平台和在线交易服务平台等，推荐使用写大多数的最终一致性策略。

![写大多数示意图][consistency2]

以三副本复制组为例，大多数为 ( n - 1 ) / 2，即 ReplSize 设为 2
...
[^_^]:
    本文使用到的所有链接及引用。
[consistency0]:images/infrastructure/Replication/consistency0.png
[consistency1]:images/infrastructure/Replication/consistency1.png
[consistency2]:images/infrastructure/Replication/consistency2.png
[consistency_alive]:images/infrastructure/Replication/consistency_alive.png
