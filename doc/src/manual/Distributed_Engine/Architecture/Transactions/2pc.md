...
在 SequoiaDB 巨杉数据库的分布式事务中，二阶段提交机制可以保证参与事务的各个数据组之间的 ACID 事务特性。

##二阶段提交执行流程##

二阶段提交机制存在协调者（即协调节点）和参与者（即数据节点）两种角色。协调者负责处理所有参与者的操作结果，推进事务的处理流程；参与者则响应协调者的请求，根据请求完成操作。

![pc][pc]

二阶段提交机制由两个阶段组成：

- 准备阶段（Pre-Commit）：协调节点询问所有参与事务的数据节点是否准备好提交。如果数据节点均已准备好提交，将回复可进行提交，否则回复不可进行提交。
- 提交阶段（Commit）：在协调节点收到所有数据节点回复的可进行提交后，将向所有参与的数据节点发送提交指令，所有的数据节点立即执行提交操作。否则协调节点向所有参与的数据节点发送回滚指令，所有的数据节点立即执行回滚操作。
...
> - 由于二阶段提交的特性，同一个事务中两个参与的数据节点不可能分别处于 DOING，COMMITTED 或者 ROLLBACKED 状态
...
[status]:manual/Distributed_Engine/Architecture/Transactions/2pc.md#事务状态
