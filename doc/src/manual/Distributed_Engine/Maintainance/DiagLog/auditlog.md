...
### 节点级
  ...
  - 执行 [updateConf()][updateConf] 命令修改"auditmask"取值并动态生效
    ...
    > **Note：**
    >
    > SequoiaDB 巨杉数据库3.0及以上版本配置节点"auditmask"后在线动态生效，v3.0 以下版本需要重启节点生效，详情可参考"[配置项参数][configuration_parameters]"章节 auditmask 参数说明。
...
> **Note：**
>
> Result 操作结果中的返回码，0 表示操作成功，非 0 表示操作失败（如 -33 创建集合失败）。非0返回码对应解读信息可参考[错误码][error_code]。
