...
下述升级过程以 SequoiaDB v3.4.4 升级至 v7.0.1 为例进行说明，其他版本间的升级操作基本一致。

1. 以 root 用户登陆目标主机，解压 SequoiaDB 巨杉数据库产品包，并为解压得到的 `sequoiadb-7.0.1-linux_x86_64-installer.run` 安装包赋可执行权限

    ```lang-bash
    # tar -zxvf sequoiadb-7.0.1-linux_x86_64-installer.tar.gz
    # chmod u+x sequoiadb-7.0.1-linux_x86_64-installer.run
    ```

2. 运行 `sequoiadb-7.0.1-linux_x86_64-installer.run` 包

   ```lang-bash
   $ ./sequoiadb-7.0.1-linux_x86_64-installer.run --mode text
   ```
...
4. 显示安装协议，输入回车，忽略阅读并同意协议；输入 2 后按回车表示读取全部文件
    ...
    ```lang-text   
    ----------------------------------------------------------------------------
    Created with an evaluation version of VMware InstallBuilder
