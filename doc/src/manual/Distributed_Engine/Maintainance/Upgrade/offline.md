...
下述升级过程以 SequoiaDB v3.4.4 升级至 v5.0.4 为例进行说明，其他版本间的升级操作基本一致。

1. 以 root 用户登陆目标主机，解压 SequoiaDB 巨杉数据库产品包，并为解压得到的 `sequoiadb-5.0.4-linux_x86_64-installer.run` 安装包赋可执行权限

    ```lang-bash
    # tar -zxvf sequoiadb-5.0.4-linux_x86_64-installer.tar.gz
...
2. 运行 `sequoiadb-5.0.4-linux_x86_64-installer.run` 包
   ...
   ```lang-bash
   $ ./sequoiadb-5.0.4-linux_x86_64-installer.run --mode text
   ```
