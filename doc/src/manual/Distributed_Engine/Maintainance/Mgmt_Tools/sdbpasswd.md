...
| ----------- | ---- | ------------------------------------------------- | -------- |
| --help      | -h   | 返回帮助说明                                      |    否    |
| --adduser   | -a   | 增加用户，支持使用字符“@”区分同名用户，格式：user@cluster1。 |  adduser/removeuser 必选其一 |
...
> - 用户输入数据库用户和密码时，长度均不能超过 256 字节。同时，当用户输入的密码包含特殊字符“!”、“&”和“$”时，需在字符前添加转义符(\\)。
...
* 增加用户 sdbadmin，且通过字符“@”区分从属于不同集群的用户
   ...
   > **Note:**
   >
   > 数据库工具在使用加密文件方式连接数据库时，会自动去掉字符“@”及后面部分，使用原始用户名连接数据库。
...
* 删除用户 sdbadmin@db1
   ...
   > **Note:**
   >
   > 删除用户时，对于使用了字符“@”的用户，需要指定全名才能匹配删除。
