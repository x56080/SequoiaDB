...
- 在线恢复：将正在运行的集群恢复至指定的一致性时间点，使用户能够快速地还原数据库到过去某个时间的状态，回滚不需要的修改。用户指定的一致性时间点需存在于当前使用的备份日志中，且该日志还有足够的空间可以写入，否则恢复将无法进行。
...
5. 在使用 sdbrestore 恢复后重启集群，将自动开启恢复模式，用于进行时间点恢复。
    ...
    开启恢复模式的集群将会阻止数据访问，特殊情况下如果不需要进行全局一致性恢复，用户可通过 [restoreAbort()][abort] 中止恢复模式，这时数据库将可以提供对外服务，但不保证数据的全局一致性。
...
6. 如果需要保证集群的全局一致性，可以执行时间点恢复

    用户可根据实际情况选择，将集群恢复至最新的一致性时间点或恢复至指定时间点。
...
SequoiaDB 巨杉数据库中，部分操作会修改数据组中的数据，但属于非事务性的操作，因此在时间点恢复中被认为是不可逆操作。时间点恢复功能的实现基于全局一致性时间，SequoiaDB 使用[时间序列协议][stp]为分布式事务分配全局时间。由于非事务操作没有全局时间，不能按照时间点进行回滚，并且会切断有效的一致性时间窗口。因此，建议用户避免在事务操作中混用非事务性操作，并在执行非事务性操作之前对数据进行备份。
