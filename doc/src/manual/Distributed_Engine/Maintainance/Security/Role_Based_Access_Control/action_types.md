...
用户执行[watch](manual/Manual/Sequoiadb_Command/Sdb/watch.md)命令时根据参数需要不同的权限：

- 如果参数指定了集合，需要的权限为该集合资源`{ cs: "<cs name>", cl: "<cl name>" }`上的 changeStream 和 find 操作。
- 如果参数指定了集合空间，需要的权限为该集合空间资源`{ cs: "<cs name>", cl: "" }`上的 changeStream 和 find 操作。
- 如果参数未指定集合空间和集合，需要的权限为非系统表资源`{ cs: "", cl: "" }`上的 changeStream 和 find 操作。