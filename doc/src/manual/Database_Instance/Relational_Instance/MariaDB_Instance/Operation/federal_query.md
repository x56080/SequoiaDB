...
3. 指定关键词进行查询，在 SQL 语句中指定 /*+sql_pushdown=sdb*/ 关键词进行联邦查询
...
5. 修改配置文件 `<INSTALL_DIR>/Setup/odbc.ini` 中[Simba Spark 64-bit]下的 HOST 和 PORT 参数，分别修改为 Spark 实例 thriftserver 的 IP 和监听端口（\<INSTALL_DIR\>为 SimbaSparkODBC 安装路径，用户可根据实际填写）
...
9. 在 SparkSQL 中准备数据
    ...
    spark-sql>INSERT INTO employee VALUES(1, "Jacky", 36);
    spark-sql>INSERT INTO employee VALUES(2, "Alice", 18);
    spark-sql>INSERT INTO manager VALUES(1, "Wireless Business");
    ```

10. 设置并打开联邦查询开关（spark_odbc_ini_path 为 SimbaSparkODBC odbc.ini 所在路径，用户可根据实际情况修改）
...
11. 在 MariaDB 中指定关键词 /*+sql_pushdown=spark*/ 进行联邦

    ```lang-sql
    MariaDB [company]>select /*+sql_pushdown=spark*/ count(*) as num from test.test_m9 a, test.test_i9 b, test.test_c9 c where a.customer_no = b.customer_no and b.customer_no = c.cust_no;
    ```
...
[simba_spark_odbc_download]:databricks.com/spark/odbc-drivers-download
