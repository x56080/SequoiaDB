SequoiaSQL-MariaDB 集群架构中，每个 MariaDB 实例均为主机模式，都可对外提供读写服务。因此 SequoiaSQL-MariaDB 提供了实例组功能，用于为应用提供统一的元数据视图，并保证集群的元数据一致性。同时，当一个 MariaDB 实例退出实例组后，连接该实例的应用可以切换到实例组内的其他实例，获得对等的读写服务，以保证服务的高可用。
...
实例组管理工具包括实例组初始化工具、实例组配置查看工具和实例组配置清除工具。
...
- **使用说明**
   ...
   ```lang-bash
   $ bin/ha_inst_group_init sql_group
   ```
...
- **使用说明**
   ...
   * 查看实例组 group2 中所有实例的配置信息
     ...
     ```lang-bash
     $ bin/ha_inst_group_list --name group2
     ```
...
- **使用说明**
   ...
   * 清除实例组 group1 中所有实例的配置信息
     ...
     ```lang-bash
     $ bin/ha_inst_group_clear group1
     ```
   ...
   * 清除实例组 group2 中服务端口为 3306 的实例配置
     ...
     ```lang-bash
     $ bin/ha_inst_group_clear group2 --inst_host="sdbserver:3306" --force
     ```
...
> **Note：**
>
> 清除实例组或实例配置并不会删除实例，删除实例需要用户手动使用 sdb_sql_ctl 命令完成。
...
3. 创建两个 MariaDB 实例并加入到实例组 mariadb
   ...
   > **Note：**
   >
   > sdb_maria_ctl 的使用可参考[实例管理工具][sdb_maria_ctl]。
