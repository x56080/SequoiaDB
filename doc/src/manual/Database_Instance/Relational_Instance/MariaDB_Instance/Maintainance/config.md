...
该参数可以配置 MariaDB 实例清理统计信息缓存的时间阈值，单位为小时，仅在开启实例组功能后生效。对表执行 CRUD 操作时，实例将比对该表上一次生成统计信息的时间。如果时间间隔超过该阈值，实例将清理统计信息缓存后再执行当前操作。实例自动清理表的统计信息缓存后，将通知同实例组的其他实例执行相同操作。
