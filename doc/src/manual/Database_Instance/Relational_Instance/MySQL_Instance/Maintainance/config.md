...
SequoiaDB 慢日志功能可参考 [慢日志][slowLog] 章节说明。

**slow_query_log**

该参数可以开启 MySQL 慢查询日志记录功能。
...
该参数可以设置 MySQL 慢查询日志输出形式。在 slow_query_log 参数为 ON 时生效。取值为 "FILE"、"TABLE" 和 "NONE" 之中的一个或多个。选择 "FILE" 则输出到日志文件；选择 "TABLE" 则输出到 mysql 系统库的表中；选择 "NONE" 则不输出。
...
该参数可以设置 MySQL 慢日志输出的系统表。在 log_output 为 "TABLE" 时生效。ON 时输出到 mysql.sdb_slow_log 表；OFF 时输出到 mysql.slow_log 表。
...
该参数可以设置 MySQL 慢日志阈值。如果查询耗时超过这个阈值，在慢日志开启时，会记录慢日志。单位为秒。
...
**long_query_include_lock_time**

该参数可以设置 MySQL 慢日志阈值是否包含元数据等锁时间。ON 时，慢日志阈值时间包含元数据等锁时间，可以记录因元数据锁等待而缓慢的查询；OFF 时，慢日志阈值时间不包含元数据等锁时间，单纯是查询在执行中的时间。
...
**sdb_slow_log_expire_days**

该参数可以设置 MySQL 慢日志记录在 mysql.sdb_slow_log 表中的过期时间。超出指定天数后慢日志记录会被清除。
...
[slowLog]:manual/Database_Instance/Relational_Instance/MySQL_Instance/Maintainance/slow_log.md
