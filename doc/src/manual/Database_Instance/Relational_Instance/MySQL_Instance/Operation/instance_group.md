SequoiaSQL-MySQL 集群架构中，每个 MySQL 实例均为主机模式，都可对外提供读写服务。因此 SequoiaSQL-MySQL 提供了实例组功能，用于为应用提供统一的元数据视图，并保证集群的元数据一致性。同时，当一个 MySQL 实例退出实例组后，连接该实例的应用可以切换到实例组内的其他实例，获得对等的读写服务，以保证服务的高可用。
...
3. 创建两个 MySQL 实例并加入到实例组 mysql
   ...
   > **Note：**
   >
   > sdb_mysql_ctl 的使用可参考[实例管理工具][sdb_mysql_ctl]。
