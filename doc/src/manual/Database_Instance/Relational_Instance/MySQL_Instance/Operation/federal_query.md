...
12. 在 MySQL 中指定关键词 /*+sql_pushdown=spark*/ 进行联邦
    ...
    ```lang-sql
    mysql> select /*+sql_pushdown=spark*/ count(t1.employee_id) as cnt from company.manager as t1 inner join company.employee as t2 on t1.employee_id = t2.id /*+ use_hash() */;
    ```
