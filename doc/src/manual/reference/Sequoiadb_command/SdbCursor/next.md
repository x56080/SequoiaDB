...
获取当前游标指向的下一条记录，更多查看 [cursor.current()](reference/Sequoiadb_command/SdbCursor/current.md) 方法。
...
* 选择集合 bar 下 age 大于8的记录，返回当前游标指向的下一条记录

 ```lang-javascript
> db.foo.bar.find( { age: { $gt: 8 } } ).next()
{
      "_id": {
