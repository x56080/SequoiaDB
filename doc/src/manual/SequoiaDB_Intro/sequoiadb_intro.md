...
SequoiaDB 巨杉数据库是一款基于湖仓一体架构的金融级分布式数据库，主要面对高并发实时处理型场景提供高性能、可靠稳定以及无限水平扩展的数据库服务。

用户可以在 SequoiaDB 巨杉数据库中创建多种类型的数据库实例，以满足上层不同应用程序各自的需求。 

SequoiaDB 支持 SequoiaDB API、MySQL、S3 对象数据引擎接口，可实现跨结构化、半结构化和非结构化的多模数据处理能力。同时，SequoiaDB 提供了 Apache SparkSQL 生态兼容能力，以及强劲的数据分析性能。
