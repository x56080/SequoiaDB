...
获取当前游标指向的记录，更多查看 [cursor.next()](manual/Manual/Sequoiadb_Command/SdbCursor/next.md)  方法。
...
返回当前游标指向的记录，出错抛异常，并输出错误信息，可以通过
...
* 选择集合 employee 下 age 大于8的记录，返回当前游标指向的记录


 ```lang-javascript
> db.sample.employee.find( { age: { $gt: 8 } } ).current()
{
      "_id": {
