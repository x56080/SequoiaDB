...
| 参数名 			| 参数类型 	| 描述 		| 是否必填 |
| ------ 			| ------ 	| ------ 	| ------   |
| snapType 			| 枚举 		| [快照类型](manual/Manual/Snapshot/snapshot.md)。 | 是 |
| cond 				| Json 对象 | 设置匹配条件以及[命令位置参数](manual/Manual/Sequoiadb_Command/location.md)。 	| 否 |
| sel 				| Json 对象 | 选择返回字段名。为 null 时，返回所有的字段名。 	| 否 |
| sort 				| Json 对象 | 对返回的记录按选定的字段排序。1为升序；-1为降序。 | 否 |
| SdbSnapshotOption	| Json 对象 | 使用一个对象来指定快照查询参数，使用方法请参考 [SdbSnapshotOption](manual/Manual/Sequoiadb_Command/AuxiliaryObjects/SdbSnapshotOption.md) | 否 |

> **Note:**

>* snapType 字段的值请参考 [快照类型](manual/Manual/Snapshot/snapshot.md)。
