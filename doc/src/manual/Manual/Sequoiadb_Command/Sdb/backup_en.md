...
- MKPublicFile ( *string* ): Specify the [master key][encryption] path, only when backing up encrypted files.

    - If the master key file is lost, the user can recreate it through the tool [sdbmkgen][sdbmkgen].
    - This parameter supports specifying a key file generated by an external tool, and the user needs to ensure that the file contains public key information.

    Format: `MKPublicFile: "/path/test/mk.pem"`
