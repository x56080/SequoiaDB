...
| MinRecoverableTime | string | 当前使用的备份日志中，所记录的第一个事务开始时间 |
| MaxTransCommitTime | string | 当前使用的备份日志中，所记录的最后一个事务提交时间  |
| LogLimitTime | string | 当前使用的备份日志中，所记录的最后一条日志的时间（该字段仅备份日志空间不足时显示） |
| FailedGroups | string | 不能执行恢复的复制组名（该字段仅备份日志空间不足时显示） |
...
- 检查指定时间点是否为可恢复的一致性时间点
    ...
    如果集群中某一复制组的备份日志没有足够的空间可以写入，即使指定的时间点为可恢复的一致性时间点，也不进行恢复，输出结果如下：
