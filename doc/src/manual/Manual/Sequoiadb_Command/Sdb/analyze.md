...
无返回值，出错抛异常，并输出错误信息。可以通过 [getLastErrObj()](manual/Manual/Sequoiadb_Command/Global/getLastErrObj.md)  或 [getLastErrMsg()](manual/Manual/Sequoiadb_Command/Global/getLastErrMsg.md) 获取错误信息 或 通过 [getLastError()](manual/Manual/Sequoiadb_Command/Global/getLastError.md) 获取错误码。该操作主要的异常如下：
...
4. 对指定集合"sample.employee"进行统计信息收集，并且指定Sample的数量

  ```lang-javascript
  > db.analyze( { Collection : "sample.employee", SampleNum : 1000 } )
  ```

5. 对指定集合"sample.employee"的索引"index"进行统计信息收集

  ```lang-javascript
  > db.analyze( { Collection : "sample.employee", Index : "index" } )
  ```

6. 对指定集合"sample.employee"生成清空统计信息缓存
