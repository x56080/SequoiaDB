...
该函数用于容灾恢复。停止集群的 Maintenance 模式，恢复正常运行状态。
...
- HostName（ *string* ）：Maintenance 模式停止生效的主机名（与 Location 互斥）

    指定的主机名需存在于当前集群中，该主机上的所有节点将停止 Maintenance 模式。

    格式：`HostName: "sdbserver"`

- Location（ *string* ）：Maintenance 模式停止生效的位置集（与 HostName 互斥）

    指定的位置集需存在于当前集群中，该位置集所有复制组节点都会停止 Maintenance 模式。
...
| SucceedNum | number | 成功停止 Maintenance 模式的复制组数量 |
...
`stopMaintenanceMode()` 函数常见异常如下：

| 错误码 | 错误类型 | 可能发生的原因 | 解决办法 |
| ------ | -------- | -------------- | -------- |
| -6 | SDB_INVALIDARG | 参数类型错误或参数值超出范围 | 检查参数类型和取值范围是否正确 |
| -334 | SDB_OPERATION_CONFLICT | 复制组处于 Critical 模式 | 检查复制组状态 |
...
**示例2：** 仅对指定域的数据组停止 Maintenance 模式
