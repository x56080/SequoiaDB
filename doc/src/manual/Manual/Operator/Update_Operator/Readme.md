...
| [$saveMin][saveMin] | 当指定值小于字段值时，将字段值更新为指定值 | db.sample.employee.update({$saveMin:{score:60}}) |
| [$saveMax][saveMax] | 当指定值大于字段值时，将字段值更新为指定值 | db.sample.employee.update({$saveMax:{score:80}}) |
