...
| RecycleDataSize | int64     | 回收站项目占用集合空间数据文件的总大小，单位为字节（仅在 v5.0.4 及以上版本生效）|
| RecycleIndexSize| int64     | 回收站项目占用集合空间索引文件的总大小，单位为字节（仅在 v5.0.4 及以上版本生效）|
| RecycleLobSize  | int64     | 回收站项目占用集合空间大对象文件的总大小，单位为字节（仅在 v5.0.4 及以上版本生效）|
