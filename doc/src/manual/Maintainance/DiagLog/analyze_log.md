...
用户在使用 SequoiaDB 巨杉数据库的过程中，如果遇到一些由数据库抛出的异常错误，可以通过 [sdbdiag.log][diaglog] 日志来定位问题。在进行集群日志分析前，用户可以先参考 SequoiaDB 的[数据库存储引擎][engine_arch]，了解 SequoiaDB 中各类节点在请求执行中的作用。
...
3. 查看[数据节点][data_node]的诊断日志，通过分析判断在连接 `sdbserver3:11821` 时失败，而 11821 端口时 11820 节点占用；节点的当前存活数量为：2，需要存活的节点数量为：3
...
[^_^]:
     本文使用的所有引用和链接
[diaglog]:manual/Maintainance/DiagLog/diaglog.md
[coord_node]:manual/infrastructure/Node/coord_node.md
