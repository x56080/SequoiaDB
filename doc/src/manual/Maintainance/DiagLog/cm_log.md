...
集群管理诊断日志级别从高到低分为六个级别，分别以数字 0~5 表示，默认日志级别为 WARNING，对应日志级别见下表。详情见[数据库配置][database_conf]。
...
用户可以通过 [updateConf][update_conf] 命令修改日志级别（diaglevel 参数），如修改日志级别为 DEBUG：

```lang-javascript
> Oma.updateConf( { diaglevel:5} ) 
```

[^_^]: 
    TODO: cm 的配置需要通过 oma 完成，目前 oma 相关功能开发中，后续更新
