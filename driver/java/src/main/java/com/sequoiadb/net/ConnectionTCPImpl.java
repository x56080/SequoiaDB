...
public class ConnectionTCPImpl implements IConnection {
    ...
    public ByteBuffer receiveMessage(boolean endianConvert) throws BaseException {
        ...
        if (input == null) {
            throw new BaseException("SDB_SYS", "input stream is null");
        }
        ...
        try {
            ...
            while (rtn < 4) {
                ...
                if (retSize == -1) {
                    throw new BaseException("SDB_NETWORK",
                            String.format("failed to get the message length, read size is: %d", rtn));
                }
                rtn += retSize;
            }
            ...
            while (rtn < msgSize) {
                ...
                if (retSize == -1) {
                    throw new BaseException("SDB_NETWORK",
                            String.format("failed to get the message body, message size is: %d, has read: %d",
                                    msgSize, rtn));
                }
                rtn += retSize;
            }

            // if the byte count we read is not equal with the message length, throw error
            if (msgSize != rtn) {
                throw new BaseException("SDB_NETWORK",
    ...
    public byte[] receiveSysInfoMsg(int msgSize) throws BaseException {
        ...
        try {
            ...
            while (rtn < msgSize) {
                ...
                if (retSize == -1) {
                    throw new BaseException("SDB_NETWORK",
                            String.format("failed to get the system info message, total size is: %d, read size is: %d",
                                    msgSize, rtn));
                }
                rtn += retSize;
            }
