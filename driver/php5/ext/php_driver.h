...
#ifndef PHP_DRIVER_H__
#define PHP_DRIVER_H__
...
{\
   ...
   {\
      pSequoiaDB = zend_read_property( Z_OBJCE_P( thisObj ), \
                                       thisObj, \
                                       ZEND_STRL("_SequoiaDB"), \
                                       0 TSRMLS_CC ) ;\
   }\
...
{\
   ...
   pReturnModel = zend_read_property( Z_OBJCE_P( pSequoiaDB ), \
                                      pSequoiaDB, \
...
//php function return json or array errno and id
...
{\
   ...
   {\
      ...
      {\
         ...
         add_assoc_string( return_value, "_id", pId, 1 ) ;\
      }\
   }\
   else\
   {\
...
{\
   zval *pReturnModel = NULL ;\
   pReturnModel = zend_read_property( Z_OBJCE_P( thisObj ), \
                                      thisObj, \
                                      ZEND_STRL("_return_model"), \
                                      0 TSRMLS_CC ) ;\
   if( Z_LVAL_P( pReturnModel ) == 1 )\
...
for( zend_hash_internal_pointer_reset( pTable ) ; \
   HASH_KEY_NON_EXISTANT != zend_hash_get_current_key_type_ex( pTable,\
                                                               NULL ) ; \
