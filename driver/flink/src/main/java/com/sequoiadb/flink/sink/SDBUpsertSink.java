...
import com.sequoiadb.flink.sink.writer.SDBUpsertSinkWriter;
...
public class SDBUpsertSink implements Sink<RowData, Void, Void, Void> {

    private static final Logger LOG = LoggerFactory.getLogger(SDBUpsertSink.class);

    private SDBDataConverter converter;

    private SDBSinkOptions sinkOptions;

    public SDBUpsertSink(SDBDataConverter converter, SDBSinkOptions sinkOptions) {
        this.converter = converter;
        this.sinkOptions = sinkOptions;
    }

    /**
     * create writer for upsert changelog stream
     *
    ...
    public SinkWriter<RowData, Void, Void> createWriter(
        ...
        return new SDBUpsertSinkWriter(converter, sinkOptions);
    }
