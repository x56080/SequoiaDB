...
*******************************************************************************/
/*
@description: sync business configure ( mariadb )
@modify list:
   2019-05-28 JiaWen He  Init

@parameter
   var BUS_JSON = {"ClusterName":"myCluster1","BusinessName":"MySQLInstance1","BusinessType":"sequoiasql-mariadb","User":"sac","Passwd":"123", "Address":[{"HostName":"ubuntu-test-03","svcname":"3306"}],"omaddr":"ubuntu-test-01:11785"} ;
*/
...
{
   var defaultFilePath = '/etc/default/sequoiasql-mariadb' ;
   var installPath = '' ;
...
function _getConfig( PD_LOGGER, oma, dbpath )
{
   ...
   {
      var error = _getErrorResult( "Failed to get mariadb config, detail: ?" ) ;
      PD_LOGGER.log( PDERROR, error ) ;
      throw error ;
   }
...
function _findInstanceConfig( PD_LOGGER, oma, businessName,
                              hostName, mariadbPort, installPath )
{
   ...
   for( var index in fileList )
   {
      ...
      if( isUnknow )
      {
         ...
         if( instanceConfig[FIELD_PORT2] == mariadbPort )
         {
            isFind = true ;
...
function testConnect( PD_LOGGER, hostName, agentPort, installPath, port, user, passwd )
{
   ...
   try
   {
      ExecSsql3( cmd, installPath, hostName, port, user, passwd, 'mysql', sql ) ;
   }
   catch( e )
   {
