...
public class CreateObjectWithReStartS3N18205 extends S3TestBase {
        ...
        public void exec() throws Exception {
            ...
            try {
                PutObjectResult result = s3Client.putObject( bucketName,
                        objectName, content );
                versionAndMd5Map.put( result.getVersionId(),
    ...
    private void checkRandomObjMd5() throws Exception {
        File localPath = null;
        // 检查当前版本对象内容
        ObjectMetadata metadata = s3Client.getObject( bucketName, objectName )
                .getObjectMetadata();
        String versionId = metadata.getVersionId();
        String actMd5 = metadata.getETag();
        String expEtag = versionAndMd5Map.get( versionId );
        Assert.assertEquals( actMd5, expEtag,
                "keyName = " + objectName + ", versionid = " + versionId );
        ...
        String downfileMd5 = ObjectUtils.getMd5OfObject( s3Client, localPath,
                bucketName, objectName, String.valueOf( version ) );
        String expEtag2 = versionAndMd5Map.get( String.valueOf( version ) );
        Assert.assertEquals( downfileMd5, expEtag2,
                "keyName = " + objectName + ", versionid = " + version );
