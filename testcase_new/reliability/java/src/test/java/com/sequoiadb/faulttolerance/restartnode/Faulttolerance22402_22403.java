...
/**
 * @Description seqDB-22402熔断模式下节点故障，指定replSize为0/-1/3 seqDB-22403半容错模式下节点故障，指定replSize为0/-1/3
 * @author wuyan
 * @Date 2020.07.08
 * @version 1.00
 */
public class Faulttolerance22402_22403 extends SdbTestBase {
    ...
    public void test(int ftlevel) throws Exception {
        ...
        FaultMakeTask faultMakeTask = NodeRestart.getFaultMakeTask(slave, 1, 10);
    ...
    private void insertData(DBCollection cl) {
        for (int i = 0; i < 100; i++) {
            ...
            for (int j = 0; j < 500; j++) {
                ...
                record.put("test", j);
                record.put("teststr",
                        "fjsldkfjlksdjflsdljfhjdshfjksdhfsdfhsdjkfhjkdshfj" + "kdshfkjdshfkjsdhfkjshafdkhasdikuhsdjfls"
                                + "hsdjkfhjskdhfkjsdhfjkdshfjkdshfkjhsdjkf" + "aaaaaaaa");
                records.add(record);
            }
            cl.insert(records);
        }
    ...
    private void checkInsertResult(String csName, String clName) {
        for (int i = 0; i < replSizes.length; i++) {
            ...
            long expCount = 50000;
            if (i == 1 || i == 2) {
                Assert.assertEquals(count, expCount, "the cl " + clName + "_" + i + " data num is error!");
            } else {
                if (count >= expCount) {
                    Assert.fail("the cl " + clName + "_" + i + " data num is error! act num is " + count);
                }
            }
            ...
            Assert.assertEquals(count1, 1, "the cl " + clName + "_" + i + " insert context is:" + record);
