...
public class CommLib {
     ...
     * @return true/false, true is success, false is failed
     */
    public void checkCLOfCatalog(Sequoiadb sdb, String csName, String clName) {
        try {
            BSONObject matcher = new BasicBSONObject();
            BSONObject subObj = new BasicBSONObject();
     ...
     * @param sdb
     */
    public void createNode(Sequoiadb sdb, String rgName, int portStart, int portStop, String path) {
        try {
            // create dataRG
     ...
     * 
     * @param db
     * @param match 匹配context正则表达式
     * @param timeout 超时时间，单位为s.(context超时时间默认为4分钟，建议设置5分钟)
     * @param exceptCata 是否排除编目组
     */
    public static void waitContextClose(Sequoiadb db, String match, int timeout, boolean exceptCata) {
        String matcher = "{'$and':[{'GroupName': {'$ne': 'SYSCoord'}}], 'IsPrimary': true, 'RawData': true}";
        if (exceptCata) {
            matcher = "{'$and':[{'GroupName': {'$ne': 'SYSCoord'}}, {'GroupName': {'$ne': 'SYSCatalogGroup'}}],"
                    + " 'IsPrimary': true, 'RawData': true}";
        ...
        while (snapshot.hasNext()) {
            nodes.add(snapshot.getNext().get("NodeName").toString());
        }
        ...
        while (true) {
            ...
            for (int i = 0; i < nodes.size(); i++) {
                try (Sequoiadb dataConn = new Sequoiadb(nodes.get(i), "", "")) {
                    ...
                    DBCursor cur = dataConn.getSnapshot(Sequoiadb.SDB_SNAP_CONTEXTS,
                            "{'Contexts': {'$elemMatch': {'Description': {'$regex': '" + match + "'}}}}", null, null);
                    if (cur.hasNext()) {
            ...
            if (success) {
                ...
                Assert.fail("Failed to wait for context timeout shutdown, the timeout time is: " + timeout
                        + "s, context: " + errorContext);
            } else {
                timeout--;
                try {
                    Thread.sleep(1000);
