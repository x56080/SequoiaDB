...
public class faulttolerance22208 extends SdbTestBase {
    ...
    public void setUp() throws ReliabilityException {
        ...
        cs.createCollection(clName, (BSONObject) JSON.parse("{'Group': '" + groupName + "'}"));

        for (int i = 0; i < replSizes.length; i++) {
            BSONObject option = (BSONObject) JSON
                    .parse("{Group:'" + groupName + "'," + "ReplSize:" + replSizes[i] + " }");
    ...
    public void test() throws ReliabilityException, InterruptedException {
        ...
        FaultToleranceUtils.checkNodeStatus(nodes.get(0).connect().toString(), "");
        FaultToleranceUtils.checkNodeStatus(nodes.get(1).connect().toString(), "");
            ...
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                ...
                for (int i = 0; i < 1000; i++) {
                    ...
                    for (int j = 0; j < 5000; j++) {
                        ...
                        record.put("b", j);
                        record.put("order", j);
                        record.put("str", "fjsldkfjlksdjflsdljfhjdshfjksdhfsdfhsdjkfhjkdshfj"
                                + "kdshfkjdshfkjsdhfkjshafdkhasdikuhsdjfls" + "hsdjkfhjskdhfkjsdhfjkdshfjkdshfkjhsdjkf"
        ...
        public void exec() throws Exception {
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                ...
                for (int i = 0; i < 1000; i++) {
                    ...
                    cl.update(null, "{$inc:{a:1, b:1}, $set:{'str':'update str times " + i + "'}}", null);
                }
            }
        }
    }
    ...
    private class TestSlowNode extends OperateTask {
        ...
        public void exec() throws Exception {
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                ...
                do {
                    String ftStatus1 = FaultToleranceUtils.getNodeFTStatus(db, nodes.get(0));
                    String ftStatus2 = FaultToleranceUtils.getNodeFTStatus(db, nodes.get(1));
                    if (ftStatus1.equals(ftmask) && ftStatus2.equals(ftmask)) {
                        isSlowNode = true;
                    ...
                    if (alreadyWaitTime > maxWaitTime) {
                        System.out.println(
                                "---node status is not slownode, in maxWaitTime ! waitTime is" + alreadyWaitTime);
                        Assert.fail();
                    }
                } while (!isSlowNode);
    ...
    private void insertAndCheckResult(Sequoiadb sdb, String csName, String clName, boolean isInsertAgain) {
        for (int i = 0; i < replSizes.length; i++) {
            DBCollection dbcl = sdb.getCollectionSpace(csName).getCollection(clName + "_" + i);
            BSONObject record = new BasicBSONObject();
            record.put("a", "text22208_test" + i);
            if (i == 1 || isInsertAgain) {
                dbcl.insert(record);
                long count = dbcl.getCount(record);
                Assert.assertEquals(count, 1, "the cl " + clName + "_" + i + " insert context is:" + record);
            } else {
                try {
                    dbcl.insert(record);
                } catch (BaseException e) {
                    if (e.getErrorCode() != -105 && e.getErrorCode() != -13 && e.getErrorCode() != -252) {
                        throw e;
                    }
                }
            }
