...
public class FaultToleranceUtils {
    public static void changeNodeStatus(String csName, String clName, int errorCode) {
        ...
        try {
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                for (int j = 0; j < 1000; j++) {
                    insertor.add((BSONObject) JSON.parse("{ 'a': '" + strBuffer.toString() + "' }"));
                }
                sdb.getCollectionSpace(csName).getCollection(clName).insert(insertor, 0);
            }
    ...
    public static String getNodeFTStatus(Sequoiadb sdb, NodeWrapper nodes) {
        DBCursor cursor = sdb.getSnapshot(Sequoiadb.SDB_SNAP_DATABASE,
                "{ 'RawData': true, 'NodeName': '" + nodes.connect().toString() + "' }", "{ 'FTStatus': '' }", null);
    ...
    public static void checkNodeStatus(String nodeName, String ftStatus) {
        try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
            ...
            do {
                DBCursor cursor = sdb.getSnapshot(Sequoiadb.SDB_SNAP_DATABASE,
                        "{ 'RawData': true, 'NodeName': '" + nodeName + "' }", "{ 'FTStatus': '' }", null);
                ...
                if (alreadyWaitTime > maxWaitTime) {
                    Assert.fail("---node status is " + ftStatus + " in maxWaitTime ! waitTime is" + alreadyWaitTime);
                }
    ...
    public static void insertError(String csName, String clName, int errorCode) {
        Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "");
        try {
            sdb.getCollectionSpace(csName).getCollection(clName).insert("{ 'a': 0 } ");
            if (errorCode != 0) {
                Assert.fail("insert record to " + clName + " should be failed!");
            }
        } catch (BaseException e) {
