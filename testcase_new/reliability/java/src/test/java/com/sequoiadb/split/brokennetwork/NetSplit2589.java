...
/**
 * @FileName:SEQDB-2589 对range分区组进行百分比切分，切分时cata主节点断网
 * 
 * @author huangqiaohui
 * @version 1.00
 *
...
public class NetSplit2589 extends SdbTestBase {
    private String clName = "testcaseCL2589";
    ...
    public void test() {
        ...
        try {
            ...
            if ( splitComplete ) {
                ...
                Assert.assertTrue( actualCount <= 11000,
    ...
    private void checkGroupData( Sequoiadb sdb, String groupName, String macher,
            int lowBoundCount, int upBoundCount ) {
        ...
        try {
            ...
            Assert.assertEquals(
                    macherCount == count
                            && lowBoundCount <= count && count <= upBoundCount,
                    true,
                    destGroupName + " count:" + count + " macherCount:"
                            + macherCount + " lowBoundCount:" + lowBoundCount
                            + "upBound:" + upBoundCount );
        } catch ( BaseException e ) {
            e.printStackTrace();
            Assert.fail( e.getMessage() + "\r\n" + Utils.getStackString( e ) );
        } finally {
