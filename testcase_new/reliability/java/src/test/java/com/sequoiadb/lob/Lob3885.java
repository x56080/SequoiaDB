...
public class Lob3885 implements StandTestInterface {
    ...
    /**
     * 1.在集合上并发执行以下操作： (1)不停写Lob，覆盖不同lob大小，例如：200k、500M ; (2)不停地删除，并发操作时，备节点正常重启
     * 2.故障恢复后，再次进行写/删除lob
     *
    ...
    public void test() throws ReliabilityException {
        ...
        LobTask createLobsTask = LobTask.getCreateLobsTask( 1000, data,
                createIds );
        createLobsTask.setName( "create lobs task" );
        LobTask deleteLobsTask = LobTask.getDeleteLobsTask( objectIds,
                deteledIdMap );
