...
/**
 * @FileName:seqDB-23650未刷盘时server组主节点多次被强杀检查选主LLT的值和事务操作 
 *           
 * @author chensiqin
 * @version 1.00
 *
...
public class KillPrimaryServer23650  extends SdbTestBase{
    ...
    /*
     *  1.server主节点未刷盘（刷盘时间默认60s）的情况下，多次强杀stp主节点导致主节点切主，
         *   通过stpq --time检查LLT的正确性，过程中同时做转账操作 
     *  2.server主节点未刷盘（刷盘时间默认60s）的情况下，多次强杀stp主节点（未切主），
         *    通过stpq --time检查LLT的正确性，过程中同时做转账操作
     * */
    
    @Test
    public void test23650() throws InterruptedException {
        try {
            //获取stp 主节点getStpNode
