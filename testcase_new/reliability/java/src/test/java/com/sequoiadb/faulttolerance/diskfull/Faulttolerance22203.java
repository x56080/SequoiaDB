...
/**
 * @Description seqDB-22203:: 容错级别为全容错，1个副本状态为:NOSPC或者DEADSYNC，不同replSize的集合中插入数据
 * @author wuyan
 * @Date 2020.06.09
 * @version 1.00
 */
public class Faulttolerance22203 extends SdbTestBase {
    ...
    public void test(String ftmask) throws Exception {
        ...
        FaultToleranceUtils.checkNodeStatus(slave.connect().toString(), ftmask);
    ...
    private class PutLobs extends OperateTask {
        ...
        public void exec() throws Exception {
            try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                DBCollection cl = sdb.getCollectionSpace(csName).getCollection(clName + "_1");
                for (int i = 0; i < 1000; i++) {
                    String ftStatus = FaultToleranceUtils.getNodeFTStatus(sdb, nodes);
