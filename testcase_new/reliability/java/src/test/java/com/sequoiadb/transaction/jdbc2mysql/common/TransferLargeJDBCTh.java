...
public class TransferLargeJDBCTh extends TransferJDBCTh {
    ...
    private static final Logger log = Logger.getLogger(TransferLargeJDBCTh.class.getName());
            ...
            while (count++ < 10) {
                try {
                    if (!TransUtil.runFlag) {
                        log.info("transfer thread " + getName() + " stop in 10s");
                        break;
                    }
                    ...
                    String sql1 = "update " + getClName() + " set balance=balance-?, th_hashcode=? where account=?";
                    String sql2 = "update " + getClName() + " set balance=balance+?, th_hashcode=? where account=?";
                    String sql3 = "insert into " + getClName() + " values(NULL, ?, ?)";
                    String sql4 = "delete from " + getClName() + " where account=?";
                    ...
                    for (int i = 0; i < 5000; i++) {
                        ...
                        ps2.setString(2, hashCode() + "|" + new Date(System.currentTimeMillis()).toString() + "|"
                                + conn.hashCode() + "|" + ps2.hashCode());
                        ...
                        if (i % 1000 == 0 && i != 0) {
                            ...
                            System.out.println(getName() + hashCode() + " executeBatch update " + i);
                        }
                    }
                    int ret1[] = ps1.executeBatch();
                    int ret2[] = ps2.executeBatch();
                    ...
                    for (int i = getInsertNum(); i < getInsertNum() + 10000; i++) {
                        ps3.setInt(1, 10000);
                        ps3.setInt(2, i);
                        ps3.addBatch();
                        if (i != 0 && 0 == i % 1000) {
                            ps3.executeBatch();
                            ps3.clearBatch();
                            System.out.println(getName() + hashCode() + " executeBatch insert " + i);
                        }
                    }
                    ps3.executeBatch();
                    ...
                    for (int i = getInsertNum(); i < getInsertNum() + 10000; i++) {
                        ...
                        if (i != 0 && 0 == i % 1000) {
                            ps4.executeBatch();
                            ps4.clearBatch();
                            System.out.println(getName() + hashCode() + " executeBatch delete " + i);
                        }
                    }
                    ps4.executeBatch();
                    conn.commit();
