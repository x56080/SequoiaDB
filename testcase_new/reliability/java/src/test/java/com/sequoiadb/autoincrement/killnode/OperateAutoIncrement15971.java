...
/**
 * @FileName seqDB-15971: 新增/修改/删除自增字段时catalog备节点异常重启 预置条件：集合中已存在多个自增字段
 *           操作步骤：1.并发执行如下操作：a.新增自增字段 b.修改自增字段属性c.删除自增字段 d.插入记录，同时catalog备节点异常重启
 *           2.再次插入记录 预期结果：1.自增字段操作成功，自增字段属性正确，记录插入成功
 *           2.待集群正常后，catalog主备节点上集合及自增字段信息一致；插入记录成功
 * @Author zhaoyu
 * @Date 2018-11-02
 * @Version 1.00
 */

public class OperateAutoIncrement15971 extends SdbTestBase {
    private String clName = "cl_15971";
    private int autoIncrementNum = 2;
    ...
    public void test() {
        ...
        try {
            ...
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(
                    cataSlave.hostName(), cataSlave.svcName(), 10 );
