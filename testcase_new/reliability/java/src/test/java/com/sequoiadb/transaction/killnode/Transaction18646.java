...
/**
 * @Description seqDB-18646:异常重启数据主节点后，执行count查询
 * @author yinzhen
 * @date 2019-6-19
 *
 */
@Test(groups = "rc")
public class Transaction18646 extends SdbTestBase {
    private Sequoiadb sdb;
    private Sequoiadb db1;
    private Sequoiadb db2;
    private String clName = "cl18646";
    ...
    public void setUp() throws ReliabilityException {
        ...
        cl.createIndex( "idx18646", "{a:1}", false, false );
    ...
    public void test() throws ReliabilityException {
        ...
        FaultMakeTask task = KillNode.getFaultMakeTask( node, 5 );
        ...
        actCount = cl.getCount( null,
                ( BSONObject ) JSON.parse( "{'':'idx18646'}" ) );
        Assert.assertEquals( actCount, expList.size() );

        cursor = cl.query( null, null, "{a:1}", "{'':'idx18646'}" );
        ...
        cursor = cl2.query( null, null, "{a:1}", "{'':'idx18646'}" );
        actList = TransUtil.getReadActList( cursor );
        Assert.assertEquals( actList, expList );
        cursor.close();
    }
