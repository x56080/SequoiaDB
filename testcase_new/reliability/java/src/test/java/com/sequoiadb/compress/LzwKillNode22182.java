...
/**
 * @FileName seqDB-22182:构建字典过程中数据组全组节点异常重启
 * @Author XiaoNi Huang
 * @Date 2020/05/11
 */

public class LzwKillNode22182 extends SdbTestBase {
    ...
    public void test() throws InterruptedException, ReliabilityException {
        ...
        FaultMakeTask faultTask;
        TaskMgr mgr = null;
        for ( int i = 0; i < nodes.size(); i++ ) {
            NodeWrapper node = nodes.get( i );
            faultTask = DiskFull.getFaultMakeTask( node.hostName(),
                    node.dbPath(), 0, 10 );
