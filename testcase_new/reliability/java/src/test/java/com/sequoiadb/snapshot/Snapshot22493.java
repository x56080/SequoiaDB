...
public class Snapshot22493 extends SdbTestBase{
    ...
    public void setup() throws ReliabilityException{
        ...
        sdb.getCollectionSpace( csName ).createCollection( clName, 
                (BSONObject)JSON.parse( "{ 'ReplSize': 1, Group: '" + groupName + "' }" ) );
    }
    
    @Test
    public void test() throws Exception{
        sdb.updateConfig( (BSONObject)JSON.parse( "{ 'syncstrategy': 'keepall' }" ) );
        ...
        do{
            ...
            DBCursor cursor = sdb.getSnapshot( Sequoiadb.SDB_SNAP_SESSIONS, "{ 'NodeSelect': 'master', "
                    + "'IsBlocked': true, 'Doing': 'Waiting for sync control' }", null, null );
            if( cursor.hasNext() ){
                isSuccess = true;
                break;
            }
