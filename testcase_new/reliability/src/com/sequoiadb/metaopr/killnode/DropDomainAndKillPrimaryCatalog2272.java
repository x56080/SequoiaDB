...
/**
* FileName: DropDomainAndKillPrimaryCatalog2272.java
* test content:when delete domain, kill -9  the catalog group master node             
* testlink case:seqDB-2272
* @author wuyan
    * @Date    2017.4.25
* @version 1.00
*/
...
public class DropDomainAndKillPrimaryCatalog2272 extends SdbTestBase {
    ...
    public void test() {        
        try {
            ...
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(masterNode.hostName(), masterNode.svcName(), 1);
    ...
    /**
	* check the result of drop domain
	* the result: 1. to drop domain success,drop the same domain again failed
	*             2. to drop domain fail,drop the same domain again successfully
	*             3. to drop domain fail,the actual drop domain successfully did not return,
	*                drop the same domain failed again
	*/
