...
public class CreateClMaster2165 implements StandTestInterface {
    ...
    /**
     * seqDB-2165 :: 版本: 1 :: 创建CL时catalog主节点断网（指定Domain）_rlb.netSplit.metaOpr.CL.003
     * <p>
     * 1、创建Domain，在域中添加两个及以上的数据组
     * 2、创建CS并指定到这个域，在该CS下创建CL（执行脚本构造循环执行创建多个CL操作）
     * 3、创建CL时catalog主节点所在主机网络中断（构造网络中断故障，如ifdown网卡）
     * 3、查看CL创建结果和catalog主节点状态
     * 4、恢复网络故障（如ifup启动网卡）
