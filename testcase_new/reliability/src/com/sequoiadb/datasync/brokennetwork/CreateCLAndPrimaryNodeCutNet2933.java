...
/**
* FileName: CreateCLAndPrimaryNodeCutNet2933.java
* test content:when create cl, the data group master node breaks the net             
* testlink case:seqDB-2933
* @author wuyan
    * @Date    2017.5.2
* @version 1.00
*/
...
public class CreateCLAndPrimaryNodeCutNet2933 extends SdbTestBase {
    ...
    public void setUp() {        
        try {
            ...
            brokenNetHost = groupMgr.getGroupByName(clGroupName).getMaster().hostName();
            if (cataPriHost.equals(brokenNetHost) && !cataGroup.changePrimary()) {
    ...
    public void test() {        
        try {
            FaultMakeTask faultTask = BrokenNetwork.getFaultMakeTask(brokenNetHost, 3, 10);
	...
	*/
    private void checkCreateCLResult() {    	  
    	...
    	//when the last cl fails,which may have actually been created successfully,may fail again
    	try {	
    		cs.createCollection(preCLName + "_" + count, option);    		    	       
        } catch (BaseException e) {            	
            // -22 SDB_DMS_EXIST  
            if (e.getErrorCode() != -22) {
            	Assert.fail("the error not -22: "+e.getErrorCode()+e.getErrorType());
            }
        }   	
    	...
    	try {
        	...
        	Assert.fail("create cl fail: "+e.getErrorCode()+e.getErrorType());            
        }   	
    	...
    	try{
    		...
    		Assert.fail( clName + " insert fail: "+e.getErrorCode()+e.getErrorType());
    	}         
    }     
    ...
    private void dropCL() {
        ...
        for (int i = 0; i < CL_NUM; i++) {
            ...
            try {
            	...
            	Assert.fail("drop cl fail: "+e.getErrorCode()+e.getErrorType());
            }
        }
    }
}