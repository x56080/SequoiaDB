...
/**
 * @FileName:SEQDB-2767 对range分区组进行范围切分，切分时cata组备节点异常重启
 * @author huangqiaohui
 * @version 1.00
 *
 */

public class KillNodeSplit2767 extends SdbTestBase {
    private String clName = "testcaseCL2767";
    ...
    public void test() {
        try {
            ...
            NodeWrapper cataSlaveNode = cataGroup.getSlave();
            System.out.println(
                    "KillNode:" + cataSlaveNode.hostName() + ":" + cataSlaveNode.svcName());

            // 建立并行任务
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(cataSlaveNode.hostName(),
                    cataSlaveNode.svcName(), 0);
    ...
    private void checkGroupData(Sequoiadb sdb, String groupName) {
        ...
        try {
            ...
            Assert.assertEquals(count, totalCount / 2, "destGroup data count:" + count);
        }
