...
/**
 * @FileName:SEQDB-2769 对range分区组进行百分比切分，切分时源备节点异常重启
 * @author huangqiaohui
 * @version 1.00
 *
 */

public class KillNodeSplit2769 extends SdbTestBase {
    private String clName = "testcaseCL2769";
    ...
    public void test() {
        try {
            ...
            NodeWrapper srcSlaveNode = srcGroup.getSlave();
            System.out
                    .println("KillNode:" + srcSlaveNode.hostName() + ":" + srcSlaveNode.svcName());

            // 建立并行任务
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(srcSlaveNode.hostName(),
                    srcSlaveNode.svcName(), 0);
