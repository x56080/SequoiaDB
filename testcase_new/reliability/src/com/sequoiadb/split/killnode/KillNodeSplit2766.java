...
/**
 * @FileName:SEQDB-2766 对range分区组进行范围切分，切分时目标组备节点异常重启
 * @author huangqiaohui
 * @version 1.00
 *
 */

public class KillNodeSplit2766 extends SdbTestBase {
    private String clName = "testcaseCL2766";
    ...
    public void test() {
        try {
            ...
            NodeWrapper destSlaveNode = destGroup.getSlave();
            System.out.println(
                    "KillNode:" + destSlaveNode.hostName() + ":" + destSlaveNode.svcName());

            // 建立并行任务
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(destSlaveNode.hostName(),
                    destSlaveNode.svcName(), 0);
