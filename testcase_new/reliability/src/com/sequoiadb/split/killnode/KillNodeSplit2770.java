...
/**
 * @FileName:SEQDB-2770 对range分区组进行百分比切分，切分时目标组主节点异常重启
 * @author huangqiaohui
 * @version 1.00
 *
 */

public class KillNodeSplit2770 extends SdbTestBase {
    private String clName = "testcaseCL2770";
    ...
    public void test() {
        try {
            ...
            NodeWrapper destMasterNode = destGroup.getMaster();
            System.out.println(
                    "KillNode:" + destMasterNode.hostName() + ":" + destMasterNode.svcName());

            // 建立并行任务
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(destMasterNode.hostName(),
                    destMasterNode.svcName(), 0);
            ...
            if (splitComplete) {
                ...
                // 源和目标数据量比对
                int bound = Utils.getBound(commSdb, csName + "." + clName, srcGroupName,
                        destGroupName);
