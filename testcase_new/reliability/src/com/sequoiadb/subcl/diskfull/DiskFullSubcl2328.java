...
public class DiskFullSubcl2328 extends SdbTestBase{
    ...
    public void test() {
        try {
            ...
            FaultMakeTask faultTask = DiskFull.getFaultMakeTask(cataPrimary.hostName(),
    ...
    public void createMainAndSubCLs() {
    	try {
			...
			for(int i = 0; i < MAINCL_NUMS; i++) {
			    cs.createCollection(mainCLName + "_" + i,
		                    (BSONObject) JSON
			 ...
			 try {
		          ...
		          for (int i = 0; i < MAINCL_NUMS; i++) {
		              ...
		              for (int j = 0; j < SUBCL_NUMS; j++) {
						  ...
						  mainCL.attachCollection(sclFullName, (BSONObject) JSON.parse("{ LowBound: { key: " + (bound * INSERT_NUMS)
				                       + " }, " + "UpBound: { key: " + (bound + 1) * INSERT_NUMS + " } }"));
						  ++bound;
				          ++lastSuccessSubCL;          
				      }
		          ...
		          System.out.println("success attach cl num is = " + (lastSuccessMainCL * lastSuccessSubCL));
		     } finally {
		          if (db != null) {
		              db.disconnect();
		          }
    ...
    public void checkAttachResult() {
    	int lastMainCLNo = (lastSuccessMainCL > 0)? (lastSuccessMainCL - 1) : 0;
    	int lastSubCLNo = (lastSuccessSubCL > 0)? (lastSuccessSubCL - 1) : 0;
    	...
    	try {
			 ...
			 mainCL.attachCollection(sclFullName, (BSONObject) JSON.parse("{ LowBound: { key: " + (bound * INSERT_NUMS)
                     + " }, " + "UpBound: { key: " + (bound * INSERT_NUMS) + " } }"));
			  
		  } catch (BaseException e) {  
			  Assert.assertEquals(e.getErrorCode(),-235,"the error code is not -235: " + e.getErrorCode());
    	  } 
    ...
    public void checkInsert(){
        try {
        	for(int i = 0; i < lastSuccessMainCL; i++) {
        		DBCollection mainCL = sdb.getCollectionSpace(csName).getCollection(mainCLName + "_" + i);
