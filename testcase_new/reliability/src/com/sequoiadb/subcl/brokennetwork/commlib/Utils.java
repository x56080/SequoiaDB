...
public class Utils {
    ...
    public static void attachAllScl(Sequoiadb db, String mclName) {
        DBCollection mcl = db.getCollectionSpace(SdbTestBase.csName).getCollection(mclName);
        int rangeStart = 0;
        for (int i = 0; i < SCLNUM; i++) {
    ...
    public static void checkAttached(Sequoiadb db, String mclName, int attachedSclCnt) {
        try {
            CollectionSpace cs = db.getCollectionSpace(SdbTestBase.csName);
    ...
    public static void checkDetached(Sequoiadb db, String mclName, int detachedSclCnt) throws ReliabilityException {
        try {
            CollectionSpace cs = db.getCollectionSpace(SdbTestBase.csName);
            // check scl detached
            int randomNum = (new Random()).nextInt(RANGE_WIDTH);
            ...
            for (int i = 0; i < detachedSclCnt; i++) {
                ...
                try {
                    ...
                    throw new ReliabilityException(sclName + " not detached!");
                } catch (BaseException e) {
