...
public class RemoveAndDataBroken2188 extends SdbTestBase{
    ...
    public void setUp() {
        ...
        try {
            ...
            System.out.println("clGroupName: " + clGroupName + " cataPriHost: " + cataPriHost + " brokenNetHost: " + brokenNetHost);
    ...
    public void test() {
        ...
        try {
            FaultMakeTask faultTask = BrokenNetwork.getFaultMakeTask(brokenNetHost, 1, 10);
    ...
    public void createAndAttachCLs() {
    	try {
    		...
    		for(int i = 0; i < SUBCL_NUMS; i++) {
    			...
    			mainCL.attachCollection(sclFullName, (BSONObject) JSON.parse("{ LowBound: { a: " + i * INSERT_NUMS
                        + " }, " + "UpBound: { a: " + ((i + 1) * INSERT_NUMS) + " } }"));
    		}
    ...
    private void insertData() {
        ...
        for (int i = 0; i < SUBCL_NUMS * INSERT_NUMS; i++) {
        	insrtObj = (BSONObject)JSON.parse("{ a: " + i +  ", b: 'testaaaaaaaaaaa' }");
