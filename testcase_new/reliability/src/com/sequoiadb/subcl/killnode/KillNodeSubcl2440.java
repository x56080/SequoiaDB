...
public class KillNodeSubcl2440 extends SdbTestBase {
    ...
    public void test() {
        try {
            ...
            FaultMakeTask faultTask = KillNode.getFaultMakeTask(primaryNode.hostName(),
            		primaryNode.svcName(), 1);
    	...
    	try {
    		...
    		for(int i = 0; i < MAINCL_NUMS; i++) {
			    ...
			    for(int j = 0; j < SUBCL_NUMS; j++) {
				    DBCollection subCL = cs.createCollection(subCLName + "_" + i + "_" + j,
						(BSONObject) JSON
	                            .parse("{ShardingKey:{b:1},ShardingType:'hash'," + "Group:'" + clGroupName + "'}"));				
				    String sclFullName = csName + "."  + subCLName + "_" + i + "_" + j;
			     	mainCL.attachCollection(sclFullName, (BSONObject) JSON.parse("{ LowBound: { a: " + (bound * INSERT_NUMS)
	                    + " }, " + "UpBound: { a: " + ((bound + 1) * INSERT_NUMS) + " } }"));
			     	++bound;
			    }
    ...
    public void insertData() {
    	for (int i = 0; i < MAINCL_NUMS; i++) {
    		...
    		try {
		          ...
		          for(num = i * SUBCL_NUMS * INSERT_NUMS; num < insertNums; num++) {
        ...
        public void exec() throws Exception {
            ...
            for (int i = 0; i < MAINCL_NUMS; i++) {
            	try {
                	...
                	System.out.println(mainCLName + "_" + i + " left record count: " + leftCount);
                } finally {
                    if (db != null) {
                        db.disconnect();
                    }
    ...
    public void checkRemovedResult() {
    	for (int i = 0; i < MAINCL_NUMS; i++) {
            ...
            Assert.assertEquals(leftCount, 0, mainCLName + "_" + i + "  removed data failed");
    	}
    }
