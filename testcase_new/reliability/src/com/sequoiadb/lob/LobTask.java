...
public abstract class LobTask extends OperateTask {
    ...
    public void exec() {
        ...
        try {
            db = getSdb();
            DBCollection cl = db.getCollectionSpace( LobUtil.csName )
                    .getCollection( LobUtil.clName );
            lobOperate( cl );
    ...
    public static LobTask getCreateLobsTask( final int num, final byte[] bytes,
            final List< ObjectId > createLobIds ) {
        return new LobTask() {
            @Override
            void lobOperate( DBCollection cl ) {
                if ( createLobIds == null )
                    throw new IllegalArgumentException(
                            "lobIds can not be null" );
                for ( int i = 0; i < num; i++ ) {
    ...
    public static LobTask getDeleteLobsTask( final List< ObjectId > lobIds,
            final Map< ObjectId, String > deletedIdMap ) {
        return new LobTask() {
            @Override
            void lobOperate( DBCollection cl ) {
