...
public class Lob11437 implements StandTestInterface {
    ...
    /**
     * 1.开启事务 2.cl中执行增删改操作 3.循环写入lob，过程中数据主节点所在主机断网（lob写入的数据组主节点） 4、故障恢复后，检查操作结果
     */
    public void test( FaultMakeTask faultMakeTask )
            throws ReliabilityException {
        setup();

        OperateTask clTask = ClTask.getClTask( 1000 );
        List< ObjectId > createdLobIds = new ArrayList<>();
        LobTask lobTask = LobTask.getCreateLobsTask( 1000, data,
                createdLobIds );
    ...
    private boolean isCountOfClInspect() throws ReliabilityException {
        ...
        long num = MyUtil.getClCountFromNode( csName, clName,
                groupWrapper.getMaster() );
        for ( NodeWrapper node : groupWrapper.getNodes() ) {
            long temp = MyUtil.getClCountFromNode( csName, clName,
                    groupWrapper.getMaster() );
            if ( temp != num )
                return false;
        }
        ...
        num = MyUtil.getClCountFromNode( csName, clName,
                groupWrapper.getMaster() );
        for ( NodeWrapper node : groupWrapper.getNodes() ) {
            long temp = MyUtil.getClCountFromNode( csName, clName,
                    groupWrapper.getMaster() );
    ...
    /**
     * 1.开启事务 2.cl中执行增删改操作 3.循环写入lob，过程中数据备节点所在主机断网（lob写入的数据组备节点） 4、执行事务提交操作
     * 5、故障恢复后，检查操作结果
     *
     * @throws ReliabilityException
     */
