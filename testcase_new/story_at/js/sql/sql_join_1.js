 ...
 * ========== ========== =========================================================
 * 01/04/2023 He Guoming filter of left join
 **************************************************************************************************/

/*********************************************测试用例***********************************************
 * 环境准备：正常集群环境即可
 * 测试场景：
 *    left join中右表的谓词条件不能下压
 * 测试步骤：
 *    1.left join在右表指定谓词条件
 *    2.left join在左表指定谓词条件
 * 期望结果：
 *    1.谓词条件能匹配正确
 **************************************************************************************************/

testConf.clName = COMMCLNAME + "_sql_join_1_a" ;

main( test ) ;

function test( testPara ) {
   var cl2name = COMMCLNAME + "_sql_join_1_b" ;
   ...
   var res = db.exec( "select T1.a, T1.b, T2.c from " + testConf.csName + "." + testConf.clName +
                      " as T1 left outer join " + testConf.csName + "." + cl2name +
   ...
   var expRes = [ { "a": 2, "b": 20, "c": 40 }, { "a": 3, "b": 30, "c": null } ] ;
   commCompareResults( res, expRes ) ;
}
