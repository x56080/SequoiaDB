 ...
 * 环境准备：一个包含三个节点的复制组
 * 测试场景：
 *    node.setAttributes()接口设置Location
 * 测试步骤：
 *    1. 设置全大写字符Location
 *    2. 设置全小写字母Location
 *    3. 设置大小写混合Location
...
function test ()
{
   ...
   data.setAttributes( { Location: location3 } );
   checkNodeLocation( data, location3 );

   data.setAttributes( { Location: location4 } );
   checkNodeLocation( data, location5 );

   data.setAttributes( { Location: location6 } );
   checkNodeLocation( data, location7 );

   data.setAttributes( { Location: location8 } );
