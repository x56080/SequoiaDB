/***************************************************************************************************
 * @Description: $leftBytes功能测试
 * @ATCaseID: leftBytes_at_1
 * @Author: Huang Youquan
 * @TestlinkCase: 无
 * @Change Activity:
 * Date       Who           Description
 * ========== ============= =========================================================
 * 03/08/2023 Huang Youquan    Init
 **************************************************************************************************/

/*********************************************测试用例***********************************************
 * 环境准备：正常集群
 * 测试场景：
 *     $leftBytes功能性测试
 * 测试步骤：
 *    1. 准备字符串类型（单字节编码）记录和非字符串类型记录
 *    2. 测试$leftBytes作为选择符{ $leftBytes : len }语法
 *    3. 测试 $leftBytes作为匹配符{ $leftBytes : len, "$": ...}语法
 *    4. 校验非法参数
 * 期望结果：
 *    第2-3步输出期望子串结果
 *    第4步对外报错-6
 *
 **************************************************************************************************/

testConf.clName = COMMCLNAME + "leftBytes_at_1";

main(test);
function test(testPara) {
  var dbcl = testPara.testCL;
  // 1.prepare data
  var docs = [
    { a: "Sequoiadb" },
    { a: "ab" },
    { a: ["Sequoiadb", 111] },
    { a: [111, true] },
    { a: 111 },
    { a: 2022.0211 },
    { a: true },
    { a: { $date: "2022-02-11T15:59:59.999Z" } },
    { a: { $timestamp: "2022-02-11T15:59:59.999Z" } },
  ];
  dbcl.insert(docs);

  //$leftBytes as selector
  // 2.{ $leftBytes : value }
  var actResult1 = dbcl.find({}, { a: { $leftBytes: 3 } });
  var expResult1 = [
    { a: "Seq" },
    { a: "ab" },
    { a: ["Seq", null] },
    { a: [null, null] },
    { a: null },
    { a: null },
    { a: null },
    { a: null },
    { a: null },
  ];
  commCompareResults(actResult1, expResult1);

  var actResult2 = dbcl.find({}, { a: { $leftBytes: -3 } });
  var expResult2 = [
    { a: "" },
    { a: "" },
    { a: ["", null] },
    { a: [null, null] },
    { a: null },
    { a: null },
    { a: null },
    { a: null },
    { a: null },
  ];
  commCompareResults(actResult2, expResult2);

  var actResult3 = dbcl.find({}, { a: { $leftBytes: 0 } });
  var expResult3 = [
    { a: "" },
    { a: "" },
    { a: ["", null] },
    { a: [null, null] },
    { a: null },
    { a: null },
    { a: null },
    { a: null },
    { a: null },
  ];
  commCompareResults(actResult3, expResult3);

  // $leftBytes as Matcher
  // 3.{ $leftBytes : value, "$": ...}
  var actResult4 = dbcl.find({ a: { $leftBytes: 2, $et: "Se" } });
  var expResult4 = [{ a: "Sequoiadb" }, { a: ["Sequoiadb", 111] }];
  commCompareResults(actResult4, expResult4);

  // 4.check arguments
  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({}, { a: { $leftBytes: true } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({}, { a: { $leftBytes: null } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({}, { a: { $leftBytes: [1, 2] } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({}, { a: { $leftBytes: [1, null] } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({}, { a: { $leftBytes: [null, true] } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({}, { a: { $leftBytes: [1, 2, 3] } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({ a: { $leftBytes: [null, true], et: "Se" } }).toArray();
  });

  assert.tryThrow(SDB_INVALIDARG, function () {
    dbcl.find({ a: { $leftBytes: [1, 2], et: "Se" } }).toArray();
  });
}
