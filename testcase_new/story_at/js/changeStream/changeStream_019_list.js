...
testConf.clName = COMMCLNAME + "_change_stream_019" ;
...
function test( testPara )
{
   ...
   // list
   var listCur = db.list( SDB_LIST_STREAMS, { "Options.Collections": testConf.csName + "." + testConf.clName } ) ;
   assert.equal( listCur.size(), 1, "expected list number is different" ) ;
   listCur.close() ;

   // list from SQL
   var listCur = db.exec( 'select * from $LIST_STREAMS AS A where A.Options.Collections = "' + testConf.csName + "." + testConf.clName + '"' ) ;
   assert.equal( listCur.size(), 1, "expected list number is different" ) ;
   listCur.close() ;
