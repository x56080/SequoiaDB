/***************************************************************************************************
 * @Description: $substrBytes作为匹配符使用
 * @ATCaseID: substrBytes_at_3
...
testConf.clName = COMMCLNAME + "substrBytes_at_3";
...
function test(testPara) {
  ...
  // 2.{ $substrBytes : value, "$": ...}
  actResult = dbcl.find({ a: { $substrBytes: 2, $et: "Se" } });
  expResult = [{ a: "Sequoiadb" }, { a: ["Sequoiadb", 111] }];
  commCompareResults(actResult, expResult);

  // 3.{ $substrBytes : [pos , len ], "$": ...}
  var actResult = dbcl.find({ a: { $substrBytes: [0, 2], $et: "Se" } });
