...
from s3tests.functional.test_s3 import FakeWriteFile, FakeFile
from s3tests_boto3.functional.utils import assert_raises
from s3tests_boto3.functional.utils import generate_random
from s3tests_boto3.functional.utils import _get_status_and_error_code
from s3tests_boto3.functional.utils import _get_status
...
class Test_S3(unittest.TestCase):
    ...
    def _bucket_is_empty(self,bucket):
        is_empty = True
    ...
    def _get_prefixes(self,response):
        """
    ...
    def test_bucket_listv2_delimiter_prefix_ends_with_delimiter(self):
        ...
        self.validate_bucket_listv2(bucket_name, 'asdf/', '/', None, 1000, False, ['asdf/'], [], last=True)
    ...
    def test_bucket_list_delimiter_prefix_underscore(self):
        ...
        marker = self.validate_bucket_list(bucket_name, prefix, delim, '', 2, True, ['_obj1_'], ['_under1/'], '_under1/')
        ...
        marker = self.validate_bucket_list(bucket_name, prefix, delim, '', 2, False, ['_under1/bar'], ['_under1/baz/'], None)
    ...
    def test_bucket_listv2_delimiter_prefix_underscore(self):
        ...
        continuation_token = self.validate_bucket_listv2(bucket_name, prefix, delim, None, 2, True, ['_obj1_'], ['_under1/'])
        continuation_token = self.validate_bucket_listv2(bucket_name, prefix, delim, continuation_token, 2, False, [],
                                                    ['_under2/'], last=True)
    ...
    def test_bucket_listv2_prefix_unreadable(self):
        key_names = ['foo/bar', 'foo/baz', 'quux']
        bucket_name = self.self._create_objects(keys=key_names)
        client = get_client()
    ...
    def test_bucket_listv2_continuationtoken_empty(self):
        ...
        self.assertEqual(response['ContinuationToken'], '')
        self.assertEqual(response['IsTruncated'], False)
        keys = self._get_keys(response)
        self.assertEqual(keys, key_names)
    ...
    def _do_set_bucket_canned_acl_concurrent(self,client, bucket_name, canned_acl, num, results):
        t = []
        for i in range(num):
            thr = threading.Thread(target=self._do_set_bucket_canned_acl, args=(client, bucket_name, canned_acl, i, results))
            thr.start()
            t.append(thr)
        return t
    ...
    def test_bucket_create_naming_bad_short_empty(self):
        ...
        self.assertEqual(error_code, 'MethodNotAllowed')
    ...
    def test_bucket_create_naming_bad_ip(self):
        self.check_bad_bucket_name('192.168.5.123')
    ...
    def test_bucket_create_exists(self):
        ...
        try:
            ...
            self.assertEqual(e.error_code, 'BucketAlreadyOwnedByYou')
    ...
    def test_bucket_create_exists_nonowner(self):
        ...
        status, error_code = _get_status_and_error_code(e.response)
        self.assertEqual(status, 409)
        self.assertEqual(error_code, 'BucketAlreadyExists')
    ...
    def test_access_bucket_publicreadwrite_object_publicreadwrite(self):
        bucket_name, key1, key2, newkey = self._setup_access(bucket_acl='public-read-write', object_acl='public-read-write')
        alt_client = get_alt_client()
        response = alt_client.get_object(Bucket=bucket_name, Key=key1)
        body = self._get_body(response)
    ...
    def test_list_buckets_anonymous(self):
        ...
        self.assertEqual(len(response['Buckets']), 0)
    ...
    def test_list_buckets_bad_auth(self):
        ...
        status, error_code = _get_status_and_error_code(e.response)
        self.assertEqual(status, 403)
        self.assertEqual(error_code, 'SignatureDoesNotMatch')
    ...
    def test_multipart_upload_size_too_small(self):
        ...
        size = 100 * 1024
        (upload_id, data, parts) = self._multipart_upload(bucket_name=bucket_name, key=key, size=size, part_size=10 * 1024)
    ...
    def test_abort_multipart_upload(self):
        ...
        response = client.head_bucket(Bucket=bucket_name)
        rgw_bytes_used = int(response['ResponseMetadata']['HTTPHeaders']['x-rgw-bytes-used'])
        self.assertEqual(rgw_bytes_used, 0)

        rgw_object_count = int(response['ResponseMetadata']['HTTPHeaders']['x-rgw-object-count'])
        self.assertEqual(rgw_object_count, 0)
    ...
    def test_cors_origin_response(self):
        ...
        self._cors_request_and_check(requests.put, obj_url, {'Origin': 'foo.suffix', 'Access-Control-Request-Method': 'GET',
                                                        'content-length': '0'}, 403, 'foo.suffix', 'GET')
        self._cors_request_and_check(requests.put, obj_url, {'Origin': 'foo.suffix', 'Access-Control-Request-Method': 'PUT',
        ...
        self._cors_request_and_check(requests.put, obj_url,
                                {'Origin': 'foo.suffix', 'Access-Control-Request-Method': 'DELETE',
                                 'content-length': '0'}, 403, None, None)
        self._cors_request_and_check(requests.put, obj_url, {'Origin': 'foo.suffix', 'content-length': '0'}, 403, None, None)

        self._cors_request_and_check(requests.put, obj_url, {'Origin': 'foo.put', 'content-length': '0'}, 403, 'foo.put',
                                'PUT')
        ...
        self._cors_request_and_check(requests.options, url, {'Origin': 'foo.bar', 'Access-Control-Request-Method': 'GET'},
                                403,
                                None, None)
        self._cors_request_and_check(requests.options, url,
                                {'Origin': 'foo.suffix.get', 'Access-Control-Request-Method': 'GET'},
                                403, None, None)
        self._cors_request_and_check(requests.options, url, {'Origin': 'startend', 'Access-Control-Request-Method': 'GET'},
                                200,
                                'startend', 'GET')
        self._cors_request_and_check(requests.options, url, {'Origin': 'start1end', 'Access-Control-Request-Method': 'GET'},
                                200,
                                'start1end', 'GET')
        self._cors_request_and_check(requests.options, url, {'Origin': 'start12end', 'Access-Control-Request-Method': 'GET'},
                                200, 'start12end', 'GET')
        self._cors_request_and_check(requests.options, url,
                                {'Origin': '0start12end', 'Access-Control-Request-Method': 'GET'},
                                403, None, None)
        self._cors_request_and_check(requests.options, url, {'Origin': 'prefix', 'Access-Control-Request-Method': 'GET'},
                                200,
                                'prefix', 'GET')
        self._cors_request_and_check(requests.options, url,
                                {'Origin': 'prefix.suffix', 'Access-Control-Request-Method': 'GET'},
                                200, 'prefix.suffix', 'GET')
        self._cors_request_and_check(requests.options, url, {'Origin': 'bla.prefix', 'Access-Control-Request-Method': 'GET'},
                                403, None, None)
        self._cors_request_and_check(requests.options, url, {'Origin': 'foo.put', 'Access-Control-Request-Method': 'GET'},
                                403,
                                None, None)
        self._cors_request_and_check(requests.options, url, {'Origin': 'foo.put', 'Access-Control-Request-Method': 'PUT'},
                                200,
                                'foo.put', 'PUT')
    ...
    class FakeReadFile(FakeFile):
        ...
        def write(self, chars):
            self.assertEqual(chars, self.char * len(chars))
        ...
        def close(self):
            self.assertEqual(self.size, self.expected_size)
    ...
    class FakeFileVerifier(object):
        ...
        def write(self, data):
            ...
            if self.char == None:
                self.char = data[0]
            self.size += size
            self.assertEqual(data, self.char * size)
    ...
    def _verify_atomic_key_data(self,bucket_name, key, size=-1, char=None):
        ...
        """
        fp_verify = self.FakeFileVerifier(char)
        client = get_client()
    ...
    def test_ranged_request_response_code(self):
        ...
        self.assertEqual(fetched_content, content[4:8])
    ...
    def test_ranged_request_skip_leading_bytes_response_code(self):
        ...
        fetched_content = self._get_body(response)
        self.assertEqual(fetched_content, content[4:])
    ...
    def test_ranged_request_return_trailing_bytes_response_code(self):
        ...
        fetched_content = self._get_body(response)
        self.assertEqual(fetched_content, content[-7:])
        self.assertEqual(response['ResponseMetadata']['HTTPHeaders']['content-range'], 'bytes 4-10/11')
        self.assertEqual(response['ResponseMetadata']['HTTPStatusCode'], 206)
    ...
    def clean_up_bucket(self,client, bucket_name, key, version_ids):
    ...
    def test_versioning_obj_plain_null_version_overwrite(self):
        ...
        response = client.get_object(Bucket=bucket_name, Key=key)
        body = self._get_body(response)
        self.assertEqual(body, content2)
    ...
    def test_versioning_copy_obj_version(self):
        ...
        for i in range(num_versions):
            ...
            body = self._get_body(response)
            self.assertEqual(body, contents[i])
    ...
    def test_sse_kms_method_head(self):
        ...
        self.assertEqual(response['ResponseMetadata']['HTTPHeaders']['x-amz-server-side-encryption-aws-kms-key-id'], kms_keyid)
    ...
    def test_copy_object_ifmatch_good(self):
        ...
        self.assertEqual(resp['Body'].read(), 'bar')
