...
/**
 * @Description: seqDB-17299 ::创建区域配置DataCSShardingType策略小于CLShardingType策略 此用例时间跳变会影响到db日志，所以不上ci
 * @author fanyu
 * @Date:2019年01月22日
 * @version:1.0
 */
public class CreateRegion17299 extends S3TestBase {
    ...
    private void test() throws Exception {
        ...
        request.withDataCSShardingType(DataShardingType.MONTH).withDataCLShardingType(DataShardingType.YEAR);
        ...
        for (int i = 0; i < dateList.size(); i++) {
            String csName = RegionUtils.getDataCSName(regionName, DataShardingType.MONTH, dateList.get(i));
            String clName = RegionUtils.getDataCLName(DataShardingType.YEAR, dateList.get(i));
