...
public class RemoveRegionAndCreateBucket17339 extends S3TestBase {
    ...
    public void testRegion() throws Exception {
        ...
        if (removeRegion.isSuccess() && !createBuckets.isSuccess()) {
            AmazonS3Exception e = (AmazonS3Exception) (createBuckets.getExceptions().get(0));
            // 404, "NoSuchRegion"
            if (e.getStatusCode() != 404) {
                Assert.fail("createBucket fail:" + e.getErrorMessage() + "/n e:" + e.getStatusCode());
            }
            checkRemoveRegionResult(false);
        } else if (createBuckets.isSuccess() && !removeRegion.isSuccess()) {
            AmazonS3Exception e = (AmazonS3Exception) (removeRegion.getExceptions().get(0));
            // -312, "RegionNotEmpty"
            if (e.getStatusCode() != 409) {
                Assert.fail("remove Region fail:" + e.getErrorMessage() + "/n e:" + e.getStatusCode());
            }
    ...
    private class CreateBuckets extends S3ThreadBase {
        ...
        public void exec() throws Exception {
            ...
            AmazonS3 s3Client = CommLib.buildS3Client(accessKeys[0], accessKeys[1]);
            try {
                for (int i = 0; i < 100; i++) {
