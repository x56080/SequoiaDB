...
public class DataSourceTestBase extends SdbTestBase {
	...
	protected boolean init(){
		...
		if (this.coordAddr == null){
			...
			/*String[]  tmpArr = splitStr(coordUrl, "@");
			if (tmpArr.length == 2){
				this.coordAddr = tmpArr[1];
				userName = tmpArr[0];
				tmpArr = splitStr(userName, ":");
				if (tmpArr.length == 2){
					userName = tmpArr[0];
					password = tmpArr[1];
				}else return false;
			}else{
				this.coordAddr = tmpArr[0];
			}
			
			tmpArr = splitStr(coordUrl, "/");
			this.coordAddr = tmpArr[0];
			if (tmpArr.length == 2){
				csName = tmpArr[1];
				tmpArr = splitStr(csName, "\\.");
				csName = tmpArr[0];
				if (tmpArr.length == 2){
					clName = tmpArr[1];
				}else{
	...
	protected void getAddrList(){
		try{
			...
			Sequoiadb sdb = new Sequoiadb(this.coordAddr, this.userName, this.password);
			...
			for (int i = 0; i < subObjs.size(); ++i){
				...
				BasicBSONList serviceDocs = (BasicBSONList)doc.get("Service");
				BasicBSONObject serviceDoc = (BasicBSONObject)serviceDocs.get(0);
				String serviceName = serviceDoc.getString("Name");
				
				taddrList.add(new InetSocketAddress(hostName, Integer.parseInt(serviceName)));
			}
			addrList = taddrList;
