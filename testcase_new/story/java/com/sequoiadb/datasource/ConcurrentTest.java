...
public class ConcurrentTest extends DataSourceTestBase {
	...
	public void createDatasource(){
		try{
			super.init();
			ds1 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds2 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds3 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds4 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds5 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds6 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds7 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds8 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
			ds9 = new SequoiadbDatasource(this.coordAddr, userName, password, null);
	...
	void addOrRemoveCoordAddr(){
		try{
			...
			for (int i = 0; i < this.addrList.size(); ++i){
				ds2.addCoord(this.addrList.get(i).getHostName() + ":" + this.addrList.get(i).getPort());
			}
			InetSocketAddress inCoordAddr = getInCoordAddr();
			for (int i = 0; i< this.addrList.size();++i){
				InetSocketAddress curAddr = this.addrList.get(i); 
				if (inCoordAddr.equals(curAddr)) continue;
				ds2.removeCoord(curAddr.getHostName() + ":" + curAddr.getPort());
			}
	...
	void getConnectionOfConcurrent(){
		try{
			synchronized(this){
				if (datasource_concurrent == null){
					super.init();
					SequoiadbDatasource ds = new SequoiadbDatasource(this.coordAddr, this.userName, this.password, null);
	...
	public void addCoordAddrAfterClose(){
		try {
			...
			for (int i = 0; i < addrList.size(); ++i){
				ds7.addCoord(addrList.get(i).getHostName() + ":" + addrList.get(i).getPort());
			}
		} catch (BaseException e) {
	...
	public void delCoordAddrAfterClose(){
		...
		try {
			for (int i = 0; i < addrList.size(); ++i){
				ds8.addCoord(addrList.get(i).getHostName() + ":" + addrList.get(i).getPort());
			}
		} catch (BaseException e) {
		...
		try{
			...
			ds8.close();
			for (int i = 0; i < addrList.size(); ++i){
				ds8.removeCoord(addrList.get(i).getHostName() + ":" + addrList.get(i).getPort());
			}
