...
public class MetaDataUtils extends SdbTestBase {
	 ...
	 * @return true/false, true is success, false is failed
	 */
	public static void checkCLOfCatalog(Sequoiadb sdb, String csName, String clName) {
		try {
			BSONObject matcher = new BasicBSONObject();
			BSONObject subObj = new BasicBSONObject();
	 ...
	 * @param clName
	 */
	public static void checkCLOfDataRG(Sequoiadb sdb, String csName, String clName) {
		try {
			BSONObject matcher = new BasicBSONObject();
			BSONObject subObj = new BasicBSONObject();
			subObj.put("$regex", "^" + csName + "." + clName);
			...
			for (int i = 0; i < dataGroupNames.size(); i++) {
				...
				if (nodeAddrs.size() < 2) { // other testCase create empty group
					...
					do {
						...
						for (int j = 0; j < nodeAddrs.size(); j++) {
							...
							while (cur.hasNext()) {
								...
								BSONObject IndexDef = (BSONObject) idxList.get("IndexDef");
								String idxName = (String) IndexDef.get("name");
							...
							if (j == (nodeAddrs.size() - 1)) {
								for (int k = 0; k < j; k++) {
//									System.out.println("k=" + k +     ", oneNodeData = " + allNodeData.get(k));
//									System.out.println("k=" + (k+1) + ", oneNodeData = " + allNodeData.get(k+1));
									if (allNodeData.get(k).equals(allNodeData.get(k + 1))) {
										if (k == (j - 1)) {
											checkSucc = true;
											break;
										}
										...
										Assert.fail("clName = " + name + ", allNodeData = " + allNodeData.toString()
	 ...
	 * @param clName
	 */
	public static boolean compareDataAndCata(Sequoiadb sdb, String csName, String clName) {
		try {
			// get all dataGroupNames
			ArrayList<String> dataGroupNames = getDataGroupNames(sdb);
			for (int i = 0; i < dataGroupNames.size(); i++) {
	 ...
	 */
	public static void compareNodeData(Sequoiadb sdb, String rgName, String csName, String clName, BSONObject matcher) {
	 ...
	 * @param sdb
	 */
	public static void createNode(Sequoiadb sdb, String rgName, int portStart, int portStop, String path) {
		try {
			// get hostname
			// String hostName =
			...
			do {
				...
				try {
					...
					int eCode = e.getErrorCode();
					if (eCode == -157 // -157:Invalid node configuration(Port is
										// occupied)
