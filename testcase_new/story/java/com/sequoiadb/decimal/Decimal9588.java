...
public class Decimal9588 extends SdbTestBase{
	private Sequoiadb sdb ;
	private SimpleDateFormat df = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss.SSS");
	
	private CollectionSpace cs = null;
	private String clName = "cl9588";
	...
	public void test(){
		//insert int
		String[] valueArr = {"-2147483648","2147483647",
				"-12.1122334455667","1122334455667.12",
		...
		try{
			...
			Assert.fail("generate decimal data:" + obj + " failed, errMsg:" + e.getMessage());
		}catch(BaseException e){
			Assert.fail("insert common data:" + obj + " failed, errMsg:" + e.getMessage());
		}
		
		//check
		for(i = 0; i < valueArr.length; i++){
			obj = new BasicBSONObject();
			BSONDecimal matcher = new BSONDecimal(valueArr[i],1000,50);
			obj.put("a", matcher);
