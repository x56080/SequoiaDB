...
public class Decimal9593 extends SdbTestBase{
	...
	public void test(String value, String expectValue, int precision, int scale){
		...
		try{
			...
			BSONDecimal decimalWithPrecision = new BSONDecimal(value , precision , scale);
			decimalBean.setDecimalWithPrecision(decimalWithPrecision);
			
			cl.save(decimalBean);
		}catch(BaseException e){
			Assert.fail("insert data:" + decimalBean + " failed, errMsg:" + e.getMessage());
		}
		...
		try {
			...
			Assert.fail("BSONObject as DecimalBean failed, errMsg:" + e.getMessage() );
		}
		...
		try{
			...
			BSONDecimal d3 = (BSONDecimal)retObj.get("decimalWithPrecision");
