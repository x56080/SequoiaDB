...
public class TestTruncate171 extends SdbTestBase {
    ...
    public void test(){
        ...
        if(!(truncateThread.isSuccess() && splitThread.isSuccess())){
            Assert.fail(truncateThread.getErrorMsg() + splitThread.getErrorMsg());
        }
    }
    ...
    private void checkSplit(Sequoiadb db, String srcGroupName, String dstGroupName){
        try{
            // get CataInfo
            BSONObject option = new BasicBSONObject();
            option.put("Name", csName + '.' + clName);
            DBCursor snapshot = db.getSnapshot(8, option, null, null);
            BasicBSONList cataInfo = (BasicBSONList) snapshot.getNext().get("CataInfo");
            snapshot.close();
            ...
            if(!(((String)srcInfo.get("GroupName")).equals(srcGroupName)
                    && ((BasicBSONObject)srcInfo.get("LowBound")).getInt("") == expSrcLowBound
                    && ((BasicBSONObject)srcInfo.get("UpBound")).getInt("") == expSrcUpBound)){
            ...
            if(!(((String)dstInfo.get("GroupName")).equals(dstGroupName)
                    && ((BasicBSONObject)dstInfo.get("LowBound")).getInt("") == expDstLowBound
                    && ((BasicBSONObject)dstInfo.get("UpBound")).getInt("") == expDstUpBound)){
