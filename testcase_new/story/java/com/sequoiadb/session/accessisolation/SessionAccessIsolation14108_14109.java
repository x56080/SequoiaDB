...
/**
* FileName: SessionAccessIsolation14108.java
* test content:seqDB-14108:multiple session connections on different coord,seting same session properties 
...
public class SessionAccessIsolation14108_14109 extends SdbTestBase {
	...
	private ConcurrentHashMap<Object,String> instanceidTosvcName
	...
	public void testSession14108(){		
		...
		for ( int i = 0; i < coordUrls.size(); i++ ){			
			accessSessions.add(new AccessSession( nodeName, coordUrls.get(i)))  ;
		}
	...
	public void testSession14109(){		
		...
		for ( int i = 0; i < coordUrls.size(); i++ ){	
			...
			accessDiffSessions.add(new AccessDiffSession( instanceid[i], nodeName,coordUrls.get(i)))  ;
		}		
		...
		for( AccessDiffSession accessDiffSession: accessDiffSessions ){			
			Assert.assertTrue(accessDiffSession.isSuccess(), accessDiffSession.getErrorMsg());
		}		
	}	
	...
	private class AccessDiffSession extends SdbThreadBase{
		...
		private AccessDiffSession(Object instanceid, String expQueryNodeName,String coordUrl){
