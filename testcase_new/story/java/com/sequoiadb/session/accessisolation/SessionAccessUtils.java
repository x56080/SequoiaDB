...
public class SessionAccessUtils extends SdbTestBase {
	...
	public static void insertData(DBCollection cl) {
		...
		for ( int i = 0; i < 1000; i++){				
			BSONObject obj = (BSONObject) JSON.parse("{a:" + i +", test:"+"'testasetatatatatat'" + "}");				
			list.add(obj);	
		}
		cl.bulkInsert(list, 0);	
	}	
	
	public static void createRGAndNode(Sequoiadb sdb, String groupName,Object[] instanceidarr,
			     int nodeNum, ConcurrentHashMap<Object,String> instanceidTosvcName){
	...
	public static void checkMasterExist( Sequoiadb sdb, String groupName){
		...
		String queryMasterNode = "select IsPrimary,NodeName from $SNAPSHOT_SYSTEM where GroupName='" + groupName + "' and IsPrimary=true ";
            ...
            if ( groupName.equals("SYSCoord")){            	
            	BasicBSONList group=(BasicBSONList)obj.get("Group");
           	 	for(Object temp:group){
           	 		...
           	 		BasicBSONList service = (BasicBSONList) nodeifg.get("Service");
