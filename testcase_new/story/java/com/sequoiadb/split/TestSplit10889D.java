...
/**
 *   1、向主表中插入数据，其中分区键字段分别覆盖多种数据类型，参考如下组合： 
 *       a、long、timestamp 
 *       b、date、float、int 
 *       c、_id+decimal
 *   2、执行数据切分（根据1中组合覆盖同步切分和异步切分）
 *   3、查看数据切分结果（分别连接coord、源组data、目标组data查询 )
 * @author chensiqin
 * @Date 2016-12-21
 *
 */
public class TestSplit10889D extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection mainCL;
    private DBCollection subCL;
    private String mainCLName = "mainCL10889D_0";
    private String subCLName = "subCL10889D_0";
    ...
    public void test() {
        ...
        //创建主表
        BSONObject option = 
                (BSONObject) JSON.parse("{IsMainCL:true,ShardingKey:{height:-1,timestamp:-1},ShardingType:\"range\"}");
        this.mainCL = SplitUtils2.createCL(this.cs, this.mainCLName, option);
        
        //a、long、timestamp
        option = 
              (BSONObject) JSON.parse("{ShardingKey:{height:-1,timestamp:-1}," +
                    "ShardingType:\"hash\",Partition:1024,Group:\"" + rgNames.get(0) + "\"}");
        this.subCL = SplitUtils2.createCL(this.cs, this.subCLName, option);
        //挂载子表
        option = (BSONObject) JSON.parse("{LowBound:{height:111," +
                "timestamp:{\"$timestamp\":\"2016-01-30-00.00.00.000000\"}}," +
                "UpBound:{height:-5," +
                "timestamp:{\"$timestamp\":\"2016-12-12-23.59.59.999999\"}}}");
        this.mainCL.attachCollection(SdbTestBase.csName+"."+this.subCLName, option);
        this.insertRecords = (ArrayList<BSONObject>) SplitUtils2.insertData(this.mainCL, 100);
        ...
        //执行切分      
        long splitAsync = this.subCL.splitAsync(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
        long[] taskIDs = {splitAsync};
    ...
    public void testCoordSplitResult(List<String> rgNames) {
        try {
            ...
            DBCursor cursor = this.mainCL.query(null,null,"{\"_id\":1}",null);
