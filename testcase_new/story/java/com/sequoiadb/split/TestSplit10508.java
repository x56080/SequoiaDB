...
public class TestSplit10508 extends SdbTestBase{
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{str:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl = SplitUtils2.createCL(this.cs, this.clName1, option); 
            this.insertRecods = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl, 10);
            BSONObject startCondition = (BSONObject) JSON.parse("{str:\"3abc\"}");
    ...
    public void testReverse() {
        try {
            ...
            this.insertRecods = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl, 10);
            BSONObject startCondition = (BSONObject) JSON.parse("{str:\"6abc\"}");
            BSONObject endCondition = new BasicBSONObject();
            this.cl.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            //连接coord节点验证数据是否正确
            testCoordSplitResult(rgNames);
