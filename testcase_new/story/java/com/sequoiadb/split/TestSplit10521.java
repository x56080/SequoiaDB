...
/**
 * range分区表指定多个分区键执行切分，其中分区键字段排序为正序 
 * @author chensiqin
 * @Date 2016-12-21
 *
 */
public class TestSplit10521 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl;
    private String clName1 = "cl10521_0";
    private String clName2 = "cl10521_1";
    private String clName3 = "cl10521_2";
    private ArrayList<BSONObject> insertRecods;
    /*
     * 组合类型:
     *   1.数组、int、timestamp
     *   2.long、float、date、decimal 
     *   3._id+long 
     */
    ...
    public void testCombination1() {
        try {
            ...
            BSONObject option = 
                    (BSONObject) JSON.parse("{ShardingKey:{arrtype:1,age:1,timestamp:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl = SplitUtils2.createCL(this.cs, this.clName1, option); 
            insertData(10);

            BSONObject startCondition = 
                    (BSONObject) JSON.parse("{arrtype:2,age:-4,timestamp:{\"$timestamp\":\"2016-11-30-00.00.00.000000\"}}"); 
            BSONObject endCondition = 
                    (BSONObject) JSON.parse("{arrtype:2,age:4,timestamp:{\"$timestamp\":\"2016-11-30-23.59.59.999999\"}}");
            this.cl.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            //连接coord节点验证数据是否正确
    ...
    public void testCombination2() {
        try {
            ...
            List<String> rgNames = SplitUtils2.getDataRgNames(this.sdb); 
            BSONObject option = 
                    (BSONObject) JSON.parse("{ShardingKey:{height:1,num:1,date:1,bdecimal:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl = SplitUtils2.createCL(this.cs, this.clName2, option); 
            insertData(10);
            
            BSONObject startCondition = new BasicBSONObject();
                    //(BSONObject) JSON.parse("{height:1,num:0.01,date:{\"$date\":\"2016-12-01\"},bdecimal:{\"$decimal\":\"12345.06789123456789012345000\"}}"); 
            BSONObject endCondition = new BasicBSONObject();
                    //(BSONObject) JSON.parse("{height:1,num:0.01,date:{\"$date\":\"2016-12-01\"},bdecimal:{\"$decimal\":\"12345.06789123456789012345255\"}}");
    ...
    public void testCombination3() {
        try {
            ...
            List<String> rgNames = SplitUtils2.getDataRgNames(this.sdb); 
            BSONObject option = 
                    (BSONObject) JSON.parse("{ShardingKey:{_id:-1,height:-1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl = SplitUtils2.createCL(this.cs, this.clName3, option); 
            insertData(10);

            BSONObject startCondition = 
                    (BSONObject) JSON.parse("{_id:9,height:1}"); 
            BSONObject endCondition = 
                    (BSONObject) JSON.parse("{_id:3,height:1}"); 
            this.cl.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            //连接coord节点验证数据是否正确
            testCoordSplitResult(rgNames);
            //连接源组data验证数据
    ...
    public void testSrcDataSplitResult(List<String> rgNames) {
        ...
        try {
            ...
            if ( this.combinationType == 1 ) {
                //切分开始条件：{arrtype:2,age:-4,timestamp:{\"$timestamp\":\"2016-11-30-00.00.00.000000\"}}; 
                //切分结束条件：{arrtype:2,age:4,timestamp:{\"$timestamp\":\"2016-11-30-23.59.59.999999\"}};
                //期望结果是第1,3,4,6,7,9条记录
                ...
                //切分开始条件：{height:1,num:0.01,date:{\"$date\":\"2016-12-01\"},bdecimal:{\"$decimal\":\"12345.06789123456789012345000\"}}
                //切分结束条件：{height:1,num:0.01,date:{\"$date\":\"2016-12-01\"},bdecimal:{\"$decimal\":\"12345.06789123456789012345255\"}}
                for( int i = 2; i < 10; i++ ) {
    ...
    public void insertData(int len){
        ...
        try{
            ...
            for (int i = 1; i < len; i++) {
                ...
                str = "2016-12-0" + i%2; 
                str = str + " 12:30:0" + i%10 + "." + (100000 + (123456+i)%100000); 
                String mydate = str;
