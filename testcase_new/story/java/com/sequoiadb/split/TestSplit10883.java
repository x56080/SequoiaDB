...
/**
 *  hash分区表指定分区键值为binary类型，执行切分 
 * @author chensiqin
 * @Date 2017-01-03
 */
public class TestSplit10883 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl1;
    private DBCollection cl2;
    private String clName1 = "cl10883_0";
    private String clName2 = "cl10883_1";
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{bindata:1},ShardingType:\"hash\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl1 = SplitUtils2.createCL(this.cs, this.clName1, option); 
            //插入数据
            insertData(this.cl1);
            this.cl1.split(rgNames.get(0), rgNames.get(1), 45);
    ...
    public void testCoordSplitResult(List<String> rgNames,boolean isReverse) {
        try {
            ...
            for (int i = 1; i < 10000; i++) {
                BSONObject o = this.insertRecords.get(i-1);
    ...
    public void testSrcDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            if ( isReverse ) {
                ...
                //百分比45%
                if ( Math.abs((5500 - actual))/5500.0 > 0.3) {
                    Assert.fail("hash 百分比切分不够准确！");
                }
            }
        } catch (BaseException e) {
    ...
    public void testDestDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            if ( isReverse ) {
                ...
                //百分比45%
                if ( Math.abs((4500 - actual))/4500.0 > 0.3) {
                    Assert.fail("hash 百分比切分不够准确！");
                }
            }
        } catch (BaseException e) {
    ...
    public void insertData(DBCollection cl){
        insertRecords = new ArrayList<BSONObject>();
        try{
