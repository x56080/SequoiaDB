...
/**
 *      1、执行splitAsync，设置范围切分条件，如范围区间a为【20,50】 
 *      2、向cl中插入数据（源和目标组分区范围内数据） 
 *      3、执行split，设置范围切分条件b，其中a为b的子集，如b范围区间为【10,60】 
 *      4、查看数据切分结果 
 * @author chensiqin
 * @Date 2016-12-21
 *
 */
public class TestSplit10526 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl;
    private String clName = "cl10526";
    ...
    public void test() {
        try {
            ...
            BSONObject option = 
                    (BSONObject) JSON.parse("{ShardingKey:{age:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            ...
            long id1 = this.cl.splitAsync(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            long[] taskIDs = { id1 };
            ...
            this.cl.split(rgNames.get(0), rgNames.get(1), 
                    (BSONObject) JSON.parse("{age:10}"), (BSONObject) JSON.parse("{age:60}"));
    ...
    public void testSrcDataSplitResult(List<String> rgNames, int splitNum) {
        ...
        try {
            ...
            if ( splitNum == 1 ) { //第一次切分后校验结果
                //切分范围[20,50),对应的期望结果是[1,20),[50,100)
                for ( int i = 1; i < 20; i++ ) {
                ...
                //切分范围[10,60),对应的期望结果是[1,10),[60,100)
                for ( int i = 1; i < 10; i++ ) {
                    expected.add(this.insertRecods.get(i-1));
                }
     ...
     * @param splitNum 第几次切分
     */
    public void testDestDataSplitResult(List<String> rgNames, int splitNum) {
        Sequoiadb dataDb = null;
        try {
            //连接目标组data查询
            ...
            List<BSONObject> expected = new ArrayList<BSONObject>();
            if ( splitNum == 1 ) { //第一次切分后校验结果
                //切分范围[20,50),对应的期望结果是[20,50)
                for ( int i = 20; i < 50; i++ ) {
                    expected.add(this.insertRecods.get(i-1));
                }
            } else if ( splitNum == 2 ) { //第二次切分后校验结果
                //切分范围[10,60),对应的期望结果是[1,60)
                for ( int i = 10; i < 60; i++ ) {
                    expected.add(this.insertRecods.get(i-1));
                }
            }
