...
public class Split10500 extends SdbTestBase {
	...
	public void split() {
		try {
			...
			taskIdArr[0] = commCL.splitAsync(srcGroupName, destGroupName, (BSONObject) JSON.parse("{sk:30}"), // 切分
	...
	public boolean isSplitComplte(long taskId) {
		...
		try {
			cursor = commSdb.listTasks((BSONObject) JSON.parse("{TaskId:" + taskId + "}"), null, null, null);
			if (cursor.hasNext()) {
				return false;
			} else {
				return true;
