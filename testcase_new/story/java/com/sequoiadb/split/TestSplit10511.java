 ...
 *
 */
public class TestSplit10511 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl;
    private String clName1 = "cl10511_0";
    private String clName2 = "cl10511_1";
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{age:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            ...
            BSONObject startCondition = (BSONObject) JSON.parse("{age:61}");
            BSONObject endCondition = (BSONObject) JSON.parse("{age:89}");
    ...
    public void testReverse() {
        try {
            ...
            this.insertRecods = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl, 100);
            BSONObject startCondition = (BSONObject) JSON.parse("{age:79}");
            BSONObject endCondition = (BSONObject) JSON.parse("{age:29}");
            this.cl.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            //连接coord节点验证数据是否正确
            testCoordSplitResult(rgNames);
    ...
    public void testSrcDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            if ( isReverse ) {
                //当切分键[79,29]为逆序时的期望结果是[1,28],[79,99]
                for (int i = 1; i <= 29; i++ ){
    ...
    public void testDestDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            List<BSONObject> expected = new ArrayList<BSONObject>();
            if ( isReverse ) {
                //当切分键[79,29]逆序时在目标组上的期望结果是[29,78]
                for (int i = 30; i <= 79; i++ ){
                    expected.add(this.insertRecods.get(i-1)); 
                }  
            }else {
