...
/**
 * @FileName:SEQDB-10499 range子表指定范围切分数据 1、向主表中插入数据记录 2、执行split，设置范围切分条件
 *                       3、查看数据切分结果（分别连接coord查询主表数据、源组data、目标组data查询子表数据；
 *                       执行find查询覆盖边界范围数据，执行data节点count带条件查询），
 *                       4、再次向主表插入数据（数据分别在源和目标组范围内） 5、查看插入数据结果
 * @author huangqiaohui
 * @version 1.00
 *
 */

public class Split10499 extends SdbTestBase {
	private String subCLName = "testcaseSubCL10499";
	private String mainCLName = "testcaseMainCL10499";
	...
	public void setUp() {
		try {
			...
			ArrayList<String> groupsName = commlib.getDataGroupNames(commSdb);
			if (groupsName.size() < 2) {
				throw new SkipException("current environment less than tow groups ");
			}

			CollectionSpace commCS = commSdb.getCollectionSpace(csName);
			mainCL = commCS.createCollection(mainCLName, (BSONObject) JSON.parse("{ShardingKey:{sk:1},IsMainCL:true}"));
			...
			mainCL.attachCollection(subCL.getFullName(), (BSONObject) JSON.parse("{LowBound:{sk:0},UpBound:{sk:100}}"));
	...
	public void split() {
		try {
			...
			subCL.split(srcGroupName, destGroupName, (BSONObject) JSON.parse("{sk:30}"), // 切分
			...
			checkSrcGroup(70, "{$or:[{sk:{$gte:0,$lt:30}},{sk:{$gte:60,$lt:100}}]}", 70);

			insertAndCheckAgain();// 再次插入数据，检查落入
	...
	private void checkCoord() {
		...
		try {
			// 比对所有数据
			cursor1 = mainCL.query(null, null, "{sk:1}", null);
			...
			// find边界sk:30,sk:60
			cursor2 = mainCL.query("{$or:[{sk:30},{sk:60}]}", "{sk:''}", "{sk:1}", null);
	...
	private void checkDestGroup(int expectedCount, String macher, int expectTotalCount) {
		Sequoiadb destDataNode = null;
		try {
			destDataNode = commSdb.getReplicaGroup(destGroupName).getMaster().connect();// 获得目标组主节点链接
			DBCollection destCL = destDataNode.getCollectionSpace(csName).getCollection(subCLName);
	...
	private void checkSrcGroup(int expectedCount, String macher, int expectTotalCount) {
		Sequoiadb srcDataNode = null;
		try {
			srcDataNode = commSdb.getReplicaGroup(srcGroupName).getMaster().connect();// 获得源主节点链接
			DBCollection srcCL = srcDataNode.getCollectionSpace(csName).getCollection(subCLName);
