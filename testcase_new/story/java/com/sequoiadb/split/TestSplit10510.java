 ...
 *
 */
public class TestSplit10510 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl;
    private String clName1 = "cl10510_0";
    private String clName2 = "cl10510_1";
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{height:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            ...
            BSONObject startCondition = (BSONObject) JSON.parse("{height:98}");
            BSONObject endCondition = (BSONObject) JSON.parse("{height:99}");
    ...
    public void testReverse() {
        try {
            ...
            this.insertRecods = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl, 100);
            BSONObject startCondition = (BSONObject) JSON.parse("{height:44}");
            BSONObject endCondition = (BSONObject) JSON.parse("{height:33}");
            this.cl.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            //连接coord节点验证数据是否正确
            testCoordSplitResult(rgNames);
    ...
    public void testCoordSplitResult(List<String> rgNames) {
        try {
            ...
            DBCursor cursor = this.cl.query(null,null,"{\"height\":1}",null);
     ...
     * @param isReverse
     */
    public void testSrcDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            DBCursor cursor = dbcl.query(null,null,"{\"height\":1}",null);
            List<BSONObject> actual = new ArrayList<BSONObject>();
            while( cursor.hasNext() ) {
                BSONObject obj = cursor.getNext();
                actual.add(obj);
            ...
            if ( isReverse ) {
                ...
                //在目标组上的期望结果是[1,97]和99
                for (int i = 0; i <= 96; i++ ){
                    expected.add(this.insertRecods.get(i)); 
                } 
                expected.add(this.insertRecods.get(98));
            }
     ...
     * @param isReverse
     */
    public void testDestDataSplitResult(List<String> rgNames, boolean isReverse) {
        Sequoiadb dataDb = null;
        try {
            //连接目标组data查询
            ...
            List<BSONObject> expected = new ArrayList<BSONObject>();
            if ( isReverse ) {
                //分区键逆序时在目标组上的期望结果是[34-44]
                for (int i = 34; i <= 44; i++ ){
                    expected.add(this.insertRecods.get(i-1)); 
                }  
            }else {
                //在源组上的期望结果是第98,99条记录
                expected.add(this.insertRecods.get(97)); 
            }
            Assert.assertEquals(actual, expected);
        } catch (BaseException e) {
            Assert.fail(e.getMessage());
