...
/**
 * 分区键为timestamp类型，执行百分比切分
 * @author chensiqin
 * @Date 2016-12-20
 *
 */
public class TestSplit10515 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl;
    private String clName1 = "cl10515_0";
    private String clName2 = "cl10515_1";
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{timestamp:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl = SplitUtils2.createCL(this.cs, this.clName1, option); 
            this.insertRecods = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl, 101);
            this.cl.split(rgNames.get(0), rgNames.get(1), 50);
            //连接coord节点验证数据是否正确
    ...
    public void testReverse() {
        try {
            ...
            this.insertRecods = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl, 101);
            this.cl.split(rgNames.get(0), rgNames.get(1), 10);
            //连接coord节点验证数据是否正确
            testCoordSplitResult(rgNames);
    ...
    public void testDestDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            if ( isReverse ) {
                //当切分键10%
                for (int i = 1; i <= 10; i++ ){
                    expected.add(this.insertRecods.get(i*10-1));
                }
            }else {
                //按50切时在组上的期望结果对应的是
                for (int i = 1; i < 101; i++ ){
