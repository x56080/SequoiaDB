...
public class Split10536 extends SdbTestBase {
    ...
    public void delteIdIndex() {
        ...
        try {
            ...
            // 校验源组和目标组的数据
            checkGroupData( db, 450, "{sk:{$gte:50,$lt:500}}", 450,
                    destGroupName );
    ...
    private void checkGroupData( Sequoiadb db, int expectedCount, String macher,
        ...
        try {
            ...
            Assert.assertEquals( count, expectedCount,
                    destDataNode.getServerAddress().toString() );// 目标组应当含有上述查询数据
            Assert.assertEquals( destCL.getCount(), expectTotalCount,
                    destDataNode.getServerAddress().toString() ); // 目标组应当含有的数据量
        } catch ( BaseException e ) {
            Assert.fail( e.getMessage() + "\r\n"
                    + SplitUtils.getKeyStack( e, this ) );
        } finally {
