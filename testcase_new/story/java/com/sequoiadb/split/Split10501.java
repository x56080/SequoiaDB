...
/**
 * @FileName:SEQDB-10501 hash分区表指定范围切分数据 1、向cl中插入数据记录，分别验证如下情况： a、普通记录 b、lob对象
 *                       c、普通记录+lob对象 2、执行split，设置范围切分条件
 *                       3、查看数据切分结果（分别连接coord、源组data、目标组data查询，查看cl编目信息,如执行db.
 *                       snapshot(8,{Name:"cs.cl"})） 4、再次插入数据（数据分别在源和目标组范围内）
 *                       5、查看插入数据结果 (备注：此用例验证c情况,a在10512有覆盖，b在7846有覆盖)
 * @author huangqiaohui
 * @version 1.00
 *
 */

public class Split10501 extends SdbTestBase {
	private String clName = "testcaseCL10501";
	...
	public void setUp() {
		try {
			...
			commCL = commCS.createCollection(clName, (BSONObject) JSON
	...
	public void split() {
		try {
			...
			commCL.split(srcGroupName, destGroupName, (BSONObject) JSON.parse("{Partition:2048}"), // 切分
			...
			Assert.assertEquals(insertedLobCopy.size() == 0, true, // 经过两次校验，insertedLobDataCopy应当为空
					"split error,destGroup and srcGroup can not fidn some lobs:" + insertedLobCopy);
	...
	private void checkGroupLob(ArrayList<String> insertedLob, String destGroupName) {
		Sequoiadb destDataNode = null;
		DBCursor cursor = null;
		try {
			destDataNode = commSdb.getReplicaGroup(destGroupName).getMaster().connect();// 获得源主节点链接
	...
	private void checkCatalog() {
		DBCursor dbc = null;
		try {
			dbc = commSdb.getSnapshot(Sequoiadb.SDB_SNAP_CATALOG, "{Name:\"" + csName + "." + clName + "\"}", null,
					null);
	...
	public void insertAndCheckAgain() {
		...
		try {
			...
			srcDataNode = commSdb.getReplicaGroup(srcGroupName).getMaster().connect();// 获得目标组主节点链接
			DBCollection destCL = destDataNode.getCollectionSpace(csName).getCollection(clName);
			...
			// 比对结果
			Assert.assertEquals(destCL.getCount(destRecord), 1, "query fail");
			Assert.assertEquals(destCL.getCount(srcRecord), 0, "query fail");
			Assert.assertEquals(srcCL.getCount(destRecord), 0, "query fail");
	...
	private void checkGroupData(ArrayList<BSONObject> insertedData, String groupName) {
		Sequoiadb dataNode = null;
		DBCursor cursor = null;
		try {
			dataNode = commSdb.getReplicaGroup(groupName).getMaster().connect();// 获得目标组主节点链接
