...
public class Split10502 extends SdbTestBase {
	...
	public void split() {
		try {
			...
			taskIdArr[0] = commCL.splitAsync(srcGroupName, destGroupName, (BSONObject) JSON.parse("{partition:2048}"), // 切分
	...
	private void checkGroupData(ArrayList<BSONObject> insertedData, String groupName) {
		...
		try {
			...
			while (cursor.hasNext()) {
				...
				Assert.assertEquals(insertedData.contains(actual), true,
						"insertedData can not find this record:" + actual + "all:" + insertedData);
				insertedData.remove(actual);
			}
