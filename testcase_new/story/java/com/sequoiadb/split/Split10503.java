...
public class Split10503 extends SdbTestBase {
	...
	public void setUp() {
		try {
			...
			subCL = commCS.createCollection(subCLName, (BSONObject) JSON.parse(
					"{ShardingKey:{sk:1},Partition:4096,ShardingType:'hash',Group:'" + srcGroupName + "'}"));
			mainCL.attachCollection(subCL.getFullName(), // 挂载
					(BSONObject) JSON.parse("{LowBound:{sk:0},UpBound:{sk:200}}"));
	...
	public void split() {
		try {
			...
			subCL.split(srcGroupName, destGroupName, (BSONObject) JSON.parse("{Partition:2048}"), // 切分
			...
			Assert.assertEquals(insertedDataCopy.size() == 0, true, // 经过两次校验，insertDataCopy应当为空
					"split error,destGroup and srcGroup can not find:" + insertedDataCopy);
