...
/**
 * @FileName:SEQDB-10498 range分区表指定百分比切分数据 1、向cl中插入数据记录 2、执行split，设置百分比切分条件
...
public class Split10498 extends SdbTestBase {
	private String clName = "testcaseCL10498";
	...
	public void split() {
		try {
			...
			// 期望目标组有50条符合{sk:{$gte:50,$lt:100}}查询条件的数据,期望目标组共有30条数据
			checkDestGroup(50, "{sk:{$gte:50,$lt:100}}", 50);
			// 期望源组有50条符合查询条件的数据,期望源组共有50条数据
			checkSrcGroup(50, "{sk:{$gte:0,$lt:50}}", 50);
	...
	public void insertAndCheckAgain() {
		BSONObject obj1 = (BSONObject) JSON.parse("{sk:80,flag:-1}");// 期望此数据落入目标组
		...
		checkDestGroup(1, "{sk:80,flag:-1}", 51);// 检测落入
		checkSrcGroup(1, "{sk:10,flag:-1}", 51);// 检测落入
	}
	...
	private void checkCoord() {
		...
		try {
			...
			// find边界sk:30,sk:60
			cursor2 = commCL.query("{sk:50}", "{sk:''}", "{sk:1}", null);
			...
			expectedResults.add((BSONObject) JSON.parse("{sk:50}"));
