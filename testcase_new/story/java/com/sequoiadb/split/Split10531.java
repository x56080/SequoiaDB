...
public class Split10531 extends SdbTestBase {
	...
	public void insertAndCheck() {
		...
		try {
			...
			Assert.assertEquals(insertedData.size() == 0, true, "srcGroup and destGroup can not find:" + insertedData);

			// 校验源和目标组LOB记录
			checkGroupLob(insertedLob, db, destGroupName);
			checkGroupLob(insertedLob, db, srcGroupName);
			Assert.assertEquals(insertedLob.size() == 0, true, "srcGroup and destGroup can not find:" + insertedLob);
	...
	private void checkGroupData(List<BSONObject> insertedData, Sequoiadb sdb, String groupName) {
		...
		try {
			...
			Assert.assertEquals(count > 250 - (250 * 0.3) && count < 250 + (250 * 0.3), true,
