...
public class TestSplit10522C extends SdbTestBase{
    ...
    public void test() {
        ...
        //创建主表
        BSONObject option = 
                (BSONObject) JSON.parse("{IsMainCL:true,ShardingKey:{_id:-1,num:-1},ShardingType:\"range\"}");
        this.mainCL = SplitUtils2.createCL(this.cs, this.mainCLName, option);
        
        //_id+float
        option = 
              (BSONObject) JSON.parse("{ShardingKey:{_id:-1,num:-1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
        this.subCL = SplitUtils2.createCL(this.cs, this.subCLName, option);
        //挂载子表
        option = (BSONObject) JSON.parse("{LowBound:{_id:11,num:2.0}, UpBound:{_id:0,num:0.0001}}");
