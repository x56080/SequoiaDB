...
/**
 * @FileName:SEQDB-10505 hash分区表指定百分比异步切分数据 1、向cl中插入数据（包括普通记录和lob）
 *                       2、执行splitAsync，设置百分比切分条件
...
public class Split10505 extends SdbTestBase {
	private String clName = "testcaseCL10505";
	...
	public void split() {
		try {
			...
			taskIdArr[0] = commCL.splitAsync(srcGroupName, destGroupName, 50);
			...
			Assert.assertEquals(insertedLobCopy.size() == 0, true, // 经过两次校验，insertedLobDataCopy应当为空
					"split error,destGroup and srcGroup can not fidn some lobs:" + insertedDataCopy);
