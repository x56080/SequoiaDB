...
public class TestSplit10524 extends SdbTestBase{
    ...
    public void test() {
        try {
            ...
            BSONObject option = 
                    (BSONObject) JSON.parse("{ShardingKey:{num:1,age:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
            this.cl = SplitUtils2.createCL(this.cs, this.clName, option); 
           
            insertData();

            BSONObject startCondition = 
                    (BSONObject) JSON.parse("{num:0.01,age:-9}"); 
            BSONObject endCondition = 
                    (BSONObject) JSON.parse("{num:0.01,age:2}");
