...
public class Split10539 extends SdbTestBase {
	...
	public void setUp() {
		...
		try {
			...
			DBCollection mainCL = commCS.createCollection(mainCLName,
					(BSONObject) JSON.parse("{ShardingKey:{sk:1},IsMainCL:true}"));
	...
	private void checkMainCL(List<BSONObject> allInsertedData) {
		...
		try {
			...
			Assert.assertEquals(allInsertedData.size(), 0, "miss some records:" + allInsertedData);
			...
			if (cursor2.hasNext()) {
				...
				actual.removeField("_id");
				BSONObject expect = (BSONObject) JSON.parse("{sk:0,alpha:0}");
				Assert.assertEquals(expect.equals(actual), true, "expect:" + expect + " actual:" + actual);
				if (cursor2.hasNext()) {
					Assert.fail("mainCL should not have this record:" + cursor2.getNext());
				}
			}
			...
			if (cursor3.hasNext()) {
				...
				actual.removeField("_id");
				BSONObject expect = (BSONObject) JSON.parse("{sk:999,alpha:999}");
				Assert.assertEquals(expect.equals(actual), true, "expect:" + expect + " actual:" + actual);
				if (cursor3.hasNext()) {
