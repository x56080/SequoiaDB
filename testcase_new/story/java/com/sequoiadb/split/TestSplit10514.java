...
public class TestSplit10514 extends SdbTestBase{
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{date:1},ShardingType:\"range\",Group:\"" + rgNames.get(0) + "\"}");
...
//            endCondition.put("date", date2);
            BSONObject startCondition = (BSONObject) JSON.parse("{date:{\"$date\":\"2017-02-02\"}}");
            BSONObject endCondition = (BSONObject) JSON.parse("{date:{\"$date\":\"2017-02-27\"}}");
...
//            startCondition.put("date", date1);
//            endCondition.put("date", date2);
            BSONObject startCondition = (BSONObject) JSON.parse("{date:{\"$date\":\"2017-03-01\"}}");
            BSONObject endCondition = (BSONObject) JSON.parse("{date:{\"$date\":\"2017-01-31\"}}");
            this.cl.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
            //连接coord节点验证数据是否正确
            testCoordSplitResult(rgNames);
