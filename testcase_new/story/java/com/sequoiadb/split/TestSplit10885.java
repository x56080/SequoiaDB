...
/**
 * hash分区表指定分区键值为数组类型，执行切分 
 * @author chensiqin
 * @Date 2016-12-21
 *
 */
public class TestSplit10885 extends SdbTestBase{
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl1;
    private DBCollection cl2;
    private String clName1 = "cl10885_0";
    private String clName2 = "cl10885_1";
    ...
    public void test() {
        try {
            ...
            BSONObject option = (BSONObject) JSON.parse("{ShardingKey:{arrtype:1},ShardingType:\"hash\",Partition:1024,Group:\"" + rgNames.get(0) + "\"}");
            this.cl1 = SplitUtils2.createCL(this.cs, this.clName1, option); 
            this.insertRecords = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl1, 100);
            ...
            this.cl1.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
    ...
    public void testReverse() {
        try {
            ...
            this.insertRecords = (ArrayList<BSONObject>) SplitUtils2.insertData(this.cl2, 100);
            BSONObject startCondition = new BasicBSONObject();
            BSONObject endCondition = new BasicBSONObject();
            startCondition.put("Partition", 256);
            endCondition.put("Partition", 1024);
            this.cl2.split(rgNames.get(0), rgNames.get(1), startCondition, endCondition);
    ...
    public void testCoordSplitResult(List<String> rgNames,boolean isReverse) {
        try {
            //连接coord节点验证数据是否正确
    ...
    public void testSrcDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            if ( isReverse ) {
                //partition范围是[256,1024]
                if ( Math.abs((25 - actual))/25.0 > 0.3) {
    ...
    public void testDestDataSplitResult(List<String> rgNames, boolean isReverse) {
        ...
        try {
            ...
            if ( isReverse ) {
                //partition范围是[256,1024]
                if ( Math.abs((75 - actual))/75.0 > 0.3) {
