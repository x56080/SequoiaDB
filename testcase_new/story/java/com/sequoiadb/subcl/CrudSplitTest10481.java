...
public class CrudSplitTest10481 extends SdbTestBase {
	...
	public void setUp() {
		...
		try {
			...
			e.printStackTrace();
			Assert.assertTrue(false, "judge at least three data sets failed "
					+ e.getMessage());
		}
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if (!coordHostName.equals(SdbTestBase.hostName)) {
					sdb_other = new Sequoiadb(coordHostName + ":"
							+ SdbTestBase.serviceName, "", "");
					break;
				}
			}
		} catch (BaseException e) {
	...
	public void test() {
		...
		try{
			...
			for (int i = 400; i < 3000; i++) {
				a = i % 200;
				maincl.insert((BSONObject) JSON.parse(" {name_:'" + i + "',a:"
						+ a + "} "));
			}
			for (int i = 0; i < 200; i++) {
				maincl.upsert(
						(BSONObject) JSON.parse(" {name:'name_" + i + "'} "),
						(BSONObject) JSON.parse(" {$set:{name:'updatename_" + i
								+ "'}} "), null);
			}
	...
	private void initData() {
		try {
			...
			for (int i = 0; i < 400; i++) {
				a = i % 200;
				initData.add((BSONObject) JSON.parse(" {name:'name_" + i
						+ "', a:" + a + "} "));
			}
		...
		try {
			...
			if (count == 2800) {
				...
				DBCursor explain_A50 = maincl.explain(
						(BSONObject) JSON.parse(" {a:50} "), null, null, null,
						0, -1, DBQuery.FLG_QUERY_STRINGOUT, null);
				DBCursor explain_A149 = maincl.explain(
						(BSONObject) JSON.parse(" {a:149} "), null, null, null,
						0, -1, DBQuery.FLG_QUERY_STRINGOUT, null);
				DBCursor explain_A150 = maincl.explain(
						(BSONObject) JSON.parse(" {a:150} "), null, null, null,
						0, -1, DBQuery.FLG_QUERY_STRINGOUT, null);
				BSONObject[] objArr = { explain_A50.getNext(),
						explain_A149.getNext(), explain_A150.getNext() };

				if ((explain_A50.getNext() == null
						|| explain_A149.getNext() == null || explain_A150
						.getNext() == null)
						&& (((BasicBSONList) objArr[0].get("SubCollections"))
								.size() == 1
								&& ((BasicBSONList) objArr[1]
										.get("SubCollections")).size() == 1 && ((BasicBSONList) objArr[2]
					...
					if (!(replicaGroupNames.get(0).equals(
							objArr[0].get("GroupName")) && (SdbTestBase.csName
							+ "." + subclNames[0]).equals(subcl_A50))) {
						System.out.println("a");
						flag = false;
					}
					String subcl_A149 = (String) ((BSONObject) ((BasicBSONList) objArr[1]
							.get("SubCollections")).get(0)).get("Name");
					if (!(replicaGroupNames.get(1).equals(
							objArr[1].get("GroupName")) && (SdbTestBase.csName
							+ "." + subclNames[1]).equals(subcl_A149))) {
