...
/**
 * FileName: CreateMainCl25.java
 * test content:  创建子表时指定hash分区时的ShardingKey与主表相同 
 * testlink case: seqDB-25
 * @author zengxianquan
 * @date 2016年12月9日
 * @version 1.00
 */
public class CreateMainCl25 extends SdbTestBase{
		
	private Sequoiadb db = null;
	private String csName = null;
	private CollectionSpace cs = null;
	private String mainclName = "maincl_25";
	private String subclName = "subcl_25";
	...
	public void tearDown(){
		try{
			...
			Assert.fail("failed to drop maincl "+"ErrorMsg:\n" +e.getMessage());
		}finally{
			db.disconnect();
		}
	}
	...
	public void attachSubcl(){
		...
		subOpt.put("time", 1);
		subclOptions.put("ShardingType","hash");
		...
		try{
			cs.getCollection(mainclName).attachCollection(csName+"."+subclName, attachOpt);			
		}catch(BaseException e){
	...
	public void checkCRUD(){
		...
		try{	
			...
			Assert.assertEquals(maincl.getCount(),0,"failed to delete data");
