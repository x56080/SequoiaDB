	...
	public void setUp() {
		try {
			...
			if (groupsName.size() < 2) {
				throw new SkipException("current environment less than one groups " + this.getClass().getName());
			}
			...
			subCL = commCS.createCollection(subCLName, (BSONObject) JSON.parse("{ShardingKey:{sk:1}}"));
