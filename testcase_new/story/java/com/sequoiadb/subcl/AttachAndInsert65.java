...
/**
 * FileName: AttachAndInsert65
 * test content: 不同coord连接，在一个coord节点上attach完子表，马上在另一个coord节点上写入数据 
 * testlink case: seqDB65
 * @author zengxianquan
 * @date 2016年12月17日
 * @version 1.00
 * */
public class AttachAndInsert65 extends SdbTestBase {
	...
	public void createCl(Sequoiadb db){
		...
		try{
			...
			BSONObject mainObj = (BSONObject)JSON.parse("{IsMainCL:true,ShardingKey:{age:1},"
																+ "ShardingType:\"range\"}");
			cs.createCollection(mainclName, mainObj);    
			
			BSONObject subObj = (BSONObject)JSON.parse("{ShardingKey:{age:1},"
	...
	public void attachSubcl(Sequoiadb db){
	    ...
	    try{
	        ...
	        BSONObject options=(BSONObject)JSON.parse("{LowBound:{age:50},UpBound:{age:100}}");
