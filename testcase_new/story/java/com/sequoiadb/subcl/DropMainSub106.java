...
public class DropMainSub106 extends SdbTestBase {
	...
	public void checkTable() {
		...
		try {
			...
			Assert.assertEquals(1, count1, "insert fail,the count1 is "
					+ count1);
		} catch (BaseException e) {
		...
		try {
			...
			Assert.assertEquals(1, count2, "insert fail,the count2 is "
					+ count2);
			mainCl1.update("{a:288}", "{$set:{name:\"upset85\"}}", null);
			long count3 = mainCl1.getCount("{a:288}");
			Assert.assertEquals(1, count3, "updata fail,the count3 is "
					+ count3);
			mainCl1.delete("{a:288}");
			long count4 = mainCl1.getCount("{a:288}");
			Assert.assertEquals(0, count4, "delete fail,the count4 is "
					+ count4);
	...
	public void attach() {
		try {
			mainCl1.attachCollection(subCl1.getFullName(),
