...
public class DropMainSub107 extends SdbTestBase {
	...
	public void testMasterTable() {
		...
		try {
			...
			Assert.assertEquals(1, count1, "insert fail,the count1 is" + count1);
			mainCl1.update("{a:388}", "{$set:{name:\"upset85\"}}", null);
			long count2 = mainCl1.getCount("{a:388}");
			Assert.assertEquals(1, count2, "updata fail,the count2 is" + count2);
			mainCl1.delete("{a:388}");
			long count3 = mainCl1.getCount("{a:388}");
			Assert.assertEquals(0, count3, "delete fail,the count3 is" + count3);
	...
	public void attach() {
		try {
			this.mainCl1
					.attachCollection(subCl1.getFullName(), (BSONObject) JSON
