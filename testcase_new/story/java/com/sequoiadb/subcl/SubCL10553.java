...
/**
 * @TestName:SEQDB-10553 子表在不同分区组上已存在索引，通过主表进行索引操作
 *                       1.创建主子表，其中子表指定在不同的数据组上，且所有子表已存在步骤2中操作的索引
...
public class SubCL10553 extends SdbTestBase {
	private Sequoiadb sdb;
	private CollectionSpace commCS;
	private DBCollection mainCL;
	private DBCollection subCL1;
	private DBCollection subCL2;
	private String mainCLName = "mainCL10553";
	private String subCL1Name = "subCL10553_1";
	private String subCL2Name = "subCL10553_2";
	...
	public void setUp() {
		try {
			...
			// 创建主子表
			mainCL = commCS.createCollection(mainCLName, (BSONObject) JSON.parse("{IsMainCL:true,ShardingKey:{sk:1}}"));
			subCL1 = commCS.createCollection(subCL1Name,
					(BSONObject) JSON.parse("{ShardingKey:{sk:1},AutoIndexId:false,EnsureShardingIndex:false,Group:\""
							+ groupList.get(0) + "\"}"));
	...
	public void test() {
		try {
			...
			queryByIndexAndCheckExplain("{sk:80}", "{sk:80,num:80,_id:{$oid:\"" + oidPrefix + "80\"}}", "ixscan");
			queryByIndexAndCheckExplain("{num:20}", "{sk:20,num:20,_id:{$oid:\"" + oidPrefix + "20\"}}", "ixscan");
			queryByIndexAndCheckExplain("{_id:{$oid:\"" + oidPrefix + "50\"}}",
					"{sk:50,num:50,_id:{$oid:\"" + oidPrefix + "50\"}}", "ixscan");
			...
			// 再次查询，检查扫描方式
			queryByIndexAndCheckExplain("{sk:80}", "{sk:80,num:80,_id:{$oid:\"" + oidPrefix + "80\"}}", "tbscan");// 通过id索引查询，期望扫描方式为ixscan
			queryByIndexAndCheckExplain("{num:20}", "{sk:20,num:20,_id:{$oid:\"" + oidPrefix + "20\"}}", "tbscan");
			queryByIndexAndCheckExplain("{_id:{$oid:\"" + oidPrefix + "50\"}}",
	...
	private void insertData() {
		...
		for (int i = 0; i < 100; i++) {
			mainCL.insert((BSONObject) JSON
					.parse("{sk:" + i + ",num:" + i + ",_id:{$oid:\"" + oidPrefix + df.format(i) + "\"}}"));
		}
	}
	...
	public void createIndexAndAttach() {
		try {
			...
			subCL2.createIndex("shardingKeyIndex", "{sk:1}", false, false);
			subCL2.createIdIndex(new BasicBSONObject());//
			subCL2.createIndex("commIndex", "{num:1}", false, false);

			mainCL.attachCollection(subCL1.getFullName(), // 挂载
					(BSONObject) JSON.parse("{LowBound:{sk:0},UpBound:{sk:50}}"));
			mainCL.attachCollection(subCL2.getFullName(),
					(BSONObject) JSON.parse("{LowBound:{sk:50},UpBound:{sk:100}}"));
		} catch (BaseException e) {
			throw e;
		}
	}
	...
	private void queryByIndexAndCheckExplain(String macher, String expectedRecord, String expectScanType) {
		DBCursor dbc1 = null;
		DBCursor dbc2 = null;
		...
		try {
			...
			if (queryReaults.size() == 1) {
				Assert.assertEquals(expected.equals(queryReaults.get(0)), true,
						"expected:" + expected.toString() + " actual:" + queryReaults.get(0));
			} else {
				Assert.fail("query resault not correct,the array:" + queryReaults);
			}

			// 检查扫描方式
			dbc2 = mainCL.explain((BSONObject) JSON.parse(macher), null, null, null, 0, -1, 0, null);
			if (dbc2.hasNext()) {
				BasicBSONList list = (BasicBSONList) dbc2.getNext().get("SubCollections");
				BSONObject message = (BSONObject) list.get(0);
				Assert.assertEquals(message.get("ScanType").equals(expectScanType), true,
						"scanType not " + expectScanType);
			} else {
	...
	public void checkIndexExist(DBCollection cl) {
		...
		try {
			...
			while (dbc.hasNext()) {
				...
				if (machFlag == false) {
					Assert.fail("cant not find " + actual.get("name") + "index message");
				}
				machFlag = false;
			}
		} catch (BaseException e) {
