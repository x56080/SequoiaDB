...
/**
 * FileName: CreateMainCl21.java
 * test content: 创建主表的CS指定domain 
 * testlink case: seqDB-21
 * @author zengxianquan
 * @date 2016年12月9日
 * @version 1.00
 */
public class CreateMainCl21 extends SdbTestBase{
	...
	public void testCreateCsByAppointDomain(){
		...
		try{
			...
			Assert.fail("Failed to create cs  "+"ErrorMsg:\n"+e.getMessage());
		}
		//创建maincl在cs中
		createMaincl();
		//检验快照信息
	...
	public void createMaincl(){
		...
		try{
			...
			Assert.fail("Failed to create maincl_21  "
					+ "ErrorMsg:\n" +e.getMessage());
		}
	}
		...
		try{
			...
			while(cursor1.hasNext()){
				...
				if(snapshotRes1 != null){
					snapshotDetails = (BasicBSONList) snapshotRes1.get("Details");
			...
			while(cursor2.hasNext()){
				...
				if(snapshotRes2 != null){
					snapshotCataInfo = (BasicBSONList) snapshotRes2.get("CataInfo");
					if(snapshotCataInfo != null&&snapshotCataInfo.size() != 0){
