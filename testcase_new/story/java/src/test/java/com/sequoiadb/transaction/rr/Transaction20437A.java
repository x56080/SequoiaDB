...
/**
 * @testcase seqDB-20437: 只读事务与只写事务并发，只写事务覆盖：插入、更新、删除，TW1事务包含TW2、TW3、TW4,不同时刻发起事务读，隔离级别为RR
 * @date 2020-01-15
 * @author zhaoxiaoni
 */
@Test(groups = "rr")
public class Transaction20437A extends SdbTestBase {
    ...
    public void setUp() {
        ...
        TW4 = new Sequoiadb( SdbTestBase.coordUrl, "", "" );
        TR7 = new Sequoiadb( SdbTestBase.coordUrl, "", "" );
        TR8 = new Sequoiadb( SdbTestBase.coordUrl, "", "" );
        TR9 = new Sequoiadb( SdbTestBase.coordUrl, "", "" );
        TR10 = new Sequoiadb( SdbTestBase.coordUrl, "", "" );
    ...
    public void test( String hint ) {
        ...
        QueryThread queryThread1 = new QueryThread( TR1, hint, new ArrayList<BSONObject>( expList ) );
        queryThread1.start();
        ...
        QueryThread queryThread2 = new QueryThread( TR2, hint, new ArrayList<BSONObject>( expList ) );
        queryThread2.start();
        ...
        cl2.update( "{ '_id': { '$lt': 100 } }", "{ '$set': { 'a': 600 } }", hint);//将0-100的记录更新成600
        cl2.delete( "{ '$and': [ { '_id': { '$gte': 100 } }, { '_id': { '$lt': 200 } }] }" );//删除100-200记录
        
        TR3.beginTransaction();
        QueryThread queryThread3 = new QueryThread( TR3, hint, new ArrayList<BSONObject>( expList ) );
        queryThread3.start();
        ...
        QueryThread queryThread4 = new QueryThread( TR4, hint, new ArrayList<BSONObject>( expList ) );
        queryThread4.start();
        ...
        cl3.update( "{ '_id': { '$lt': 100 } }", "{ '$set': { 'a': 800 } }", hint);//将0-100的记录更新成800
        cl3.delete( "{ '$and': [ { '_id': { '$gte': 200 } }, { '_id': { '$lt': 300 } }] }" );//删除200-300记录
        
        TR5.beginTransaction();
        QueryThread queryThread5 = new QueryThread( TR5, hint, new ArrayList<BSONObject>( expList ) );
        queryThread5.start();
        ...
        QueryThread queryThread6 = new QueryThread( TR6, hint, new ArrayList<BSONObject>( expList ) );
        queryThread6.start();
        ...
        cl4.update( "{ '_id': { '$lt': 100 } }", "{ '$set': { 'a': 900 } }", hint);//将_id字段值为0-100的记录的a字段值更新成900
        cl4.delete( "{ '$and': [ { '_id': { '$gte': 300 } }, { '_id': { '$lt': 400 } }] }" );//删除_id字段值为300-400的记录

        TR7.beginTransaction();
        QueryThread queryThread7 = new QueryThread( TR7, hint, new ArrayList<BSONObject>( expList ) );
        queryThread7.start();
        ...
        expList = TransUtils.deleteList( expList, 100, 200 );
        QueryThread queryThread8 = new QueryThread( TR8, hint, new ArrayList<BSONObject>( expList ) );
        queryThread8.start();
        
        //10.TW1插入记录R11s，更新R10s为R12s,删除记录R5s+R7s+R9s,开启读事务TR9,所有读事务读记录
        TransUtils.insertRandomDatas( cl1, 1000, 1100 );
        cl1.update( "{ '_id': { '$lt': 100 } }", "{ '$set': { 'a': 1100 } }", hint);
        cl1.delete( "{ '$and': [ { '_id': { '$gte': 400 } }, { '_id': { '$lt': 900 } }] }" );
        
        TR9.beginTransaction();
        QueryThread queryThread9 = new QueryThread( TR9, hint, new ArrayList<BSONObject>( expList ) );
        queryThread9.start();
        ...
        expList = TransUtils.updateList( expList, 0, 100, 1100 );
        expList = TransUtils.deleteList( expList, 100, 200 );
        expList = TransUtils.deleteList( expList, 100, 200 );
        expList = TransUtils.deleteList( expList, 100, 200 );
        QueryThread queryThread10 = new QueryThread( TR10, hint, new ArrayList<BSONObject>( expList ) );
        queryThread10.start();
        ...
        Assert.assertTrue( queryThread3.isSuccess(), queryThread3.getErrorMsg() );
        Assert.assertTrue( queryThread4.isSuccess(), queryThread4.getErrorMsg() );
        Assert.assertTrue( queryThread5.isSuccess(), queryThread5.getErrorMsg() );
        Assert.assertTrue( queryThread6.isSuccess(), queryThread6.getErrorMsg() );
        Assert.assertTrue( queryThread7.isSuccess(), queryThread7.getErrorMsg() );
        Assert.assertTrue( queryThread8.isSuccess(), queryThread8.getErrorMsg() );
        Assert.assertTrue( queryThread9.isSuccess(), queryThread9.getErrorMsg() );
    ...
    class QueryThread extends SdbThreadBase {
        ...
        public QueryThread( Sequoiadb db, String hint, List<BSONObject> expList ) {
