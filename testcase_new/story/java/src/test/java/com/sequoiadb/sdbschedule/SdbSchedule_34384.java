...
/**
 * @Dscription Test of transfer with delete some data in source collection and
 *             disable "delete_more_lob_in_target" option
 */
public class SdbSchedule_34384 extends SdbTestBase {

    private Sequoiadb sDB;
    private Sequoiadb tDB;
    private String scheduleName = "SdbSchedule_34384_schedule";
    private String csName = "SdbSchedule_34384";
    private String clName = "SdbSchedule_34384";
    private String sourceSite = "rootsite";
    private String targetSite;
    private DBCollection sCL;
    private int lobCount = 10;
    private List< ObjectId > removeLobIds = new ArrayList<>();
    private List< ObjectId > notRemoveLobIds = new ArrayList<>();
    private int removeLobCount = 5;
    ...
    public void setUp() {
        ...
        for ( int i = 0; i < lobCount; i++ ) {
            ...
            String content = "This is a test lob content for SdbSchedule_34384 "
                    + i;
            lob.write( content.getBytes() );
            lob.close();
            if ( i < removeLobCount ) {
                removeLobIds.add( lob.getID() );
            }
            else {
                notRemoveLobIds.add( lob.getID() );
            }
        }
    }
    ...
    public void test() throws Exception {
        ...
        for ( ObjectId removeLobId : removeLobIds ) {
            sCL.removeLob( removeLobId );
        }
        ...
        try ( DBCursor cursor = tCL.listLobs(
                new BasicBSONObject( "Oid",
                        new BasicBSONObject( "$in", removeLobIds ) ),
                null, null, null, 0, -1 ) ;) {
            int count = 0;
            ...
            Assert.assertEquals( count, removeLobCount );
        }

        try ( DBCursor cursor = tCL.listLobs(
                new BasicBSONObject( "Oid",
                        new BasicBSONObject( "$in", notRemoveLobIds ) ),
                null, null, null, 0, -1 ) ;) {
            int count = 0;
            while ( cursor.hasNext() ) {
                cursor.getNext();
            ...
            Assert.assertEquals( count, notRemoveLobIds.size() );
        }
