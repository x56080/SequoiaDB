...
/**
 * @testcase seqDB-23501:混合隔离级别，RR事务后开启，查询符合RR隔离级别
 * @date 2021-01-26
 * @author chenxiaodan
 */
@Test(groups = { "rr" })
public class Transaction23501 extends SdbTestBase {
	private Sequoiadb db1 = null;
	private Sequoiadb db2 = null;
	private String clName = "cl23501";
	...
	public void test() throws InterruptedException {
		...
		// 连接2，开启rr读事务
		db2.setSessionAttr((BSONObject) JSON.parse("{TransIsolation: 3}"));
		TransUtils.beginTransaction(db2);

		// 连接1执行事务
		cl1.insert("{_id:3, a:3, b:3}");
		cl1.update("{_id:1}", "{$set:{a:4}}", null);
		cl1.delete("{_id:2}");

		// 连接2查询
		cl2 = db2.getCollectionSpace(csName).getCollection(clName);
		TransUtils.queryAndCheck(cl2, "{'':null}", expList);
		TransUtils.queryAndCheck(cl2, "{'':'a'}", expList);
