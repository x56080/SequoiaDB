...
/**
 * @description seqDB-31538:并发创建全文索引和删除数据
 * @author Cheng Jingjing
 * @date 2023.5.29
 * @version 1.10
 */
public class Fulltext31538 extends FullTestBase {
    ...
    public void test() throws Exception {
        ...
        thExecutor.addWorker(new dropData());
        ...
        try (Sequoiadb db2 = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
            ...
            Assert.assertEquals( dbCursor.getNext(), null  );
            DBCursor esCursor = cl2.query(
                    "{'':{'$Text':{'query':{'match_all':{}}}}}", "{}",
                    "{_id:1}", "{'':'" + fullIdxName + "'}");
            Assert.assertEquals( esCursor.getNext(), null );
        }
    ...
    private class dropData {
        ...
        private void dropRecords() {
            try ( Sequoiadb db = new Sequoiadb( coordUrl, "", "" )) {
                DBCollection cl = db.getCollectionSpace( csName )
                        .getCollection( clName );
                BSONObject matcher = new BasicBSONObject( "$exists", 1 );
                BSONObject deleteConf = new BasicBSONObject( "tobj.test", matcher );
                cl.deleteRecords( deleteConf );
            }
        }
    }
