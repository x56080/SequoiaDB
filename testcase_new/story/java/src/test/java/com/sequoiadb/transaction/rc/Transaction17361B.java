...
/**
 * @Description seqDB-17361:插入与删除并发， 删除的记录同时匹配已提交记录及其他事务插入的记录，事务回滚，过程中读，过程中读
 *              R2>R1
 * @author luweikang
 * @date 2019年1月15日
 */
@Test(groups = { "rc", "rr" })
public class Transaction17361B extends SdbTestBase {

    private String clName = "transCL_17361B";
    ...
    public void setUp() {
        ...
        insertR1 = ( BSONObject ) JSON
                .parse( "{_id:'insertID17361B_1',a:1,b:1,c:1}" );
        insertR2 = ( BSONObject ) JSON
                .parse( "{_id:'insertID17361B_2',a:2,b:2,c:2}" );
    ...
    public void test( String indexKey, List< BSONObject > expPositiveReadList1,
        ...
        try {
            ...
            expDataList.add( insertR1 );
            expDataList.add( insertR2 );
            TransUtils.queryAndCheck( cl, orderBy1, hintTbScan, expDataList );

            // 非事务正序索引读
            TransUtils.queryAndCheck( cl, orderBy1, hintIxScan, expDataList );

            // 非事务逆序记录读
            Collections.reverse( expDataList );
            TransUtils.queryAndCheck( cl, orderBy2, hintTbScan, expDataList );

            // 非事务逆序索引读
            TransUtils.queryAndCheck( cl, orderBy2, hintIxScan, expDataList );
