...
/**
 * @Description seqDB-20342 SHARED_READ|WRITE和SHARED_READ|WRITE并发读写lob
 * @author luweikang
 * @Date 2019.8.26
 */

public class ShareLob20342B extends SdbTestBase {
    ...
    private String clName = "cl20342B";
    private String mainCLName = "mainCL20342B";
    private String subCLName = "subCL20342B";
    ...
    public void testLob( String clName, byte[] writeLobBuff ) {
        ...
        // SHARED_READ|WRITE读区域和SHARED_READ|WRITE读区域相离
        lob2.lockAndSeek( writeSize * 3, writeSize );
        lob2.read( readLob1 );
        byte[] expData1 = Arrays.copyOfRange( lobBuff, writeSize * 3,
                writeSize * 4 );
        RandomWriteLobUtil.assertByteArrayEqual( readLob1, expData1,
                "lob data is wrong" );

        // SHARED_READ|WRITE读区域和SHARED_READ|WRITE读区域相交
        try {
            lob3.lockAndSeek( writeSize + 1024, writeSize );
            lob3.read( readLob2 );
