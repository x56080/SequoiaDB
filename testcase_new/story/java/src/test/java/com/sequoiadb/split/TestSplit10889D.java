...
public class TestSplit10889D extends SdbTestBase {
    ...
    public void test() {
        ...
        BSONObject option = ( BSONObject ) JSON.parse(
                "{IsMainCL:true,ShardingKey:{height:-1,timestamp:-1},ShardingType:\"range\"}" );
        this.mainCL = SplitUtils2.createCL( this.cs, this.mainCLName, option );

        // a、long、timestamp
        option = ( BSONObject ) JSON
                .parse( "{ShardingKey:{height:-1,timestamp:-1},"
        ...
        // 执行切分
        long splitAsync = this.subCL.splitAsync( rgNames.get( 0 ),
