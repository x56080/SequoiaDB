...
public class Transaction20443A extends SdbTestBase {
    ...
    public Object[][] useIndex() {
        return new Object[][] { { "{ \"\": \"index_20443\" }" }, { "{ \"\": null }" } };
    }
    ...
    public void test( String hint ) {
        ...
        //TR1使用内置SQL读记录
        String sql = "select * from " + csName + "." + clName + " order by _id /*+use_index(index_20443A)*/";
        DBCursor cursor = db1.exec( sql );
        actList = TransUtils.getReadActList( cursor );
        Assert.assertEquals( actList, expList );

        actList.clear();
        sql = "select * from " + csName + "." + clName + " order by _id /*+use_index(NULL)*/";
        cursor = db1.exec( sql );
        actList = TransUtils.getReadActList( cursor );
        Assert.assertEquals( actList, expList );
        ...
        //TR1使用内置SQL读记录
        actList.clear();
        sql = "select * from " + csName + "." + clName + " order by _id /*+use_index(index_20443A)*/";
        cursor = db1.exec( sql );
        actList = TransUtils.getReadActList( cursor );
        Assert.assertEquals( actList, expList );
