...
/**
 * FileName: TestConcurrentWriteLob10423.java test content:Concurrent write lob
 * ,contains the same oid and different oid testlink case:seqDB-10423
 * 
 * @author wuyan
 * @Date 2016.9.12
 * @version 1.00
...
public class TestConcurrentWriteLob10423 extends SdbTestBase {
    ...
    private class WriteSameOidLobTask extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            ...
            try {
                ...
                if ( e.getErrorCode() != -297 && e.getErrorCode() != -269
                        && e.getErrorCode() != -5 ) {
                    Assert.assertTrue( false, "same oid write fail "
    ...
    private void checkWriteOkLobBySameOid( ObjectId oid,
            byte[] sameOidWriteOkBuff ) {
        // read and check the lob data
        DBLob rLob = dbcl.openLob( oid );
        byte[] rbuff = new byte[ ( int ) rLob.getSize() ];
        rLob.read( rbuff );
        rLob.close();
        LobOprUtils.assertByteArrayEqual( rbuff, sameOidWriteOkBuff,
                "lob:" + oid.toString() + "  data is wrong!" );
