...
/**
 * @FileName:seqDB-10878: hash分区表指定分区键值为OID类型，执行切分 1、创建cl，指定shardingType为“hash”
 *                        2、向cl中插入数据，其中分区键字段值为OID类型 3、执行splitAysnc异步切分，设置范围切分条件
 *                        4、查看数据切分结果（分别连接coord、源组data、目标组data查询
 * @Author linsuqiang
 * @Date 2017-01-04
 * @Version 1.00
 */
public class TestSplit10878 extends SdbTestBase {
    private Sequoiadb sdb;
    private String clName = "cl_10878";
    ...
    public void test() {
        try {
            ...
            taskIDs[ 0 ] = cl.splitAsync( srcGroup, dstGroup, startCondition,
                    endCondition );
