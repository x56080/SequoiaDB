...
public class SubCL10553 extends SdbTestBase {
    ...
    public void checkIndexExist( DBCollection cl ) {
        DBCursor dbc = null;
        BSONObject idIndex = ( BSONObject ) JSON.parse(
                "{name: \"$id\",key: {_id: 1},v: 0,unique: true,dropDups: false,enforced: true}" );
        BSONObject shardingKeyIndex = ( BSONObject ) JSON.parse(
                "{name: \"shardingKeyIndex\",key: {sk: 1},v: 0,unique: false,dropDups: false,enforced: false}" );
        BSONObject commIndex = ( BSONObject ) JSON.parse(
                "{name: \"commIndex\",key: {num: 1},v: 0,unique: false,dropDups: false,enforced: false}" );
        ...
        try {
            dbc = cl.getIndexes();
            while ( dbc.hasNext() ) {
                BSONObject actual = ( BSONObject ) dbc.getNext()
                        .get( "IndexDef" );
