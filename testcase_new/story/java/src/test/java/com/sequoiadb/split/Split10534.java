    ...
    public void createIndex() {
        ...
        try {
            ...
            for ( int i = 0; i < 62; i++ ) {
                ...
                int oder = i % 2 == 0 ? 1 : -1;
                cl.createIndex( indexName, "{" + indexName + ":" + oder + "}",
                        false, false );
                BSONObject indexObj = ( BSONObject ) JSON.parse( "{name:'"
                        + indexName + "',key: {" + indexName + ": " + oder
                        + "},v: 0,unique: false,dropDups: false,enforced: false}" );
            ...
            BSONObject shardingKeyIndex = ( BSONObject ) JSON.parse(
                    "{name: \"$shard\",key: {sk: 1},v: 0,unique: false,dropDups: false,enforced: false}" );
            indexes.add( idIndex );
            ...
            checkDestGroup( db, 250, "{sk:{$gte:0,$lt:250}}", 250,
                    srcGroupName );
            // 期望有250条符合条件的记录，并且目标组中只有250条记录
            checkDestGroup( db, 250, "{sk:{$gte:250,$lt:500}}", 250,
                    destGroupName );
        ...
        public void exec() throws Exception {
            ...
            try {
                ...
                cl.split( srcGroupName, destGroupName,
                        ( BSONObject ) JSON.parse( "{sk:250}" ), // 切分
