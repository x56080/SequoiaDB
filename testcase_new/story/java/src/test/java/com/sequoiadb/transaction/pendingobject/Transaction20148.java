...
/**
 * @FileName seqDB-20148:事务中插入并更新记录，其他事务插入更新前的记录，回滚事务
 * @Author zhaoyu
 * @Date 2019年11月1日
 */
@Test
public class Transaction20148 extends SdbTestBase {
    ...
    public void test() {
        ...
        try {
            ...
            DBCollection tcl1 = db1.getCollectionSpace(csName).getCollection(clName);
            ...
            for (int i = 0; i < insertNum - 1; i++) {
                tcl1.update("{a:" + i + "}", "{$inc:{a:" + insertNum + "}}", null);
            }

            // 事务中插入记录，唯一索引值与插入记录的值相同
            DBCollection tcl2 = db2.getCollectionSpace(csName).getCollection(clName);
            for (int i = 0; i < insertNum - 1; i++) {
                String record = "{_id:20148" + i + ",a:" + i + ",b:'insert20148'}";
            ...
            TransUtils.queryAndCheck(cl, "{a:1}", "{a:''}", new ArrayList<BSONObject>());
        } finally {
            db1.commit();
            db2.commit();
            db1.close();
