...
public class SubCL101 extends SdbTestBase {
    ...
    private void setUp() {
        ...
        mCL = cs.createCollection( mCLName, ( BSONObject ) JSON
                .parse( "{IsMainCL:true, ShardingKey:{_id:1}}" ) );
        DBCollection sCL = cs.createCollection( sCLName, ( BSONObject ) JSON
    ...
    private void test() {
        ...
        try {
            ...
            srcRecCnt = srcRgDB.getCollectionSpace( csName )
                    .getCollection( sCLName ).getCount();

            trgRgDB = sdb.getReplicaGroup( trgRg ).getMaster().connect();
            trgRecCnt = trgRgDB.getCollectionSpace( csName )
                    .getCollection( sCLName ).getCount();
        } finally {
            if ( srcRgDB != null )
                srcRgDB.disconnect();
