 ...
 * @version 1.00
 */
public class CrudAttachDetachTest10482 extends SdbTestBase {
    ...
    private DBCollection[] subcls = { null, null };
    private String mainclName = "maincl_10482";
    private String[] subclNames = { "maincl_10482_subcl_0",
            "maincl_10482_subcl_1" };
    ...
    private void createCL() {
        ...
        try {
            ...
            subcls[ 0 ] = cs.createCollection( subclNames[ 0 ],
                    ( BSONObject ) JSON.parse( subclOptions[ 0 ] ) );
            subcls[ 1 ] = cs.createCollection( subclNames[ 1 ],
    ...
    private void initData() {
        try {
            ...
            subcls[ 1 ].split( replicaGroupNames.get( 1 ),
                    replicaGroupNames.get( 0 ), 50 );
        } catch ( BaseException e ) {
            e.printStackTrace();
    ...
    class AttachDetachThread extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            DBCollection maincl = sdb_other
                    .getCollectionSpace( SdbTestBase.csName )
                    .getCollection( mainclName );
            for ( int i = 0; i < 30; i++ ) {
                maincl.detachCollection(
                        SdbTestBase.csName + "." + subclNames[ 0 ] );
                maincl.attachCollection(
                        SdbTestBase.csName + "." + subclNames[ 0 ],
                        ( BSONObject ) JSON.parse(
                                "{ LowBound:{a:0},UpBound:{a:100} }" ) );
                maincl.detachCollection(
                        SdbTestBase.csName + "." + subclNames[ 1 ] );
                maincl.attachCollection(
                        SdbTestBase.csName + "." + subclNames[ 1 ],
                        ( BSONObject ) JSON.parse(
                                "{ LowBound:{a:100},UpBound:{a:200} }" ) );
            }
        }
    }
