...
/**
 * @FileName:seqDB-10882: hash分区表指定分区键值为decimal类型，执行切分
 *                        1、创建cl，指定shardingType为“hash”
 *                        2、向cl中插入数据,其中分区键字段值为decimal类型 3、执行split，设置范围切分条件
 *                        4、查看数据切分结果（分别连接coord、源组data、目标组data查询
 * @Author linsuqiang
 * @Date 2017-01-04
 * @Version 1.00
 */
public class TestSplit10882 extends SdbTestBase {
    private Sequoiadb sdb;
    private String clName = "cl_10882";
    ...
    private void initData() {
        ...
        for ( int i = 0; i < recsCnt; i++ ) {
            String dcmlStr = i + "12345.06789123456789012345";
            BSONDecimal decimal = new BSONDecimal( dcmlStr );
