...
function test ()
{
   ...
   cl.insert( docs );
   cl.remove( { $not: [{ score: { $gte: 85, $lte: 96, $ne: 90 } }, { "info.age": { $gt: 25, $lt: 30 } }, { "info.name": { $nin: ["Kate", "Tom"] } }, { "info.sex": { $in: ["男"] } }] } );
   var expectRes = [{ no: 1013, score: 86, interest: ["basketball", "movie", "photo"], major: "电学", dep: "物电学院", info: { name: "Jaden", age: 28, sex: "男" } }]
   var cursor = cl.find();
   commCompareResults( cursor, expectRes );
