...
*@Description: find and update on maincl,update the ShardingKey of subcl,
                        each subcl is in one group;
*@author:      wuyan
*@createdate:  2017.8.22
**************************************/
var csName = CHANGEDPREFIX + "_cs_12159";
var mainCLName = CHANGEDPREFIX + "_mcl_12159";
...
function main ()
{
   ...
   insertData( mainCL, doc );

   //find and update ShardingKey of subcls
   var updateCondition = { $inc: { no: 2 }, $set: { test: "testUpdate" } };
   var findCondition = { "$and": [{ "a": { "$gte": 4 } }, { "a": { "$lte": 29 } }] };
   findAndUpdateData( mainCL, findCondition, updateCondition, true, true );

   //check the update result
   var expRecs = [{ a: -1, no: 12, test: 1 }, { a: 2, no: 13, test: 2 }, { a: 3, no: 14, test: 3 },
   { a: 4, no: 14.36, test: "testUpdate" }, { a: 12, no: 3.23, test: "testUpdate" },
   { a: 6, no: { "$timestamp": "2012-01-01-13.14.26.124233" }, test: "testUpdate" },
   { a: 15, no: "test15", test: "testUpdate" }, { a: 16, no: 14.36, test: "testUpdate" },
   { a: 20, no: 3.23, test: "testUpdate" }, { a: 23, no: "test23", test: "testUpdate" },
   { a: 29, no: { $decimal: "125.456" }, test: "testUpdate" }];
...
function createAndAttachCL ( mainCL )
{
   try
   {
      var subCLName1 = COMMCLNAME + "_subcl_12159_1";
      var subCLName2 = COMMCLNAME + "_subcl_12159_2";
      var subCLName3 = COMMCLNAME + "_subcl_12159_3";
   ...
   {
      throw buildException( "createAndAttachCL()", e, "create/attach subcl",
         "create/attach success", "create/attach fail" );
   }
