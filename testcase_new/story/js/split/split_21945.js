/******************************************************************************
@Description : seqDB-21945:range表，分区键为单字段逆序，带匹配符查询
...
function test ( arg )
{
   ...
   // subCL split to multi group, e.g: [max,200), [200,100), [100,min)
   cl.split( dataGroupNames[0], dataGroupNames[1], { "a": 200 }, { "a": 100 } );
   cl.split( dataGroupNames[0], dataGroupNames[2], { "a": 100 }, { "a": { "$minKey": 1 } } );

   // $gt
   var findCond = { "a": { "$gt": 200 } };
   ...
   var findCond = { "a": { "$in": [100, 101] } };
   commCompareResults( cl.find( findCond ).sort( { "a": 1 } ), docs.slice( 100, 102 ) );
   ...
   // $and, 子集包含
   var findCond = { "$and": [{ "a": { "$gte": 100 } }, { "a": { "$gte": 101 } }] };
   ...
   // $and + other machers
   var findCond = { "$and": [{ "a": { "$gte": 200 } }, { "a": { "$isnull": 0 } }, { "a": { "$exists": 1 } }] };
   commCompareResults( cl.find( findCond ).sort( { "a": 1 } ), docs.slice( 200 ) );
   checkHitDataGroups( cl.find( findCond ).explain( { "Run": true } ), [dataGroupNames[0], dataGroupNames[1]] );
   ...
   // $or
   var findCond = { "$or": [{ "a": { "$lte": 100 } }, { "a": { "$gt": 200 } }] };
   commCompareResults( cl.find( findCond ).sort( { "a": 1 } ), docs.slice( 0, 101 ).concat( docs.slice( 201 ) ) );
   checkHitDataGroups( cl.find( findCond ).explain( { "Run": true } ), [dataGroupNames[0], dataGroupNames[2]] );

   var findCond = { "$or": [{ "a": { "$lte": 100 } }, { "a": { "$gte": 201 } }] };
   commCompareResults( cl.find( findCond ).sort( { "a": 1 } ), docs.slice( 0, 101 ).concat( docs.slice( 201 ) ) );
   checkHitDataGroups( cl.find( findCond ).explain( { "Run": true } ), [dataGroupNames[0], dataGroupNames[2]] );
   ...
   var findCond = {
      "$or": [
         { "$and": [{ "a": { "$gt": 0 } }, { "a": { "$lte": 100 } }] },
         { "$and": [{ "a": { "$gt": 200 } }, { "a": { "$lt": 300 } }] }
      ]
   };
   commCompareResults( cl.find( findCond ).sort( { "a": 1 } ), docs.slice( 1, 101 ).concat( docs.slice( 201, 300 ) ) );
   checkHitDataGroups( cl.find( findCond ).explain( { "Run": true } ), [dataGroupNames[0], dataGroupNames[2]] );
   ...
   commCompareResults( cl.find( findCond ).sort( { "a": 1 } ), docs.slice( 0, 1 ).concat( docs.slice( 101, 201 ) ) );
   checkHitDataGroups( cl.find( findCond ).explain( { "Run": true } ), dataGroupNames );
