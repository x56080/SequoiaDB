/******************************************************************************
 * @Description   : seqDB-34122:子表数据量差异较小，查看索引统计信息
 * @Author        : chenzejia
...
function test ()
{
   var mainCLName = "maincl_34122";
   var subCLNamePrefix = "subcl_34122_";
   var indexName = "idx_34122";
   var subTableNum = 5;
   var sampleNum = 180;
   var subClRecordsArray = [17000, 20000, 18000, 2000, 19000];
   ...
   // 此时中位采样率为SampleNum*数据组数/18000
   var dataGroupNames = commGetDataGroupNames( db );
   var sampleRecords = dataGroupNames.length * sampleNum * ( subTableNum - 2 ) + ( ( dataGroupNames.length * sampleNum ) / sortSubClRecordsArray[2] ) * ( sortSubClRecordsArray[sortSubClRecordsArray.length - 1] + sortSubClRecordsArray[sortSubClRecordsArray.length - 2] );
   var totalRecords = 0;
   for( var i = 0; i < subClRecordsArray.length; i++ )
   {
