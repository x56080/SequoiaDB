/******************************************************************************
 * @Description   : seqDB-34137:存在少量重复值，样本数超过阈值，查看索引统计信息
 * @Author        : chenzejia
...
function test ()
{
   var mainCLName = "maincl_34137";
   var subCLNamePrefix = "subcl_34137_";
   var subTableNum = 5;
   var indexName = "idx_34137";
   var totalRecords = 20000;
   var sampleNum = 5000;
   var statmcvlimit = 2500;
   var perSubRecords = totalRecords / subTableNum;
   var distinctValNumArray = [];
   for( var i = 0; i < 2000; i++ )
   {
      distinctValNumArray.push( i );
   }
   ...
   try
   {
      ...
      var distinctValNum = actIndexStat.DistinctValNum;
      ...
      delete ( actIndexStat.DistinctValNum );
      delete ( actIndexStat.MCV );

      // 校验MCV字段
      assert.equal( values.length, distinctValNum[0] );
      assert.equal( frac.length, distinctValNum[0] );
