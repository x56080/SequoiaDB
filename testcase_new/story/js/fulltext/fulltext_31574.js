/******************************************************************************
 * @Description   : seqDB-31574 :: 创建全文索引指定映射为wildcard类型，插入索引字段类型为兼容转换类型
 * @Author        : wu yan 
...
testConf.clName = COMMCLNAME + "_es_31574";
testConf.clOpt = { ShardingKey: { tlong: 1 }, ShardingType: "hash", AutoSplit: true };
...
function test ( testPara )
{
   ...
   // 创建全文索引前插入数据
   var doc = [{ no: 1, tlong: 3000000000, tstr: "2041-03-04", tdouble: -1.7E+308, tbool: true, tdate: { "$date": "2012-01-01" }, ttime: { "$timestamp": "2012-01-01-13.14.26.124233" }, tarr: ["test1", "test2", "arr!0"] },
   ...
   dbcl.createIndex( indexName, {
      ...
      "Mappings": {
         "Fields": {
            "no": { "Type": "wildcard" }, "tlong": { "Type": "wildcard" }, "tstr": { "Type": "wildcard" },
            "tdouble": { "Type": "wildcard" }, "tbool": { "Type": "wildcard" }, "tdate": { "Type": "wildcard" }, "ttime": { "Type": "wildcard" },
            "tarr": { "Type": "wildcard" }
         }
      }
   } );
   ...
   var actRecords2 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "wildcard": { "tstr": "test*" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   ...
   //检索long类型数据
   var actRecords4 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "wildcard": { "tlong": "*9223*" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords4 = dbOpr.findFromCL( dbcl, { "$or": [{ "tlong": { "$numberLong": "-9223372036854775808" } }, { "tlong": { "$numberLong": "9223372036854775807" } }] }, { _id: { "$include": 0 } }, { _id: 1 } );
   checkResult( expectRecords4, actRecords4 );

   //检索double类型数据
   var actRecords5 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "wildcard": { "tdouble": "5*" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords5 = dbOpr.findFromCL( dbcl, { "tdouble": 5000.46 }, { _id: { "$include": 0 } }, { _id: 1 } );
   ...
   //检索date类型数据
   var actRecords7 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "wildcard": { "tdate": "0000-01-01" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords7 = dbOpr.findFromCL( dbcl, { "tdate": { "$date": "0000-01-01" } }, { _id: { "$include": 0 } }, { _id: 1 } );
   checkResult( expectRecords7, actRecords7 );

   //检索time类型数据
   var actRecords8 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "wildcard": { "ttime": "1902-01-01*" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords8 = dbOpr.findFromCL( dbcl, { "ttime": { "$lte": { "$timestamp": "1902-01-02-00.00.00.000000" } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   checkResult( expectRecords8, actRecords8 );

   //检索array类型数据
   var actRecords9 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "wildcard": { "tarr": "arr!0" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords9 = dbOpr.findFromCL( dbcl, { "tarr": { "$elemMatch": { "$et": "arr!0" } } }, { _id: { "$include": 0 } }, { _id: 1 } );
