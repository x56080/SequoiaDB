/******************************************************************************
 * @Description   : seqDB-31573 ::  创建全文索引指定映射为keyword类型，插入索引字段类型为不兼容转换类型
 * @Author        : wu yan 
...
testConf.clName = COMMCLNAME + "_es_31573";
...
function test ( testPara )
{
   ...
   var indexName = "index_31573";
   dbcl.createIndex( indexName, { tobj: "text", tarr: "text" }, { "Mappings": { "Fields": { "tobj": { "Type": "keyword" }, "tarr": { "Type": "keyword" } } } } );
   ...
   //检索object类型数据  
   var actRecords2 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "match": { "tobj.city": "shanghai" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords2 = [];
   checkResult( expectRecords2, actRecords2 );

   //检索array类型数据
   var actRecords3 = dbOpr.findFromCL( dbcl, { "": { "$Text": { "query": { "match": { "tarr": "test1" } } } } }, { _id: { "$include": 0 } }, { _id: 1 } );
   var expectRecords3 = [];
