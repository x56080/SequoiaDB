...
function main ()
{
   ...
   //指定$shard索引生成默认统计信息
   analyze( db, { Collection: mainclFullName, Mode: 3, Index: "$shard" } );
   ...
   println( "---check all explain after anlyze shard index set mode=3 success" );
   ...
   //统计信息加载至缓存
   analyze( db, { Collection: mainclFullName, Mode: 4, Index: "$shard" } );
   ...
   println( "---check all explain after anlyze shard index set mode=4 success" );
   ...
   //清空统计信息
   analyze( db, { Collection: mainclFullName, Mode: 5, Index: "$shard" } );
   ...
   //主备节点上检查访问计划
   checkExplainAnalyzeShardIndex();
   println( "---check all explain after anlyze shard index set mode=5 success" );
   ...
   //指定普通索引生成默认统计信息
   analyze( db, { Collection: mainclFullName, Mode: 3, Index: "a1" } );
   ...
   println( "---check all explain after anlyze common index set mode=3 success" );
   ...
   //统计信息加载至缓存
   analyze( db, { Collection: mainclFullName, Mode: 4, Index: "a1" } );
   ...
   println( "---check all explain after analyze common index set mode=4 success" );
   ...
   //清空统计信息
   analyze( db, { Collection: mainclFullName, Mode: 5, Index: "a1" } );
   ...
   //主备节点上检查访问计划
   checkExplainAnalyzeCommonIndex();
   println( "---check all explain after anlyze common index set mode=5 success" );
