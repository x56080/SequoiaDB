...
function main ()
{
   ...
   println( "---check all explain after analyze maincl set mode=3 success" );
   ...
   //统计信息加载至缓存
   analyze( db, { CollectionSpace: maincsName, Mode: 4 } );
   ...
   println( "---check all explain after modify stat and anlyze maincs set mode=4 success" );
   ...
   //主备节点上检查访问计划
   checkExplainAfterAnalyzeMaincs();
   println( "---check result after update maincs stat but no analyze success!" );

   //清空统计信息
   analyze( db, { CollectionSpace: maincsName, Mode: 5 } );
   ...
   //主备节点上检查访问计划
   checkExplainBeforeAnalyze();
   println( "---check result after update stat and analyze maincs mode set 5 success" );
