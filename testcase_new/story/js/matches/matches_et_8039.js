/************************************************************************
*@Description:   seqDB-8039:使用$et查询，目标字段为数组且元素为数值型，走索引查询
...
{  
   try
   {
      var clName = COMMCLNAME+"_matches8039" ;
...
function insertRecs( cl, rawData )
{
   ...
   cl.insert({a:0, b:[{b1: rawData[0]["int"][0]}, 
                      {b2: rawData[0]["int"][1]}]});
   cl.insert({a:1, b: {b1:{c1:{$numberLong: rawData[1]["long"][0]}}, 
...
function findRecs( cl, rawData, dataType )
{
   ...
   {
      var rc = cl.find( {b:{$et:{b1:{c1:{$numberLong: rawData[1]["long"][0]}}, 
                                 b2:{c2:{$numberLong: rawData[1]["long"][1]}}}}} ).sort({a:1});
   }
...
function checkResult( arrRc, objRc, rawData, indexName )
{
   ...
   {
      println( "---The real results after the find by matches[$et]: \n"+ JSON.stringify( findRtn ) );
