/************************************************************************
*@Description:   seqDB-8073:使用$ne查询，目标字段为数组且元素为数值型，不走索引查询
...
function main ()
{
   try
   {
      var clName = COMMCLNAME + "_matches8073";
...
function findRecs ( cl, rawData, dataType )
{
   ...
   {
      var rc = cl.find( {
         "b.b1": { $ne: rawData[0]["int"][0] },
         "b.b2": { $ne: rawData[0]["int"][1] }
      } ).sort( { a: 1 } );
   }
   else if( dataType === "object" )
   {
      var rc = cl.find( {
         "b.b1.c1": { $ne: { $numberLong: rawData[1]["long"][0] } },
         "b.b2.c2": { $ne: { $numberLong: rawData[1]["long"][1] } }
      } ).sort( { a: 1 } );
   }
