/******************************************************************************
@Description : 1.query use Operator $lte with index
@Modify list :
...
//no index
//select * from ... where a<=9 and b=90
try
{
	var sel = varCL.find( { a: { $lte: 9 }, b: 90 } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
...
try
{
	var sel = varCL.find( { a: { $lte: 6 }, b: { $gt: 50 } } ).sort( { a: 1 } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
	{
		...
		var ret = sel.current();
		//expected result:{a:6,b:60}
		if( ret.toObj()['a'] != 6 || ret.toObj()['b'] != 60 )
		{
			flag = false;
			throw 2;
		}
...
try
{
	var sel = varCL.find( { a: { $lte: 0 }, b: { $lte: 10 } } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
