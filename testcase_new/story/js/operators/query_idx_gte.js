/******************************************************************************
@Description : 1.query use Operator $gte with index
@Modify list :
...
//select * from ... where a>=6 and b>=60
try
{
	var sel = varCL.find( { a: { $gte: 6 }, b: { $gte: 60 } } ).sort( { a: 1 } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 4;
	var flag = true;
	while( sel.next() )
	{
		...
		var ret = sel.current();
		//expected result:{a:6,b:60} {a:7,b:70} {a:8,b:80} {a:9,b:90}
		if( ret.toObj()['a'] != ( size + 5 ) || ret.toObj()['b'] != ( ( size + 5 ) * 10 ) )
		{
			flag = false;
			throw 2;
		}
...
try
{
	var sel = varCL.find( { a: { $gte: 0 }, b: { $lte: 10 } } ).sort( { b: 1 } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 2;
	var flag = true;
	while( sel.next() )
