/******************************************************************************
@Description : 1.query use Operator $isnull with index
@Modify list :
...
try
{
	var sel = varCL.find( { a: { $isnull: 0 }, b: { $lt: 20 } } ).sort( { b: 1 } );
	var size = 0;
	var exprows = 2;
	var flag = true;
	while( sel.next() )
...
try
{
	var sel = varCL.find( { a: { $isnull: 1 }, b: 10 } ).sort( { a: 1 } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
...
try
{
	var sel = varCL.find( { a: { $isnull: 1 }, b: { $isnull: 1 } } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
