/******************************************************************************
@Description : 1.query use Operator $and with index
@Modify list :
...
//no index
//select * from ... where a>1 and a<4
try
{
	var sel = varCL.find( { $and: [{ a: { $gt: 1 } }, { a: { $lt: 4 } }] } ).sort( { a: 1 } );
	var size = 0;
	var exprows = 2;
	var flag = true;
	while( sel.next() )
	{
		...
		var ret = sel.current();
		//expected result:{a:2,b:20} {a:3,b:30}
...
//select * from ... where a>3 and a in(4,5) and b<50 
try
{
	var sel = varCL.find( { $and: [{ a: { $gt: 3 } }, { a: { $in: [4, 5] } }], b: { $lt: 50 } } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
...
//select * from ... where a<=60 and b=20;
try
{
	var sel = varCL.find( { $and: [{ a: { $lt: 60 } }, { b: 20 }] } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
