/******************************************************************************
@Description : 1.query use Operator $ne with index
@Modify list :
...
try
{
	var sel = varCL.find( { a: { $ne: 1 }, b: 20 } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
...
//select * from ... where b!=10 and a!=0
try
{
	var sel = varCL.find( { b: { $ne: 10 }, a: { $ne: 0 } } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
...
try
{
	var sel = varCL.find( { a: { $ne: 0 }, b: { $lte: 10 } } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
