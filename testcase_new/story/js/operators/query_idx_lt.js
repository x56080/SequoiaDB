/******************************************************************************
@Description : 1.query use Operator $lt with index
@Modify list :
...
//no index
//select * from ... where a<9 and b=0
try
{
	var sel = varCL.find( { a: { $lt: 9 }, b: 0 } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
...
} catch( e )
{
	...
	{
		println( "return incorrect record! expected:{a:0, b:0} returned:" + ret );
		throw e;
	}
}
...
//select * from ... where a<6 and b>=50
try
{
	var sel = varCL.find( { a: { $lt: 6 }, b: { $gte: 50 } } ).sort( { a: 1 } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
	{
		...
		var ret = sel.current();
		//expected result:{a:5,b:50}
		if( ret.toObj()['a'] != 5 || ret.toObj()['b'] != 50 )
		{
			flag = false;
			throw 2;
		}
...
//select * from ... where a<1 and b<=10
try
{
	var sel = varCL.find( { a: { $lt: 1 }, b: { $lte: 10 } } ).sort( { b: 1 } ).hint( { "": CHANGEDPREFIX_IDX } );
	var size = 0;
	var exprows = 1;
	var flag = true;
	while( sel.next() )
