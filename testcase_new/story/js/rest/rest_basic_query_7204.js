...
function queryAndCheck ()
{
	var word = "query";
	tryCatch(
		["cmd=" + word, "name=" + csName + '.' + clName, 'sort={"age":1}'],
		[0], "queryAndCheck: fail to run rest command: " + word );
	...
	try
	{
		while( rc.next() )
		{
			if( rc.current().toObj()["name"] !== JSON.parse( arr[i] ).name )
			{
				...
				println( "the " + i + " record, cl.find().current().name: " + rc.current().toObj()["name"] +
					", rest return: " + JSON.parse( arr[i] ).name );
