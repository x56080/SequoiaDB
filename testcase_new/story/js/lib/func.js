...
func.js 中方法：
   ...
   2、创建
      ...
      创建并返回 schema      commCreateSchema ( db, schemaName, schemaDef, schemaAttr, ignoreExisted )
   ...
   3、删除
      ...
      删除 schema            commDropSchema ( db, schemaName, ignoreNotExist )  
...
var COMMSCHEMANAME = CHANGEDPREFIX + "_schema";
...
/******************************************************************************
@description  创建并返回 schema 对象
@author liuli
@parameter
   schemaDef      {object}   :  必填参数，创建 schema 指定的详细信息
   schemaAttr     {object}   :  默认值为{}，创建 schema 指定的属性
   ignoreExisted  {boolean}  :  默认为 false，重复创建 schema 报错
@return  {object}  创建或者获取的 schema 对象
***************************************************************************** */
function commCreateSchema ( db, schemaName, schemaDef, schemaAttr, ignoreExisted )
{
   if( schemaDef == undefined )
   {
      throw new Error( "schemaDef is a required parameter" );
   }
   if( schemaAttr == undefined ) { schemaAttr = {}; }
   if( ignoreExisted == undefined ) { ignoreExisted = false; }
   commCheckType( schemaDef, "object" );
   commCheckType( schemaAttr, "object" );
   commCheckType( ignoreExisted, "boolean" );

   // create schema
   try
   {
      return db.createSchema( schemaName, schemaDef, schemaAttr );
   }
   catch( e )
   {
      if( commCompareErrorCode( e, SDB_SCHEMA_EXIST ) && ignoreExisted )
      {
         // think right
      ...
      {
         commThrowError( e, "create schema [" + schemaName + "] failed: " + e );
      }
   }
}
...
@author liuli
@parameter
   ignoreNotExist {boolean}  :  默认为 true，要删除的 schema 不存在时不报错
***************************************************************************** */
function commDropSchema ( db, schemaName, ignoreNotExist )
{
   if( ignoreNotExist == undefined ) { ignoreNotExist = true; }
   commCheckType( ignoreNotExist, "boolean" );
   ...
   {
      if( commCompareErrorCode( e, SDB_SCHEMA_NOT_EXIST ) && ignoreNotExist )
      {
         // think right
      }
      else
      {
         commThrowError( e, "drop schema [" + schemaName + "] failed: " + e )
      }
   }
}
