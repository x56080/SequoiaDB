...
function test( testPara )
{
   ...
   testPara.testCL.update( { "$pull_by": { "a": { "$field": "b.field" } } } );
   ...
   expResult = [ { "a": [], "b": { "field": { "key1": "value1" } } } ];
   ...
   for( var i = 0; i < allTypeData.length; i++ )
   {
      ...
      testPara.testCL.update( { "$pull_by": { "a": { "$field": "b" } } }, { "c": i } );
