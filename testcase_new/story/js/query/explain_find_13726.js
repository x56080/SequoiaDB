...
{	  
	...
	{        
      ...
      var expOpt = {ScanType:"ixscan", IndexName:sdIdxName, Query:{"$and":[{"sk":{"$gt":-1}}]}, 
                    groupList:groupList, subclList:subclList};
      checkExplain( rc, expOpt );
      
      println("---begin to: cl.find({mk:{$gt:101}}).explain()");
      var rc = cl.find({mk:{$gt:101}}).explain();
      var expOpt = {ScanType:"tbscan", IndexName:"", Query:{"$and":[{"mk":{"$gt":101}}]}, 
      ...
      var expOpt = {ScanType:"ixscan", IndexName:sdIdxName, Query:{"$and":[]}, 
                    groupList:groupList, subclList:subclList};
      checkExplain( rc, expOpt ); 
      ...
      var expOpt = {ScanType:"ixscan", IndexName:indexName, Query:{"$and":[{"b":{"$gt":1}}]},
                    groupList:groupList, subclList:subclList};
      checkExplain( rc, expOpt );
      
      println("---begin to: cl.find().sort({b:1}).explain()");
      var rc = cl.find().sort({b:1}).explain();
      var expOpt = {ScanType:"ixscan", IndexName:indexName, Query:{"$and":[]},
                    groupList:groupList, subclList:subclList};
      checkExplain( rc, expOpt );  
