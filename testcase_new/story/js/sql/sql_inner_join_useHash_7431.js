/****************************************************
@description:	inner join with /*+use_hash()*/
/*        testlink cases:   seqDB-7431
@input:        1 insert into records
               2 select with [inner join, use_hash, order by g.Cno]
/*
...
println( "------Begin to exec select with [inner join, use_hash, order by g.Cno]." );
try
{
	var cur = db.exec( "select s.Sname,s.Sdept,g.Cno,g.Grade from " + csName + "." + clName1 + " as s" + " inner join " + csName + "." + clName2 + " as g" + " on s.Sno=g.Sno order by g.Cno /*+use_hash()*/" );
}
catch( e )
{
