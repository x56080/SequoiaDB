/****************************************************
@description:	left join, basic case
         testlink cases:   seqDB-7432
@input:        1 insert into records
               2 select with [left outer join, order by A.id]
               3 select with [left outer join, order by A.id desc]
...
println( "------Begin to exec select with [left outer join, order by A.id]." );
try
{
   var res = db.exec( "select A.name , B.user , A.id from " + csName + "." + clName1 + " as A left outer join " + csName + "." + clName2 + " as B on A.id=B.id order by A.id asc" );
}
catch( e )
{
...
println( "------Begin to exec select with [left outer join, order by A.id desc]." );
try
{
   var res = db.exec( "select A.name , B.user , B.id from " + csName + "." + clName1 + " as A left outer join " + csName + "." + clName2 + " as B on A.id=B.id order by A.id desc" );
}
catch( e )
{
   println( "Failed to exec select." );
...
if( value["name"] !== "A100" )
{
   throw "Failed to compare the values.";
}
var number = 1;
while( res.next() )
{
