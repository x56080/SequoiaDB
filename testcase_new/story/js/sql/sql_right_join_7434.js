/****************************************************
@description:	right join, basic case
         testlink cases:   seqDB-7434
@input:        1 insert into records
               2 select with [right outer join, order by A.id]
               3 select with [right outer join, order by A.id desc]
...
try
{
   var res = db.exec( "select A.name , B.user , A.id from " + csName + "." + clName1 + " as A right outer join " + csName + "." + clName2 + " as B on A.id=B.id order by A.id asc" );
}
catch( e )
{
...
println( "------Begin to exec select with [right outer join, order by A.id]." );
var number = 0;
while( res.next() )
{
   number++;
...
println( "------Begin to exec select with [right outer join, order by A.id desc]." );
try
{
   var res = db.exec( "select A.name , B.user , B.id from " + csName + "." + clName1 + " as A right outer join " + csName + "." + clName2 + " as B on A.id=B.id order by B.id desc" );
}
catch( e )
{
   println( "Failed to exec select." );
