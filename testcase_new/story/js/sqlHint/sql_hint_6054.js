...
function selectRecs ( csName, clName1, clName2, clName3, idxName )
{
   ...
   var rc = db.exec( "select t1.a, t2.cnt from " + csName + "." + clName1 + " as t1 inner join ( select t3.b, t3.cnt, t4.c from ( select count(a) as cnt, b from " + csName + "." + clName2 + " group by b ) as t3 left outer join " + csName + "." + clName3 + " as t4 on t3.b = t4.c /*+use_index(t4, " + idxName + ") use_hash()*/ ) as t2 on t1.a = t2.b /*+use_index(t1, " + idxName + ") use_hash()*/" );
...
function checkResult ( rtRecsArray, preTotalIndexRead, aftTotalIndexRead )
{
   ...
   //println( JSON.stringify( rtRecsArray ) );
