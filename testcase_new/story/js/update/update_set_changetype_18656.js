...
function checkRecords ( cl )
{
    ...
    {
        throw buildException( "checkRecords", null, "check count", expRecs.length, actRecs.length );
    }

    if( JSON.stringify( actRecs ) !== JSON.stringify( expRecs ) )
    {
        throw buildException( "checkRecords", null, "check records", JSON.stringify( expRecs ), JSON.stringify( actRecs ) );
    }
}