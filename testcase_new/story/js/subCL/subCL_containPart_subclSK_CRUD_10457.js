...
function main ()
{
	...
	try 
	{
		if( mainCl.find( { b: 15 } ).count() != query_B15.length ) 
		{
			throw new Error();
		}
	}
	...
	{
		throw buildException( "failed to query test", e, "query by condition b = 15", "query result sum equals", "query result sum not equals" );
	}
	...
	try
	{
		...
		{
			if( cursor.current().toObj().name !== "update_name" )
			{
				throw new Error();
			}
		}
	...
	{
		throw buildException( "failed to update test", e, "update by condition c=5 set name='update_name'", "update all c=5 record success", "update all c=5 record failed" )
	}
	...
	{
		throw buildException( "failed to delete test", e, "delete by condition b=13", "delete all b=13 record success", "delete all a=13 record failed" );
	}
