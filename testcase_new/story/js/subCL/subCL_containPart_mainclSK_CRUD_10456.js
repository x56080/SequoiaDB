...
function main ()
{
	...
	try 
	{
		if( mainCl.find( { a: 5 } ).count() != query_A5.length ) 
		{
			throw new Error();
		}
	}
	...
	{
		throw buildException( "failed to query test", e, "query by condition a = 5", "query result sum equals", "query result sum not equals" );
	}
	...
	{
		throw buildException( "failed to update test", e, "update by condition b=15 set name='update_name'", "update all b=15 record success", "update all b=15 record failed" )
	}
	...
	{
		throw buildException( "failed to delete test", e, "delete by condition a=10", "delete all a=10 record success", "delete all a=10 record failed" )
	}
