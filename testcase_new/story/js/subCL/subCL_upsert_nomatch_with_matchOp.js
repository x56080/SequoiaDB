...
function getOtherDataGroups ( db, SourceGroupName )
{
...
function test_range_attach_hash_upsert_basic ( db )// NOT Error, test mainCL'ShardingType is range and subCL's ShardingType is hash , insert's result
{
	...
	try
	{
		...
		for( var i = 0; i < numberOfsubCl; ++i )
		{
			...
			var desDataGroupName = getOtherDataGroups( db, sourceDataGroupName );
	...
	try
	{
		mainCL.upsert( { $set: { "localtion.street": "人民路12号" } }, { mineTime: { $et: "2013-06-14" } } );
	}
	catch( e )
	{
	...
	try
	{
		rc = mainCL.find( { localtion: { $elemMatch: { street: "人民路12号" } } } );
	}
	catch( e )
	{
...
function main ()
{
	...
	test_range_attach_hash_upsert_basic( db );
