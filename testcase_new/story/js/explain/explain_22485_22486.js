...
function main()
{
   ...
   var clName = CHANGEDPREFIX + "_22485_22486";
   commDropCL( db, COMMCSNAME, clName );
   var cl = commCreateCL( db, COMMCSNAME, clName );
   cl.createIndex( indexName, { "a": 1 }, false );

   var expIXBound = { "a": [[{ "$minElement": 1 }, { "$maxElement": 1 }]]};
   var cond = { "a": { "$regex": "^rg", "$options": "i" } };
   var ixBound = cl.find( cond ).hint( { "": indexName } ).explain().current().toObj().IXBound;
   ...
   ixBound = cl.find( cond ).hint( { "": indexName } ).explain().current().toObj().IXBound;
   if( !commCompareObject( expIXBound, ixBound ) )
   {
      throw new Error( "expIXBound: " + JSON.stringify( expIXBound ) + ", ixBound: " + JSON.stringify( ixBound ) );
   } 
