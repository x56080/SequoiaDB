...
function test ()
{
   ...
   {
      checkSnapshot( nodeConn, cpuExpect, null, hostName );
   } else
   {
      checkSnapshot( nodeConn, cpuExpect, memExpect, hostName );
   }
   remoteObj.close();
   nodeConn.close();
}

function checkSnapshot ( nodeConn, cpuExpect, memExpect, hostName )
{
   // 编目节点非聚合快照
   var cursor = nodeConn.snapshot( SDB_SNAP_SYSTEM, { RawData: true, HostName: hostName }, { CPU: null, Memory: null, HostName: null } );
   // 编目节点聚合快照
   var cursorPol = nodeConn.snapshot( SDB_SNAP_SYSTEM, { HostName: hostName }, { CPU: null, Memory: null, HostName: null } );
   while( cursor.next() )
   {
