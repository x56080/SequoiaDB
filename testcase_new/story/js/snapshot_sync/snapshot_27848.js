...
testConf.clName = COMMCLNAME + "_27848";
...
{
   var filePath = WORKDIR + "/lob27848/";
   var fileName = "filelob_27848";
   var fileSize = 1024 * 2;
   ...
   csInfo[0]["TotalUsedLobSpace"] += 262144 * lobPages * nodeNum;
   ...
   // 计算结果为0.000005086263020833333，空间使用率显示为0
   ...
   for( var i in csInfoRawData )
   {
      ...
      csInfoRawData[i]["TotalUsedLobSpace"] += 262144 * lobPages;
      ...
      csInfoRawData[i]["LobUsageRate"] = 0;
   ...
   for( var i in clInfo )
   {
      ...
      clInfo[i]["TotalUsedLobSpace"] += 262144 * lobPages;
   ...
   for( var i in clInfoRawData )
   {
      ...
      clInfoRawData[i]["TotalUsedLobSpace"] += 262144 * lobPages;
      clInfoRawData[i]["LobUsageRate"] = 0;
   ...
   clDetail[0]["TotalUsedLobSpace"] += 262144 * lobPages;
   ...
   // 元数据寻址此时增加数量等于大对象页增加数量
   dataBastInfo[0]["TotalLobAddressing"] += lobPages * 2;
   dataBastInfo[0]["TotalLobRead"] += lobPages + 1;
   dataBastInfo[0]["TotalLobTruncate"] += lobPages;
   var cursor = sdb.snapshot( SDB_SNAP_DATABASE );
   checkSnapshot( cursor, dataBastInfo, lobPages * 2 );
   ...
   for( var i in dataBastInfoRawData )
   {
      ...
      if( dataBastInfoRawData[i]["NodeName"] == masterNodeName )
      {
         ...
         dataBastInfoRawData[i]["TotalLobAddressing"] += lobPages * 2;
      }
      else if( dataBastInfoRawData[i]["NodeName"] == coordUrl[0] )
      {
         // coord节点会统计部分流量指标
   ...
   var option = new SdbSnapshotOption().cond( { RawData: true } ).sort( { NodeName: 1 } );
   var cursor = sdb.snapshot( SDB_SNAP_DATABASE, option );
   checkSnapshot( cursor, dataBastInfoRawData, lobPages * 2 );

   // 查看集合空间快照并校验结果，聚合结果
   csInfo[0]["TotalLobDelete"] += 1;
   csInfo[0]["TotalLobAddressing"] += lobPages * 2;
   ...
   var cursor = sdb.snapshot( SDB_SNAP_COLLECTIONSPACES, { Name: testConf.csName } );
   checkSnapshot( cursor, csInfo, lobPages * 2 );
   ...
   for( var i in csInfoRawData )
   {
      if( csInfoRawData[i]["NodeName"] == masterNodeName )
      {
         ...
         csInfoRawData[i]["TotalLobAddressing"] += lobPages * 2;
         // 查看集合空间快照并校验结果，聚合结果
   ...
   var option = new SdbSnapshotOption().cond( { Name: testConf.csName, RawData: true } ).sort( { NodeName: 1 } );
   var cursor = sdb.snapshot( SDB_SNAP_COLLECTIONSPACES, option );
   checkSnapshot( cursor, csInfoRawData, lobPages, lobPages * 2 );
   ...
   for( var i in clInfo )
   {
      if( clInfo[i]["NodeName"] == masterNodeName )
      {
         ...
         clInfo[i]["TotalLobAddressing"] += lobPages * 2;
   ...
   for( var i in clInfoRawData )
   {
      if( clInfoRawData[i]["NodeName"] == masterNodeName )
      {
         ...
         clInfoRawData[i]["TotalLobAddressing"] += lobPages * 2;
   ...
   clDetail[0]["TotalLobAddressing"] += lobPages * 2;
   ...
   var cursor = dbcl.getDetail();
   checkSnapshotToCL( cursor, clDetail, true, lobPages * 2 );
