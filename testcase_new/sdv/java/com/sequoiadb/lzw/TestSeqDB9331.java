...
public class TestSeqDB9331 extends SdbTestBase{
    ...
    /**
     * 1、插入大量数据（>100万以达到构建字典条件），记录字段类型为decimal，覆盖如下场景：
     *   1）decimal默认精度，如{ "key" : { $decimal:"123.456" } }
     *   2）decimal总位数为1000，小数位为999位
    ...
    private void checkData() {
        try {
            long count = this.cl.getCount("{$and:[{_id:{$gte:0}},{_id:{$lt:112}}]}");
