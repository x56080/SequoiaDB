...
/**
 * @FileName:seqDB-6647:已存在压缩的记录，删除CL后创建同名CL
 * 1、删除已存在压缩数据的CL 
 * 2、检查返回结果 
...
public class TestLzw6647 extends SdbTestBase {
    ...
    private DBCollection createCL(){
        ...
        try{
            dataGroupName = ((ArrayList<String>)getDataGroups(sdb)).get(0);
    ...
    private void checkDropped(){
        ...
        for(int i = 0; i < tryTimes; i++){
            // check on data node
            String url = sdb.getReplicaGroup(dataGroupName).getMaster().getNodeName();
            Sequoiadb dataDB = new Sequoiadb(url, "", "");
            DBCursor dataSnapshot = dataDB.getSnapshot(4, clNameOpt, null, null);
