...
public class DBCollectionTest extends SdbTestBase {
	...
	public void connectSdb(){
		/*String []tmp = coordUrl.split("/");
		if (2 != tmp.length){
			System.out.printf("input coordUrl: %s, request [host:port/csprefix.clprefix]");
			Assert.assertTrue(false);
		}
		
		String connStr = tmp[0];
		String[] names = tmp[1].split("\\.");
		if (2 == names.length){
			csName = names[0] + Thread.currentThread().getId();
			clName = names[1] + Thread.currentThread().getId();
		}else{
			csName = names[0] + Thread.currentThread().getId();
			clName = names[0] + Thread.currentThread().getId();
		}*/
		...
		try{
			...
			System.out.printf("get|crate CollectionSpace %s failed, errMsg:%s\n", SdbTestBase.csName, e.getMessage());
			Assert.assertTrue(false);
		}
		...
		try{
			if (cs.isCollectionExist(clName)){
				...
				cl = cs.createCollection(clName,(BSONObject)JSON.parse("{ReplSize:0}"));
			}
		}catch(BaseException e){
	...
	public void testBasicOper(BSONObject doc){
		try{
			...
			//System.out.printf("insert(%s) failed, errMsg:%s\n",doc.toString(),e.getMessage());
		...
		try{
			...
			while(cr.hasNext()){
				...
				Assert.assertTrue(obj.equals(doc), obj.toString() + "doc:" + doc.toString());
		...
		try{
			...
			while(cr.hasNext()){
				...
				if (subobj instanceof Integer){
					Assert.assertEquals((Integer)subobj + 1, obj.getInt("random"));
					...
					arr.put("0", 0);
					arr.put("1", 1);
					Assert.assertTrue(obj.get("addField").equals(arr), arr.toString());
				}else{
