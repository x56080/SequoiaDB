...
public class Sdv8470 extends SdbConfTestBase {
    ...
    /**
     * 1、3组3节点的集群，开启参数 --signalinterval 1的配置（所有节点），重启节点使配置生效 2、7个并发同时执行：连接sdb ->
     * 插入数据 -> 关闭连接，如： for(i=0;i<100000;i++){db = new
     * Sdb("localhost:11810");db.hxn.hxn.insert({a:1});db.close();} 3、重试多次执行
     */
    @Test
    public void test() {
