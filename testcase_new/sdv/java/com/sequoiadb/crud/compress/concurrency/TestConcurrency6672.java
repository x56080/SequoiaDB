...
/**
 * @FileName:seqDB-6672:并发做增删改查操作
 * 1、CL压缩类型为lzw，并发对该CL做增删改查操作 
 * 2、检查返回结果 
 * @Author linsuqiang
 * @Date 2016-12-22
 * @Version 1.00
...
public class TestConcurrency6672 extends SdbTestBase {
    ...
    /**
     * before test, 9000 records were ready and it's range is [0,9000)
     * range [0   ,3000 ) is to delete
     * range [3000,6000 ) is to update
     * range [6000,9000 ) is to query
     * range [9000,12000) will be added by doing insert
     */
    @Test
    public void test(){
        ...
        if(!(deleteThread.isSuccess() 
                && updateThread.isSuccess() 
                && queryThread.isSuccess() 
                && insertThread.isSuccess())){
        ...
        public void exec() throws BaseException{
            ...
            try{
                ...
                for(int i = 0; i < 3000; i++){
                    ...
                    updModifier.put("$set", (BSONObject)JSON.parse("{b:'hahahahaha'}"));
    ...
    private void checkUpdated(){
        ...
        // check count
        BSONObject updMatcher = new BasicBSONObject();
        updMatcher = (BSONObject)JSON.parse("{$and:[{a:{$gte:3000}},{a:{$lt : 6000}}]}");
        if((int)cl.getCount(updMatcher) != 3000){
            Assert.fail("fail to delete");
        }
        // check content
        BSONObject updSelector = (BSONObject)JSON.parse("{_id:{$include:0}}");
        BSONObject updOrder = (BSONObject)JSON.parse("{a:1}");
        DBCursor updCursor = cl.query(updMatcher, updSelector, updOrder, null);
    ...
    private void checkQueried(){
        ...
        // check count
        BSONObject qryMatcher = new BasicBSONObject();
        qryMatcher = (BSONObject)JSON.parse("{$and:[{a:{$gte:6000}},{a:{$lt : 9000}}]}");
        if((int)cl.getCount(qryMatcher) != 3000){
            Assert.fail("queried result is wrong");
        }
        // check content
        BSONObject qrySelector = (BSONObject)JSON.parse("{_id:{$include:0}}");
        BSONObject qryOrder = (BSONObject)JSON.parse("{a:1}");
        DBCursor qryCursor = cl.query(qryMatcher, qrySelector, qryOrder, null);
    ...
    private void checkInserted(){
        ...
        insMatcher = (BSONObject)JSON.parse("{$and:[{a:{$gte:9000}},{a:{$lt : 12000}}]}");
        if((int)cl.getCount(insMatcher) != 3000){
            Assert.fail("fail to insert");
        }
        // check content
        BSONObject insSelector = (BSONObject)JSON.parse("{_id:{$include:0}}");
        BSONObject insOrder = (BSONObject)JSON.parse("{a:1}");
        DBCursor insCursor = cl.query(insMatcher, insSelector, insOrder, null);
