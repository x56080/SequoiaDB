...
/**
 * @FileName:seqDB-6641: 主子表，自动切分数据
    ...
    private DBCollection createAndAttachCL(DBCollection mcl, String sclName){
        ...
        try{
            ...
            if(sclName.equals(sclName1)){
                attachOpt.put("LowBound", (BSONObject)JSON.parse("{a:0}"));
                attachOpt.put("UpBound", (BSONObject)JSON.parse("{a:2048}"));
            }else if(sclName.equals(sclName2)){
                attachOpt.put("LowBound", (BSONObject)JSON.parse("{a:2048}"));
                attachOpt.put("UpBound", (BSONObject)JSON.parse("{a:4096}"));
            }
    ...
    private void checkCompression(Sequoiadb dataDB, String clName){
        ...
        for(int i = 0; i < tryTimes; i++){
            ...
            if(!snapshot.hasNext()){
                ...
                throw new BaseException("snapshot is not exist. cl exists: " + cs.isCollectionExist(clName));
            }
            ...
            boolean typeRight = ((String)detail.get("CompressionType")).equals("snappy");
