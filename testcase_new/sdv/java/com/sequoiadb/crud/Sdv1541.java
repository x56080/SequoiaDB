...
public class Sdv1541 extends SdbTestBase {
    ...
    /**
     * 以下操作全部并发执行，并连接到相同的coord操作，数据量达到百万级别：
     * 1.创建100条bulkInsert线程
     * 2.创建100条find线程，指定从备节点上读
     * 3.1条更新线程，更新多条数据
     * 4.1条删除线程，更新多条数据
     * 5.创建索引
     */
    @Test
    public void test() {
        dbcl.bulkInsert(Arrays.asList(genrateData(10000)),0);

        ClTask insert = new ClTask() {
            @Override
            void opration(DBCollection cl) {
                cl.bulkInsert(Arrays.asList(genrateData(1000)),0);
            }
        };
