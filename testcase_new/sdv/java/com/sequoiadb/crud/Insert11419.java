...
public class Insert11419 extends SdbTestBase {
    ...
    /**
     * 1.插入100条记录
     * 2.并发插入新数据，并同时更新步骤1中插入的数据，待操作完成后，检查插入及更新的记录
     */
    @Test
    public void test() {
        for (int i = 0; i < 100; i++) {
            ...
            public void exec() throws Exception {
                ...
                try {
                    ...
                    DBCollection cl = db.getCollectionSpace(SdbTestBase.csName)
                            .getCollection(CLNAME);
                    for (int i = 0; i < 10; i++) {
                        cl.update(new BasicBSONObject("a",new BasicBSONObject("$isnull",0)), (BSONObject) JSON.parse("{$inc:{a:1}}"), new BasicBSONObject());
                    }
                } finally {
                    if (db != null)
                        db.disconnect();
