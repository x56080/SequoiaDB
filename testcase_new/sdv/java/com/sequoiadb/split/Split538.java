...
public class Split538 extends SdbTestBase {
	...
	public void setUp() {
		...
		try {
			...
			ArrayList<String> tmp = SplitUtils.getGroupName(commSdb, csName, clName);// 获取目标组名，和源组名
	...
	public void splitCL() {
		...
		try {
			...
			cl.split(srcGroupName, destGroupName, (BSONObject) JSON.parse("{a:2000}"),
	...
	private void checkCatalog(Sequoiadb sdb) {
		...
		try {
			...
			BSONObject expectLowBound = (BSONObject) JSON.parse("{\"a\":2000}");
			BSONObject expectUpBound = (BSONObject) JSON.parse("{\"a\":3000}");
