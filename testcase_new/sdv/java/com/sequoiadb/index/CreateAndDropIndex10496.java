...
/**
* FileName: CreateAndDropIndex10496.java
* test content:concurrent create different indexs, than concurrent drop different indexes  
* testlink case:seqDB-10496
* @author wuyan
    * @Date    2017.11.8
* @version 1.00
*/
public class CreateAndDropIndex10496 extends SdbTestBase {
    ...
    private void setup() {
        ...
        dbcl = sdb.getCollectionSpace(SdbTestBase.csName).createCollection(CLNAME);
        insertData(dbcl);
    }
    
    @Test
    private void CreateIndex(){
    	String[] indexNames = {"testa","testb","testdecimal","testchar","testdate"};
    	String[] indexKeys = {"{'a':1}","{'b':-1}","{'decimal':-1}","{'str':1,'test':-1}","{'date':1}"};
   	
    	List<CreateIndexTask> createIndexTasks = new ArrayList<>(5);
    	List<DropIndexTask> dropIndexTasks = new ArrayList<>(5);
    	for( int i = 0; i < 5; i++){
    		createIndexTasks.add(new CreateIndexTask(indexNames[i], indexKeys[i]));
    	...
    	for( CreateIndexTask createIndexTask : createIndexTasks ){
			Assert.assertTrue( createIndexTask.isSuccess(), createIndexTask.getErrorMsg());
		}
    	...
    	for( DropIndexTask dropIndexTask : dropIndexTasks ){
			Assert.assertTrue( dropIndexTask.isSuccess(), dropIndexTask.getErrorMsg());
		}
    ...
    private class CreateIndexTask extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            ...
            try {
                ...
                cl.createIndex( indexName, (BSONObject) JSON.parse(indexKey), false, false);                
            } finally {
                if (db != null)
                    db.disconnect();
            }
    ...
    private void insertData(DBCollection cl) {  
		...
		for ( int i = 0; i < 10; i++){
			...
			for (int j = 0; j < 10000; j++) {
				...
				obj.put("b", num);
				obj.put("c", num);
				obj.put("test","testeaaaaasdgadgaasdga"+num);
				obj.put("str", "test_" + String.valueOf(num));
				//insert the decimal type data
				String str = "32345.067891234567890123456789" + num;
    ...
    private void checkCreateIndexResult(String[] indexNames, String[] indexKeys){
    	for( int i = 0; i < indexNames.length; i++){
    		...
    		while(cursorIndex.hasNext()){
    			...
    			Assert.assertEquals(record.get("key"), JSON.parse(indexKeys[i]));
    ...
    private void checkDropIndexResult(String[] indexNames){
    	for( int i = 0; i < indexNames.length; i++){
    		...
    		while(cursorIndex.hasNext()){
    			...
    			BSONObject object = cursorIndex.getNext();
    			Assert.assertNull(object,"the index:"+indexNames[i]+" is exist!");
    		}
    		cursorIndex.close();
    	}
