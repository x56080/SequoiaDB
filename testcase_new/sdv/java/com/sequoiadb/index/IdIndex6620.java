...
/**
 *用例要求：
 *1、指定已存在id索引的cl，删除集合中的 $id 索引(dropIdIndex())
 *2、查看索引删除结果
 *3、对该cl分别执行插入、更新、删除数据操作
 *4、查看返回的结果是否正确 
 * @author huangwenhua
 * @Date   2016.12.15
 * @version 1.00
...
public class IdIndex6620 extends SdbTestBase {
	...
	public void dropIdIndex(){
		...
		try {
			...
			cursor1 = cl.explain(null, null, null, (BSONObject)JSON.parse("{'':'$id'}"), 0, -1, 0, null);
			String scanType = null;
			while(cursor1.hasNext()){
				BSONObject record = cursor1.getNext();
				if(record.get("Name").equals(SdbTestBase.csName + "." + this.clName)){
