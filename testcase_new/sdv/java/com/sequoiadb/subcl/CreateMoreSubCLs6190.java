...
/**
 * FileName: CreateMoreSubCLs6190.java
 * test content: 创建多个子表，指定的ShardingKey字段不同_SD.subCL.01.023 
 * testlink case: seqDB-6190
 * @author zengxianquan
 * @date 2016年12月22日
 * @version 1.00
 */
public class CreateMoreSubCLs6190 extends SdbTestBase{
	...
	public void attachSubcls(){	
		...
		try{
			...
			maincl.attachCollection(SdbTestBase.csName+".subcl6190_1", option1);
			
			String jsonStr2 = "{'LowBound':{'time':100},UpBound:{'time':200}}";
			BSONObject option2 = (BSONObject) JSON.parse(jsonStr2);
			maincl.attachCollection(SdbTestBase.csName+".subcl6190_2", option2);
		}catch(BaseException e){
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	public void checkCRUD(){
		...
		try{	
			...
			Assert.assertEquals(maincl.getCount(),90,"failed to delete data");		
		}catch(BaseException e){
			e.printStackTrace();
