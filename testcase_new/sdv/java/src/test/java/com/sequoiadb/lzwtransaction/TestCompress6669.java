...
/**
 * 1、CL压缩类型为lzw，开启事务，对CL做增删改查数据 2、执行事务提交 3、检查返回结果，并检查数据压缩情况
 * 
 * @author chensiqin
 * @Date 2016-12-16
 */
public class TestCompress6669 extends SdbTestBase {
    private Sequoiadb sdb;
    private CollectionSpace cs;
    private DBCollection cl;
    private String clName = "cl6669";
    ...
    /**
     * 1、CL压缩类型为lzw，开启事务，对CL做增删改查数据 2、执行事务lzw，开启事务，对CL做增删改查数据 3、检查返回结果，并检查数据压缩情况
     */
    @Test
    public void test() {
        try {
            ...
            // 对cl做增删改查操作
            util.insertData(this.cl, 115, 120, 1024);
            checkAdd();
            ...
            this.sdb.commit();
            checkCommit();
            // 提交后记录跟开启事务前数据一致且记录压缩状态一致
            BSONObject after = getSnapshotDetail();
            Assert.assertEquals(before.get("CompressionType").toString(), after.get("CompressionType").toString());
            Assert.assertEquals(before.get("DictionaryCreated").toString(), after.get("DictionaryCreated").toString());
            if ((double) after.get("CurrentCompressionRatio") >= 1) {
                Assert.fail("CurrentCompressionRatio >= 1 !");
            }
            Assert.assertEquals(before.get("TotalDataPages").toString(), after.get("TotalDataPages").toString());
        } catch (BaseException e) {
            Assert.fail(e.getMessage());
        } finally {
            sdb.commit();
    ...
    private void checkDelete() {
        try {
            DBCursor cursor = this.cl.query((BSONObject) JSON.parse("{_id:{$et:115}}"), null, null, null);
    ...
    public void checkAdd() {
        try {
            DBCursor cursor = this.cl.query((BSONObject) JSON.parse("{$and:[{_id:{$gte:115}},{_id:{$lt:120}}]}"), null,
                    null, null);
            int i = 0;
            while (cursor.hasNext()) {
                cursor.getNext();
