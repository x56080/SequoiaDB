...
public class TestSeqDB6670 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        if ( CommLib.isStandAlone( this.sdb ) ) {
    ...
    public void checkData() {
        // 协调节点检查数据正确性
        Assert.assertEquals(
                this.cl.getCount( "{$and:[{_id:{$gte:0}},{_id:{$lt:115}}]}" ),
                115 );
        // 连接主节点数据正确性
        checkDataInMaster();
        // 连接从节点检查数据正确性
    ...
    public void createDataGroup() {
        ...
        rg.createNode( hostName, this.port1, SdbTestBase.workDir + port1 + "/",
                configure );
        rg.createNode( hostName, this.port2, SdbTestBase.workDir + port2 + "/",
                configure );
        rg.createNode( hostName, this.port3, SdbTestBase.workDir + port3 + "/",
                configure );
