/************************************************************************
*@Description:  seqDB-816:主表为range分区,子表为hash分区,在主表做update_SD.subCL.01.003
...
function insertRecs ( mainCL, insertRecsNum )
{
   ...
   {
      mainCL.insert( { a: i, b: -i, c: "test" } );
   }
}

function updateRecs ( mainCL )
{
   println( "\n---Begin to update records." );

   mainCL.update( { $set: { b: "hello" } }, { a: { $lt: 50 } } );
}
...
function checkResult ( csName, mainCL, subCLName, insertRecsNum )
{
   ...
   var actUpsertRecsCnt1 = mainCL.find( { a: { $lt: 50 }, b: "hello" } ).count();
