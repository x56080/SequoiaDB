...
/**
 * @Descreption seqDB-32694 :: 版本: 1 :: 增量同步过程重复更新数据
 * @Author wangxingming
 * @CreateDate 2023/8/8
 ...
 * @Version 1.0
 */
public class SequoiaShake32694 {
    ...
    public void test() throws Exception {
        ...
        Document document = new Document( "id", 1 ).append( "name", "name_1" )
                .append( "time", new Date( Long.MIN_VALUE ) );
        ...
        Callable< Void > task2 = new Callable< Void >() {
            @Override
            public Void call() throws Exception {
                collection.updateOne( Filters.eq( "id", 1 ), new Document(
                        "$set", new Document( "name", "name_2" ) ) );
                collection.updateOne( Filters.eq( "id", 1 ), new Document(
                        "$set", new Document( "name", "name_3" ) ) );
                collection.updateOne( Filters.eq( "id", 1 ), new Document(
                        "$set", new Document( "name", "name_4" ) ) );
                return null;
            }
        };
