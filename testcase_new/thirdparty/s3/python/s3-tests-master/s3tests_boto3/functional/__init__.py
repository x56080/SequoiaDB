...
def setup():
    ...
    config.default_host = cfg.host
    config.default_port = cfg.port
    config.default_is_secure = cfg.is_secure
    # print('secure:', config.default_is_secure)

    proto = 'http' if config.default_is_secure else 'https'
    config.default_endpoint = "%s://%s:%s" % (proto, config.default_host, config.default_port)

    # vars from the main section
    config.main_access_key = cfg.access_key
    config.main_secret_key = cfg.secret_key
    config.main_display_name = cfg.display_name
    config.main_user_id = cfg.user_id
    config.main_email = cfg.email
    try:
        config.main_kms_keyid = cfg.kms_keyid
    except (ConfigParser.NoSectionError, ConfigParser.NoOptionError):
        config.main_kms_keyid = 'testkey-1'
    try:
        config.main_kms_keyid2 = cfg.kms_keyid
    except (ConfigParser.NoSectionError, ConfigParser.NoOptionError):
        config.main_kms_keyid2 = 'testkey-2'

    try:
        config.main_api_name = cfg.api_name
    except (ConfigParser.NoSectionError, ConfigParser.NoOptionError):
        config.main_api_name = ""
        pass

    config.alt_access_key = cfg.alt_access_key
    config.alt_secret_key = cfg.alt_secret_key
    config.alt_display_name = cfg.alt_display_name
    config.alt_user_id = cfg.alt_user_id
    config.alt_email = cfg.alt_email

    config.tenant_access_key = cfg.tenant_access_key
    config.tenant_secret_key = cfg.tenant_secret_key
    config.tenant_display_name = cfg.tenant_display_name
    config.tenant_user_id = cfg.tenant_user_id
    config.tenant_email = cfg.tenant_email

    # vars from the fixtures section
    try:
        template = cfg.bucket_prefix
