...
from s3tests_boto3 import s3conf
...
# def read_config(fp):
#     config =
#     g = yaml.load(fp,Loader=yaml.FullLoader)
#
#     # g = yaml.safe_load_all(fp)
#     for new in g:
#         print('new:',new)
#         print('bunchify:',bunch.bunchify(new))
...
def connect(conf):
    ...
    calling_formats = dict(
        ordinary=boto3.s3.connection.OrdinaryCallingFormat(),
        subdomain=boto3.s3.connection.SubdomainCallingFormat(),
        vhost=boto3.s3.connection.VHostCallingFormat(),
    )
    ...
    conn = boto3.s3.connection.S3Connection(**kwargs)
    return conn
...
def setup():
    ...
    try:
        path = 'E:/tmp/s3-tests-master/s3-tests-master/s3tests.conf'
