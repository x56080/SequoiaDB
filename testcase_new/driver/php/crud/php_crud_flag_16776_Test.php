...
class TestCURDFlag16776 extends PHPUnit_Framework_TestCase
{
   ...
   function test()
   {
      ...
      self::checkRecordNum( 1, array('test' => 'recordNotExist' ),  "inser the record, flag = SDB_FLG_INSERT_CONTONDUP" );
       ...
       //bulkInsert already exist record set flag=2
      ...
      $data3 = array( '_id' => 'aaa', 'test' => "testflagSDB_FLG_INSERT_RETURN_OID2");
      $rc = self::$cl -> insert( $data3, SDB_FLG_INSERT_RETURN_OID );
      if($rc['_id'] !=  'aaa' )
      {
         throw new Exception( "expect _id = aaa  but found _id = ". $rc['_id'] . " set flag = SDB_FLG_INSERT_RETURN_OID");
      }
      self::checkErrno( 0, self::$db -> getError()['errno'] );
      self::checkRecordNum( 1, array('_id' => 'aaa'),  "bulkInser the same record, check SDB_FLG_INSERT_RETURN_OID" );
      
      $testData11 =  array( "flag" => "testflag=2");
      $rc = self::$cl -> insert( $testData11, 2 );
      self::checkErrno( 0, self::$db -> getError()['errno'] );
      self::checkRecordNum( 1, array( "flag" => "testflag=2"), "test flag = 2" );
     
      //SDB_FLG_INSERT_REPLACEONDUP:if the record hit index key duplicate error,database will replace the existing record bythe inserting new record.
      $testData1 =  array("_id" => 11,"test" => "SDB_FLG_INSERT_REPLACEONDUP" );
      self::$cl -> insert( $testData1, SDB_FLG_INSERT_REPLACEONDUP );
      self::checkErrno( 0, self::$db -> getError()['errno'] );
      self::checkRecordNum( 1, $testData1,  "inser the record, flag = SDB_FLG_INSERT_REPLACEONDUP" );

      $testData2 =  array("_id"=>11,"test" => "CHANGE");
      self::$cl -> insert( $testData2, SDB_FLG_INSERT_REPLACEONDUP );
      self::checkErrno( 0, self::$db -> getError()['errno'] );
      self::checkRecordNum( 1, $testData2,  "inser the record, flag = SDB_FLG_INSERT_REPLACEONDUP" );
