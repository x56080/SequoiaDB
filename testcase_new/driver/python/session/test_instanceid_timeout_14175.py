# @decription: instanceid, timeout
# @testlink:   seqDB-14175
...
class TestSessiontimeout14175(testlib.SdbTestBase):
   def setUp(self):
      # check standalone
      if testlib.is_standalone():
         self.skipTest('run mode is standalone')
      self.data_rg_name = "session14175"
      testlib.drop_cs(self.db, self.cs_name, ignore_not_exist = True)

   def test_session_timeout_14175(self):
      # create data rg
      ...
      # set session attr, timeout 1s
      opt = {'PreferedInstance' : 11, 'Timeout' : 1000};
      ...
      self.check_explain(expect_node, act_node)
      
      # check session attr
      act_session_attr = self.db.get_session_attri()
      expect_session_attr = {'PreferedInstance' : 11, 'PreferedInstanceMode' : 'random', 'Timeout' : 1000}
      self.check_session_attr(expect_session_attr, act_session_attr)
      
      # reset session attr, timeout 1ms
      opt = {'PreferedInstance' : 12, 'Timeout' : 1};
      self.db.set_session_attri(options = opt)
      
      # query and check explain
      #cond = {'a' : {'$gt' : 1}}         
      #self.check_query_timeout(condition = cond)
      ...
      opt = {'PreferedInstance' : 11, 'Timeout' : 60000};
      self.db.set_session_attri(options = opt)
   ...
   def check_query_timeout( self , **kwargs ):
      try:
         self.cl.query(**kwargs)
         self.fail('Need Check Query TimeOut')
      except SDBBaseError as e:
         if -13 != e.code:
            self.fail('check session timeout failed')   
