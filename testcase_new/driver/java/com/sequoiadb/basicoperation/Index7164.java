...
public class Index7164 extends SdbTestBase {
	...
	public void test(){
		...
		try{
			cl.createIndex(indexName, indexObj, isUnique, enforced, sortBufferSize);
		}catch(BaseException e){
			Assert.fail("create index failed, errMsg:" + e.getMessage());
		}
		Assert.assertNotNull(cl.getIndex(indexName), "expect index is not null");;
		...
		DBCursor explainCursor = cl.explain(matcher, null, orderBy, null, 0, -1, 0, explainOption);
		...
		while(cursorIndex.hasNext()){
			BSONObject record = (BSONObject) cursorIndex.getNext().get("IndexDef");
