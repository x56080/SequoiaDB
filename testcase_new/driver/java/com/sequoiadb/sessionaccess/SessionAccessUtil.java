...
public class SessionAccessUtil extends SdbTestBase {
    ...
    public static BasicBSONList createRG( Sequoiadb db, String rgName ) {
        ...
        int[][] param = { { reservedPortBegin + 10, 7 },
                { reservedPortBegin + 20, 8 }, { reservedPortBegin + 30, 9 }, };
        String hostName = db.getReplicaGroup( "SYSCatalogGroup" ).getMaster()
                .getHostName();
        ...
        for ( int[] ints : param ) {
            ...
            rg.createNode( hostName, ints[ 0 ],
                    SdbTestBase.reservedDir + "/data/" + ints[ 0 ], config );

            BSONObject node = new BasicBSONObject();
            node.put( "nodeName",
                    hostName + ":" + String.valueOf( ints[ 0 ] ) );
    ...
    public static boolean isMaster( Sequoiadb db, String groupName,
            String nodeName ) {
    ...
    public static int getInstanceidByNodeName( BasicBSONList nodes,
            String nodeName ) {
    ...
    public static String getNodeNameByInstanceId( BasicBSONList nodes,
            String instanceId ) {
        for ( int i = 0; i < nodes.size(); i++ ) {
            BasicBSONObject node = ( BasicBSONObject ) nodes.get( i );
    ...
    public static List< Integer > getInstanceidList( BasicBSONList nodes ) {
        ...
        for ( int i = 0; i < nodes.size(); i++ ) {
            BasicBSONObject node = ( BasicBSONObject ) nodes.get( i );
            instanceidList
                    .add( Integer.parseInt( node.getString( "instanceid" ) ) );
        }
        return instanceidList;
    }

    public static String getActualDataNodeName( DBCollection dbcl ) {
        DBCursor cur = dbcl.explain( null, null, null, null, 0, 10, 0,
