...
/**
 *  seqDB-11898:data类型实现BSON和JSON类型转换
 *  seqDB-11899:binary类型实现BSON和JSON类型转换
...
public class BSONCover {
    ...
    /**
     * 验证:
     *  1.使用Date构造BSONTimestamp,以及toDate接口
...
//         nanoseconds loss
    ...
    /**
     * 测试用例 seqDB-11898
     * 验证：JSON.parse()解析date时，是否使用当前时区.
     */
    @Test
    ...
    /**
     * seqDB-11900
     * 验证：Timestimp类型在BSONObject和JSON之间转换
     */
    @Test
    ...
    /**
     * seqDB-11899
     * 验证：binary在序列化为JSON字符串时，是否转换为正确的二进制字符串，JSON.parse是否能正常解析。
     */
    @Test
