...
* 			1.createReplicaGroup()
* 			2.createNode()
* 			3.activateReplicaGroup()
* 			4.getReplicaGroup()
* 			5.getDetail()
* 			6.listReplicaGroups()
* 			7.getReplicaGroupsInfo()
* 			8.stop()
* 			9.start()
* 			10.getMaster()
* 			11.getSlave()
...
public class ClusterManager7065 extends SdbTestBase{
	...
	public void test(){
		...
		//create data node
		try{
			dataRGAdd.createNode(coordIP, dataPortAdd1, dataPathAdd1, dataConfigue1 );
		...
		try{
			...
			BasicBSONList serviceList = (BasicBSONList) groupObj.get("Service");
			BasicBSONObject serviceObj = (BasicBSONObject) serviceList.get(0);
			service = Integer.parseInt((String)serviceObj.get("Name")) ;
		}catch( BaseException e){
			Assert.fail("get replica group configure failed" + e.getMessage());
		}
		...
		try{
			...
			while(replicaCursor.hasNext()){
				...
				if(groupName1.equals(dataRGName) ){
					BasicBSONList group = (BasicBSONList) replicaRecord.get("Group");
					BasicBSONObject groupObj = (BasicBSONObject) group.get(0);
					hostName1 = (String) groupObj.get("HostName");
					nodeStatus1 = (int) groupObj.get("Status");
					dbpath1 = (String) groupObj.get("dbpath");
					BasicBSONList serviceList1 = (BasicBSONList) groupObj.get("Service");
					BasicBSONObject serviceObj1 = (BasicBSONObject) serviceList1.get(0);
					service1 = Integer.parseInt((String)serviceObj1.get("Name")) ;
				}
			}
		}catch( BaseException e ){
		...
		//node operate
		try{
			Node node = sdb.getReplicaGroup(dataRGName).getNode(coordIP + ":" + dataPortAdd1);
			node.stop();
			node.start();
		}catch( BaseException e ){
