...
public class CommLib {
	...
	public void createDomain(Sequoiadb sdb, String domainName,BSONObject domainOption){
		try{
	...
	public void checkDomainInfo(Sequoiadb sdb, 
								BSONObject matcher,
								BSONObject selector,
								BSONObject orderBy,
								BSONObject hint, 
								ArrayList<String> expectDomainGroups,
								String expectDomainName,boolean expectAutoSplit){
		...
		while (dbCursor.hasNext()) {
			...
			//check group name
			Assert.assertEqualsNoOrder(domainGroupArrs.toArray(), expectDomainGroups.toArray());
	...
	public void alterDomain(Sequoiadb sdb, String domainName, BSONObject domainOption){
		try{
