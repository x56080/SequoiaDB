...
public class MetaData7080 extends SdbTestBase{
	...
	public void test(){
		...
		try{
			mainCL.attachCollection(subCSName+"."+subCLName1, attachOption1);
			mainCL.attachCollection(subCSName+"."+subCLName2, attachOption2);
		}catch(BaseException e){
		...
		try{
			...
			Assert.fail("insert " + i + "th record failed, insert value :"+ insertData + 
					", errMsg:" + e.getMessage());
		}
		
		Assert.assertEquals(subCL1.getCount(), 2,
				subCLName1 + "count actual:" + subCL1.getCount() + ";the expect :" + 2);
		Assert.assertEquals(subCL2.getCount(), 2,
				subCLName2 + "count actual:" + subCL2.getCount() + ";the expect :" + 2);
		...
		for(i = 0; i<value.length; i++){
			try{
				...
				if(-135 != e.getErrorCode()){
					Assert.assertTrue(false,"insert data, errMsg:" + e.getMessage());
				}
			}
		}
		...
		Assert.assertEquals(mainCL.getCount(), 0,
				mainCLName + "count actual:" + mainCL.getCount() + ";the expect :" + 0);
