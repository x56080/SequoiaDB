...
class snapshotSysTest14118 : public testing::Test
{
protected:
   ...
   INT32 init()
   {
      ...
      CHECK_RC( rc, "fail to get groups, rc = %d", rc ) ;
      rgName = groups[0].c_str() ;
      rc = getGroupNodes( db, rgName, nodes ) ;
      CHECK_RC( rc, "fail to get group nodes, rc = %d", rc ) ;
      idx = nodes[0].find( ":" ) ;
...
INT32 getDiskName( sdbConnectionHandle db, CHAR* name )
{
   ...
   CHECK_RC( rc, "fail to snapshot system, rc = %d", rc ) ;
   rc = sdbNext( cursor, &obj ) ;
   CHECK_RC( rc, "fail to get next, rc = %d", rc ) ;
   ...
   CHECK_RC( rc, "fail to close cursor, rc = %d", rc ) ;
...
TEST_F( snapshotSysTest14118, snapshotSys )
{
   ...
   rc = sdbConnect( dataHostName, dataSvcName, USER, PASSWD, &dataDb ) ;
   ...
   {
      rc = sdbConnect( dataHostName, dataSvcName, USER, PASSWD, &dataDbs[i] ) ;
