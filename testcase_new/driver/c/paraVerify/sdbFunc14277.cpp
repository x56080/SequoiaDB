...
class sdbParaVerify : public testing::Test
{
protected:
...
TEST_F( sdbParaVerify, connect )
{
   ...
   const CHAR* svcName = SVCNAME ;
   const CHAR* user = USER ;
   const CHAR* passwd = PASSWD ;
   CHAR addr[100] = { 0 } ;
   sprintf( addr, "%s%s%s", HOSTNAME, ":", SVCNAME ) ;
...
TEST_F( sdbParaVerify, snapshot )
{
   ...
   ASSERT_EQ( SDB_CLT_INVALID_HANDLE, rc ) ;
   rc = sdbGetSnapshot( db, 13, NULL, NULL, NULL, &cursor ) ;
   ASSERT_EQ( SDB_INVALIDARG, rc ) ;
...
TEST_F( sdbParaVerify, trace )
{
   ...
   // test sdbTraceStart
   rc = sdbTraceStart( NULL, traceBufSize, NULL, NULL ) ;
   ASSERT_EQ( SDB_INVALIDARG, rc ) ;
   rc = sdbTraceStart( SDB_INVALID_HANDLE, traceBufSize, NULL, NULL ) ;
   ASSERT_EQ( SDB_INVALIDARG, rc ) ;
   rc = sdbTraceStart( cs, traceBufSize, NULL, NULL ) ;
   ASSERT_EQ( SDB_CLT_INVALID_HANDLE, rc ) ;
...
TEST_F( sdbParaVerify, pop )
{
   ...
   // test sdbPop, sdbPop not exist in 2.8
}
...
TEST_F( sdbParaVerify, analyze )
{
   ...
   // test sdbAnalyze, sdbAnalyze not exist in 2.8
}
