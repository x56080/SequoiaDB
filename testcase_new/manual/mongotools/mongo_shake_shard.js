...
/*********************************************测试用例***********************************************
 * 环境准备：
 *    部署 MongoDB 数据库，mongos 的端口为 27017
...
function fullTest() {
  ...
  printCostTime(startMongoShake, ["full", false]);
...
function incrTest() {
  ...
  printCostTime(startMongoShake, ["incr", false]);
...
function allTest() {
  ...
  // 2. start mongoshake in all mode
  printCostTime(startMongoShake, ["all", false]);

  // 3. check data in SequoiaDB
  printCostTime(checkSequoiadbData, [Test_DB, 60]);

  // 4. drop database in SequoiaDB
  printCostTime(dropSequoiaDB, [Test_DB]);

  // 5. generate data to MongoDB
  printCostTime(genDataToMongoDB, [4]);

  // 6. check data in SequoiaDB
  printCostTime(checkSequoiadbData, [Test_DB, 60]);
