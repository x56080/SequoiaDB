...
namespace engine
{
   ...
   */
   ...
   /*
   note: _clsCataGIndex implement
   */
   _clsCataGIndex::_clsCataGIndex()
   {
   ...
   _clsCataGIndex::_clsCataGIndex ( const _clsCataGIndex &right )
   {
      _indexName = right._indexName ;
      _indexCLUID = right._indexCLUID ;
   }

   _clsCataGIndex& _clsCataGIndex::operator= ( const _clsCataGIndex &right )
   {
      _indexName = right._indexName ;
      _indexCLUID = right._indexCLUID ;
   ...
   INT32 _clsCataGIndex::init( const BSONObj &globalIndex )
   {
      INT32 rc = SDB_OK ;
      BSONElement element = globalIndex.getField( CAT_GIDX_CL_UNIQUEID ) ;
      if ( NumberLong != element.type() )
      {
         rc = SDB_INVALIDARG ;
         PD_LOG( PDERROR, "Failed to get collection's unique id:"
                 "fieldName=%s,rc=%d", CAT_GIDX_CL_UNIQUEID, rc) ;
         goto error ;
      }
      _indexCLUID = ( utilCLUniqueID ) element.numberLong() ;

      element = globalIndex.getField( CAT_GIDX_INDEXNAME ) ;
      if ( String != element.type() )
      {
         rc = SDB_INVALIDARG ;
         PD_LOG( PDERROR, "Failed to get collection's unique id:"
                 "fieldName=%s,rc=%d", CAT_GIDX_INDEXNAME, rc) ;
         goto error ;
      }
      _indexName = element.valuestr() ;
   ...
   INT32 _clsCataGIndex::toBSONObj( BSONObjBuilder &builder )
   {
      INT32 rc = SDB_OK ;

      try
      {
         builder.append( CAT_GIDX_INDEXNAME, _indexName ) ;
         builder.append( CAT_GIDX_CL_UNIQUEID, (long long)_indexCLUID ) ;
      }
      catch( std::exception &e )
      {
         rc = SDB_SYS ;
   ...
   utilCLUniqueID _clsCataGIndex::getIndexCollectionUID()
   {
      return _indexCLUID ;
   }

   const string& _clsCataGIndex::getIndexName() const
   {
      return _indexName ;
   }

   /*
   note: _clsCataGIndexGroup implement
   */
   _clsCataGIndexGroup::_clsCataGIndexGroup()
   {
   }
   ...
   INT32 _clsCataGIndexGroup::init( const BSONObj &globalIndexes )
   {
      INT32 rc = SDB_OK ;
      BSONElement element ;
      ...
      try
      {
         element = globalIndexes.getField( CAT_GLOBAL_INDEX ) ;
         if ( element.eoo() )
         {
            // do not have global index
            goto done ;
         }

         PD_CHECK( Array == element.type(), SDB_INVALIDARG, error, PDERROR,
                   "Failed to get global indexes:field=%s,rc=%d",
                   CAT_GLOBAL_INDEX, rc ) ;

         BSONObjIterator iter( element.embeddedObject() ) ;
         while ( iter.more() )
         {
            _clsCataGIndex index ;
            BSONObj indexObj ;
            BSONElement indexEle = iter.next() ;
            PD_CHECK( Object == indexEle.type(), SDB_INVALIDARG, error, PDERROR,
                      "Index info must be Object type:type=%d,rc=%d",
                      indexEle.type(), rc ) ;

            indexObj = indexEle.embeddedObject() ;
            rc = index.init( indexObj ) ;
            PD_RC_CHECK( rc, PDERROR, "Failed to init global index info:"
                         "index=%s,rc=%d", indexObj.toString().c_str(), rc ) ;

            _indexMap[ index.getIndexName() ] = index ;
         }
      }
      catch( std::exception &e )
      {
   ...
   INT32 _clsCataGIndexGroup::toBSONObj( BSONObjBuilder &builder )
   {
      INT32 rc = SDB_OK ;

      try
      {
         BSONArrayBuilder arrBuilder( builder.subarrayStart(CAT_GLOBAL_INDEX) ) ;
         CLS_GINDEX_MAP_IT iter = _indexMap.begin() ;
         while ( iter != _indexMap.end() )
         {
            BSONObjBuilder tmpBuilder ;

            _clsCataGIndex &tmpIndex = iter->second ;
            rc = tmpIndex.toBSONObj( tmpBuilder ) ;
            PD_RC_CHECK( rc, PDERROR, "Failed to get index's bson value:rc=%d",
                         rc ) ;

            arrBuilder.append( tmpBuilder.obj() ) ;
            ++iter ;
         }
   ...
   INT32 _clsCataGIndexGroup::addIndex( const _clsCataGIndex &indexInfo )
   {
      INT32 rc = SDB_OK ;
      CLS_GINDEX_MAP_IT iter = _indexMap.find( indexInfo.getIndexName() ) ;
      if ( iter != _indexMap.end() )
      {
         rc = SDB_IXM_EXIST ;
         PD_LOG( PDERROR, "Index is exist:index=%s",
      ...
      try
      {
         _indexMap[indexInfo.getIndexName()] = indexInfo ;
      }
      catch( std::exception &e )
      {
         rc = SDB_SYS ;
   ...
   INT32 _clsCataGIndexGroup::delIndex( const string &indexName )
   {
      _indexMap.erase( indexName ) ;
      return SDB_OK ;
   }

   INT32 _clsCataGIndexGroup::getSize()
   {
      return _indexMap.size() ;
   }

   BOOLEAN _clsCataGIndexGroup::isIndexExist( const string &indexName )
   {
      CLS_GINDEX_MAP_IT iter = _indexMap.find( indexName ) ;
      return iter != _indexMap.end() ? TRUE : FALSE ;
   }

   BOOLEAN _clsCataGIndexGroup::getIndexInfo( const string &indexName,
                                              _clsCataGIndex &indexInfo )
   {
      CLS_GINDEX_MAP_IT iter = _indexMap.find( indexName ) ;
      if ( iter != _indexMap.end() )
      {
         indexInfo = iter->second ;
         return TRUE ;
      }
   ...
   INT32 _clsCataGIndexGroup::getIndexes( CLS_GINDEX_LIST &globalIndexes )
   {
      INT32 rc = SDB_OK ;
      CLS_GINDEX_MAP_IT iter = _indexMap.begin() ;
      ...
      try
      {
         while ( iter != _indexMap.end() )
         {
            globalIndexes.push_back( iter->second ) ;
            ++iter ;
         }
      }
      catch ( std::exception &e)
      {
         rc = SDB_OOM ;
         PD_LOG( PDERROR, "Catch exception:e=%s", e.what() ) ;
         goto error ;
      }
   ...
   */
                                       ...
                                       BOOLEAN allowUpdateID )
   {
      ...
      rc = _gIndexGroup.init( catSet ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to init globalIndexes:rc=%d", rc ) ;
   ...
   BOOLEAN _clsCatalogSet::isExistGlobalIndex( const string &indexName )
   {
      return _gIndexGroup.isIndexExist( indexName ) ;
   }

   BOOLEAN _clsCatalogSet::getGlobalIndex( const string &indexName,
                                           _clsCataGIndex &gIndex )
   {
      return _gIndexGroup.getIndexInfo( indexName, gIndex ) ;
   }

   INT32 _clsCatalogSet::addGlobalIndex( const _clsCataGIndex &gIndex )
   {
      return _gIndexGroup.addIndex( gIndex ) ;
   }

   INT32 _clsCatalogSet::delGlobalIndex( const string &indexName )
   {
      return _gIndexGroup.delIndex( indexName ) ;
   }

   INT32 _clsCatalogSet::toGlobalIndexBson( BSONObjBuilder &builder )
   {
      return _gIndexGroup.toBSONObj( builder ) ;
   }

   INT32 _clsCatalogSet::getGlobalIndexSize()
   {
      return _gIndexGroup.getSize() ;
   }

   INT32 _clsCatalogSet::getGlobalIndexes( CLS_GINDEX_LIST &gIndexes )
   {
      return _gIndexGroup.getIndexes( gIndexes ) ;
   }
