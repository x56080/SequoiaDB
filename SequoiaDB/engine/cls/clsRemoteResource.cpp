/*******************************************************************************
   ...
   Source File Name = clsRemoteResource.cpp
...
namespace engine
{
   ...
   */
   ...
   INT32 _clsRemoteResource::init( _netRouteAgent *pAgent,
   ...
   netRouteAgent *_clsRemoteResource::getRouteAgent()
   {
      return _pAgent ;
   }

   IOmProxy *_clsRemoteResource::getOmProxy()
   {
      return _pOmProxy ;
   }

   coordOmStrategyAgent *_clsRemoteResource::getOmStrategyAgent()
   {
      return _pOmStrategyAgent ;
   }

   INT32 _clsRemoteResource::updateOmGroupInfo( CoordGroupInfoPtr &groupPtr,
                                                pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   BSONObj _clsRemoteResource::_buildOmGroupInfo()
   {
      BSONObj obj ;
   ...
   void _clsRemoteResource::_initAddressFromPair( const vector< pmdAddrPair > &vecAddrPair,
                                                  INT32 serviceType,
   ...
   void _clsRemoteResource::_addAddrNode( const MsgRouteID &id,
   ...
   void _clsRemoteResource::clearCataNodeAddrList()
   {
   ...
   BOOLEAN _clsRemoteResource::addCataNodeAddrWhenEmpty( const CHAR *pHostName,
                                                         const CHAR *pSvcName )
   {
      BOOLEAN add = FALSE ;
      MsgRouteID routeID ;
   ...
   void _clsRemoteResource::setCataGroupInfo( CoordGroupInfoPtr &groupPtr,
                                              BOOLEAN inheritStat )
   {
      ossScopedLock lock( &_nodeMutex, EXCLUSIVE ) ;
   ...
   void _clsRemoteResource::setOmGroupInfo( CoordGroupInfoPtr &groupPtr )
   {
      ossScopedLock lock( &_nodeMutex, EXCLUSIVE ) ;
      _omGroupInfo = groupPtr ;
   }

   INT32 _clsRemoteResource::syncAddress2Options( BOOLEAN flush, BOOLEAN force )
   {
      INT32 rc = SDB_OK ;
      ...
      SDB_ASSERT( NULL != _pOptionsCB, "options CB is invalid" ) ;
      PD_CHECK( NULL != _pOptionsCB, SDB_SYS, error, PDERROR,
                "Failed to sync address, no option CB" ) ;
   ...
   INT32 _clsRemoteResource::getGroupInfo( UINT32 groupID,
                                           CoordGroupInfoPtr &groupPtr )
   {
      ...
      else if ( CATALOG_GROUPID == groupID )
      {
         groupPtr = getCataGroupInfo() ;
         rc = SDB_OK ;
      }
   ...
   INT32 _clsRemoteResource::getGroupInfo( const CHAR *groupName,
                                           CoordGroupInfoPtr &groupPtr )
   {
      ...
      else if ( 0 == ossStrcmp( CATALOG_GROUPNAME, groupName ) )
      {
         groupPtr = getCataGroupInfo() ;
         rc = SDB_OK ;
      }
   ...
   UINT32 _clsRemoteResource::getGroupsInfo( GROUP_VEC &vecGroupPtr,
                                             BOOLEAN exceptCata,
                                             BOOLEAN exceptCoord )
   {
      UINT32 count = 0 ;
   ...
   UINT32 _clsRemoteResource::getGroupList( CoordGroupList &groupList,
                                            BOOLEAN exceptCata,
                                            BOOLEAN exceptCoord )
   {
      UINT32 count = 0 ;
   ...
   INT32 _clsRemoteResource::getGroupIDs( VEC_UINT32 &groupIDs,
                                          BOOLEAN exceptCata,
                                          BOOLEAN exceptCoord )
   {
      INT32 rc = 0 ;
   ...
   INT32 _clsRemoteResource::getGroupNames( VEC_POOLSTR &groupNames,
                                            BOOLEAN exceptCata,
                                            BOOLEAN exceptCoord )
   {
      INT32 rc = 0 ;
      ...
      while( it != _mapGroupInfo.end() )
      {
         ...
         try
         {
            groupNames.push_back( it->second->groupName().c_str() ) ;
         }
         catch ( exception &e )
         {
            PD_LOG( PDERROR, "Failed to save group name, occur exception %s",
                    e.what() ) ;
            rc = ossException2RC( &e ) ;
            goto error ;
         }
   ...
   INT32 _clsRemoteResource::getGroupNames( vector< string > &groupNames,
                                            BOOLEAN exceptCata,
                                            BOOLEAN exceptCoord )
   {
      INT32 rc = 0 ;
      ...
      while( it != _mapGroupInfo.end() )
      {
         ...
         try
         {
            groupNames.push_back( it->second->groupName() ) ;
         }
         catch ( exception &e )
         {
            PD_LOG( PDERROR, "Failed to save group name, occur exception %s",
   ...
   INT32 _clsRemoteResource::addGroupInfo( const BSONObj &groupObj )
   {
      INT32 rc = SDB_OK ;

      CoordGroupInfoPtr groupPtr ;

      rc = _addGroupInfo( groupObj, groupPtr ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to add group info, rc: %d", rc ) ;
                                           ...
                                           CoordGroupInfoPtr &groupPtr )
   {
      ...
      rc = _addGroupInfo( groupObj, tmpPtr ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to add group info, rc: %d", rc ) ;
   ...
   INT32 _clsRemoteResource::_addGroupInfo( const BSONObj &groupObj,
                                            CoordGroupInfoPtr &groupPtr )
   {
      INT32 rc = SDB_OK ;
      ...
      try
      {
         BSONElement beGroupID = groupObj.getField( CAT_GROUPID_NAME ) ;
         if ( beGroupID.eoo() || !beGroupID.isNumber() )
         {
            rc = SDB_SYS ;
            PD_LOG ( PDERROR, "Reply object[%s] is invalid",
                     groupObj.toString().c_str() ) ;
            goto error ;
         }
         ...
         rc = groupPtr->updateGroupItem( groupObj ) ;
         if ( rc )
         {
            PD_LOG( PDERROR, "Update group info from bson[%s] failed, "
                    "rc: %d", groupObj.toString().c_str(), rc ) ;
            goto error ;
         }
   ...
   INT32 _clsRemoteResource::addGroupInfo( UINT32 groupID,
                                           NET_ROUTE_MAP &nodes,
                                           UINT32 primaryNodeID )
   {
      ...
      rc = _addGroupInfo( groupID, nodes, primaryNodeID, groupPtr ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to add group info, rc: %d", rc ) ;
                                           ...
                                           CoordGroupInfoPtr &groupPtr )
   {
      ...
      rc = _addGroupInfo( groupID, nodes, primaryNodeID, tmpPtr ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to add group info, rc: %d", rc ) ;
   ...
   INT32 _clsRemoteResource::_addGroupInfo( UINT32 groupID,
                                            NET_ROUTE_MAP &nodes,
                                            UINT32 primaryNodeID,
                                            CoordGroupInfoPtr &groupPtr )
   {
      ...
      try
      {
         pGroupInfo = SDB_OSS_NEW CoordGroupInfo( groupID ) ;
         if ( NULL == pGroupInfo )
         {
            rc = SDB_OOM ;
            PD_LOG ( PDERROR, "Alloc group info failed" ) ;
         ...
         rc = groupPtr->updateNodes( nodes ) ;
         if ( rc )
         {
            PD_LOG( PDERROR, "Update group info from node map failed, "
                    "rc: %d", rc ) ;
            goto error ;
         }
         ...
         if ( 0 != primaryNodeID )
         {
            ...
            primaryID.columns.groupID = groupID ;
            primaryID.columns.nodeID = primaryNodeID ;
            if ( CATALOG_GROUPID == groupID )
            {
               primaryID.columns.serviceID = MSG_ROUTE_CAT_SERVICE ;
            }
            else
            {
               primaryID.columns.serviceID = MSG_ROUTE_SHARD_SERVCIE ;
            }
            groupPtr->updatePrimary( primaryID, TRUE ) ;
         }
   ...
   INT32 _clsRemoteResource::updateGroupInfo( UINT32 groupID,
                                              CoordGroupInfoPtr &groupPtr,
                                              _pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::updateGroupInfo( const CHAR *groupName,
                                              CoordGroupInfoPtr &groupPtr,
                                              _pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::updateGroupsInfo( GROUP_VEC &vecGroupPtr,
                                               pmdEDUCB *cb,
   ...
   INT32 _clsRemoteResource::updateGroupList( CoordGroupList &groupList,
                                              pmdEDUCB *cb,
                                              const BSONObj *pCondObj,
                                              BOOLEAN exceptCata,
                                              BOOLEAN exceptCoord,
   ...
   void _clsRemoteResource::removeGroupInfo( UINT32 groupID )
   {
      ossScopedLock lock( &_nodeMutex, EXCLUSIVE ) ;
   ...
   void _clsRemoteResource::removeGroupInfo( const CHAR *groupName )
   {
      MAP_GROUP_NAME_IT itName ;
      ossScopedLock lock( &_nodeMutex, EXCLUSIVE ) ;
   ...
   void _clsRemoteResource::addGroupInfo( CoordGroupInfoPtr &groupPtr,
                                          BOOLEAN inheritStat )
   {
   ...
   CoordGroupInfoPtr _clsRemoteResource::getCataGroupInfo()
   {
      ossScopedLock lock( &_nodeMutex, SHARED ) ;
      return _cataGroupInfo ;
   }

   CoordGroupInfoPtr _clsRemoteResource::getOmGroupInfo()
   {
      ossScopedLock lock( &_nodeMutex, SHARED ) ;
      return _omGroupInfo ;
   }

   INT32 _clsRemoteResource::groupID2Name( UINT32 id, string &name )
   {
      INT32 rc = SDB_OK ;
      MAP_GROUP_INFO_IT it ;
   ...
   INT32 _clsRemoteResource::groupName2ID( const CHAR *name, UINT32 &id )
   {
      INT32 rc = SDB_OK ;
      MAP_GROUP_NAME_IT itName ;
   ...
   void _clsRemoteResource::_clearGroupName( UINT32 groupID )
   {
   ...
   void _clsRemoteResource::_addGroupName( const string &name, UINT32 id )
   {
      _mapGroupName[name] = id ;
   }

   void _clsRemoteResource::getCataNodeAddrList( CoordVecNodeInfo &vecCata )
   {
      ossScopedLock lock( &_nodeMutex, SHARED ) ;
      vecCata = _cataNodeAddrList ;
   }

   INT32 _clsRemoteResource::_updateCataGroupInfoByAddr( _pmdEDUCB *cb,
                                                         CoordGroupInfoPtr &groupPtr )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::_processGroupReply( MsgHeader *pMsg,
                                                 CoordGroupInfoPtr &groupPtr )
   {
      INT32 rc = SDB_OK ;
      CoordGroupInfo *pGroupInfo = NULL ;
   ...
   INT32 _clsRemoteResource::_processGroupContextReply( INT64 &contextID,
                                                        GROUP_VEC &vecGroupPtr,
                                                        pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::_updateCataGroupInfo( pmdEDUCB *cb,
   ...
   INT32 _clsRemoteResource::updateCataGroupInfo( CoordGroupInfoPtr &groupPtr,
                                                  pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::_updateRouteInfo( const CoordGroupInfoPtr &groupPtr,
   ...
   INT32 _clsRemoteResource::_updateGroupInfo( MsgHeader *pMsg,
                                               pmdEDUCB *cb,
                                               MSG_ROUTE_SERVICE_TYPE type,
                                               CoordGroupInfoPtr &groupPtr )
   {
   ...
   INT32 _clsRemoteResource::getCataInfo( const CHAR *collectionName,
   ...
   void _clsRemoteResource::removeCataInfo( const CHAR *collectionName )
   {
   ...
   {
      ossScopedLock _lock( &_cataMutex, EXCLUSIVE ) ;
      MAP_CATA_INFO_IT iter = _mapCataInfo.find( collectionName ) ;
      if ( iter != _mapCataInfo.end() )
      {
         removedCataPtr = iter->second ;
         _mapCataInfo.erase( iter ) ;
      }
   }

   void _clsRemoteResource::removeCataInfoWithMain( const CHAR *collectionName )
   {
                                                    ...
                                                    CoordCataInfoPtr &removedMainCataPtr )
   {
      ...
      if ( it != _mapCataInfo.end() )
      {
         removedCataPtr = it->second ;
         pCatSet = removedCataPtr->getCatalogSet() ;
         _mapCataInfo.erase( it ) ;
         if ( pCatSet && pCatSet->isSubCL() )
         {
            it = _mapCataInfo.find( pCatSet->getMainCLName().c_str() ) ;
            if ( it != _mapCataInfo.end() )
            {
               removedMainCataPtr = it->second ;
               _mapCataInfo.erase( it ) ;
            }
         }
      }
   ...
   void _clsRemoteResource::removeCataInfoByCS( const CHAR *csName,
                                                BOOLEAN needRemoveRelated )
   {
      UINT32 len = 0 ;
      clsCatalogSet *pCatSet = NULL ;
      ...
      while( it != _mapCataInfo.end() )
      {
         ...
         {
            if ( needRemoveRelated && pCatSet && pCatSet->isSubCL() )
            {
               const CHAR *mainCLName = pCatSet->getMainCLName().c_str() ;
               if ( 0 == ossStrncmp( mainCLName, csName, len ) &&
                    '.' == mainCLName[ len ] )
               {
                  _mapCataInfo.erase( it ++ ) ;
                  continue ;
   ...
   INT32 _clsRemoteResource::_addCataInfo( const BSONObj &cataObj,
                                           CoordCataInfoPtr &cataPtr )
   {
      INT32 rc = SDB_OK ;

      /// init catalog info
      rc = coordInitCataPtrFromObj( cataObj, cataPtr,
                                    _getCataInfoParseGroupID() ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to init catalog info from obj[%s], "
                    "rc: %d", cataObj.toPoolString().c_str(), rc ) ;

      /// add to catalog map
      rc = _addCataInfo( cataPtr ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to add catalog info, rc: %d", rc ) ;
   ...
   INT32 _clsRemoteResource::_addCataInfo( CoordCataInfoPtr &cataPtr )
   {
      INT32 rc = SDB_OK ;
      ...
      {
         PD_LOG( PDERROR, "Failed to add catalog info, occur exception %s",
                 e.what() ) ;
         rc = ossException2RC( &e ) ;
         goto error ;
      }
   ...
   INT32 _clsRemoteResource::addCataInfo( const BSONObj &cataObj )
   {
      INT32 rc = SDB_OK ;

      CoordCataInfoPtr tmpPtr ;

      rc = _addCataInfo( cataObj, tmpPtr ) ;
      PD_RC_CHECK( rc, PDERROR, "Failed to add catalog info, rc: %d", rc ) ;
   ...
   UINT32 _clsRemoteResource::checkAndRemoveCataInfoBySub( const CHAR *collectionName )
   {
      string strSubCLName = collectionName ;
      MAP_CATA_INFO_IT it ;
      clsCatalogSet *pCatSet = NULL ;
   ...
   void _clsRemoteResource::invalidateStrategy()
   {
      if ( _pOmStrategyAgent )
      {
   ...
   void _clsRemoteResource::invalidateDataSourceInfo( const CHAR *name )
   {
      if ( _pDataSourceMgr )
      {
         if ( name )
   ...
   void _clsRemoteResource::invalidateCataInfo( const CHAR *clFullName )
   {
      ossScopedLock _lock( &_cataMutex, EXCLUSIVE ) ;
   ...
   void _clsRemoteResource::invalidateGroupInfo( UINT64 identify )
   {
      ossScopedLock _lock( &_nodeMutex, EXCLUSIVE ) ;
   ...
   INT32 _clsRemoteResource::_processCatalogReplyByCLUID(
                                                    MsgHeader *pMsg,
                                                    CoordCataInfoPtr &cataPtr )
   {
      INT32 rc = SDB_OK ;
      ...
      PD_CHECK( 1 == pReply->numReturned, SDB_INVALIDARG, error, PDERROR,
                "Reply obj must 1" ) ;
      ...
      while ( offset < pMsg->messageLength )
      {
         ...
         try
         {
            ...
            rc = addCataInfo( obj, tmpPtr ) ;
            PD_RC_CHECK( rc, PDERROR, "Failed to add catalog info [%s], "
                         "rc: %d", obj.toPoolString().c_str(), rc ) ;
   ...
   INT32 _clsRemoteResource::_updateCataInfoByCLUID( utilCLUniqueID clUID,
                                                     CoordCataInfoPtr &cataPtr,
                                                     pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
      ...
      try
      {
         obj = BSON( CAT_CL_UNIQUEID << (INT64)clUID ) ;
      }
      catch( std::exception &e )
      {
         PD_LOG( PDERROR, "Occur exception: %s", e.what() ) ;
   ...
   retry:
      ...
      /// process reply
      pReply = pSub->getRspMsg() ;
      rc = _processCatalogReplyByCLUID( pReply, cataPtr ) ;
      if ( rc )
      {
         coordGroupSessionCtrl *pGroupCtrl = session.getGroupCtrl() ;
   ...
   INT32 _clsRemoteResource::updateCataInfoByCLUID( utilCLUniqueID clUID,
                                                    CoordCataInfoPtr &cataPtr,
                                                    pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
      ...
      rc = _updateCataInfoByCLUID( clUID, tmpCataPtr, cb ) ;
      if ( rc )
      {
         PD_LOG( PDERROR, "Update collection[%llu]'s catalog info failed, "
                 "rc: %d", clUID, rc ) ;
         goto error ;
      }
      ...
      if ( tmpCataPtr->isMainCL() && tmpCataPtr->getSubCLCount() > 0 )
      {
         ...
         else
         {
            ...
            try
            {
               obj = BSON( CAT_MAINCL_NAME << tmpCataPtr->getName() ) ;
            }
            catch( std::exception &e )
            {
               PD_LOG( PDERROR, "Occur exception: %s", e.what() ) ;
            ...
            if ( rc )
            {
               PD_LOG( PDERROR, "Update collection[%s]'s all sub-collections "
                       "catalog info failed, rc: %d",
                       tmpCataPtr->getName(), rc ) ;
               /// remove main-cl's cataloginfo
               removeCataInfo( tmpCataPtr->getName() ) ;
               goto error ;
            }
         }
      }
   ...
   INT32 _clsRemoteResource::updateCataInfo( const CHAR *collectionName,
                                             CoordCataInfoPtr &cataPtr,
                                             pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::getOrUpdateCataInfo( const CHAR *collectionName,
                                                  CoordCataInfoPtr &cataPtr,
                                                  pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::getOrUpdateGroupInfo( UINT32 groupID,
                                                   CoordGroupInfoPtr &groupPtr,
                                                   pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::getOrUpdateGroupInfo( const CHAR *groupName,
                                                   CoordGroupInfoPtr &groupPtr,
                                                   pmdEDUCB *cb )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _clsRemoteResource::_updateCataInfo( const BSONObj &obj,
   ...
   INT32 _clsRemoteResource::_processCatalogReply( MsgHeader *pMsg,
                                                   const CHAR *collectionName,
                                                   CoordCataInfoPtr &cataPtr )
   {
      INT32 rc = SDB_OK ;
      ...
      if ( rc )
      {
         PD_LOG( PDERROR, "Received unexpected reply for catalog info "
                 "request from node[%s], flag: %d",
                 routeID2String( pMsg->routeID ).c_str(), rc ) ;
         goto error ;
      }
      ...
      while ( offset < pMsg->messageLength )
      {
         ...
         try
         {
            ...
            PD_RC_CHECK( rc, PDERROR, "Failed to add catalog info from "
                         "obj [%s], rc: %d", obj.toPoolString().c_str(), rc ) ;
      ...
      if ( objNum < 1 || isSpec )
      {
         PD_LOG( PDERROR, "Received unexpected reply for catalog info from "
   ...
   void _clsRemoteResource::updateNodeStat( const MsgRouteID &nodeID, INT32 rc )
   {
      CoordGroupInfoPtr groupPtr ;
