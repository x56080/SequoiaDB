                         ...
                         sdbCollectionSpace &cs)
{
   ...
   if(SDB_DMS_CS_NOTEXIST == rc)
   {
      ...
      if(SDB_OK != rc)
      {
          ossPrintf("Failed to create collectionspace:%s, error=%d"OSS_NEWLINE,
                    csName, rc);
          goto error;
      }
   }
   ...
   {
      ossPrintf("Failed to get collectionspace:%s, error=%d"OSS_NEWLINE,
                csName, rc);
      goto error;
   }
...
INT32 sequoiaFS::init(INT32 argc, CHAR **argv, vector<string> *options4fuse)
{
   ...
   if(SDB_OK != rc)
   {
      ossPrintf("Failed to get collection, cl=%s, error=%d, exit."OSS_NEWLINE,
                this->_collection.c_str(), rc);
      rc = -ENOENT;
      goto error;
   }
