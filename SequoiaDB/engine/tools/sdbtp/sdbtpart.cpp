/*******************************************************************************
   ...
   Source File Name = sdbtpart.cpp

   Descriptive Name = sdbtpart main

   When/how to use: this program may be used on binary and text-formatted
   versions of PMD component. This file contains main function for sdbtpart,
   which is used to start SequoiaDB Time Protocol Service.
...
namespace engine
{
   ...
   #define SDBTPART_LOG_FILE_NAME   "sdbtpart.log"
   #define SDBTPART_OPTION_OPTIONS  "options"
   ...
   #define COMMANDS_OPTIONS \
       ...
       ( SDBTPART_OPTION_OPTIONS, value<string>(), "options" )
#else
   #define COMMANDS_OPTIONS \
       ( PMD_COMMANDS_STRING( PMD_OPTION_HELP, ",h"), "help" ) \
       ( PMD_OPTION_VERSION, "version" ) \
                                 ...
                                 string &options )
   {
      ...
      if ( vm.count( SDBTPART_OPTION_OPTIONS ) )
      {
         options = vm[ SDBTPART_OPTION_OPTIONS ].as<string>() ;
   ...
   static INT32 mainEntry( INT32 argc, CHAR **argv )
   {
      ...
      CHAR sdbtpPathName[ OSS_MAX_PATHSIZE + 1 ] = { 0 } ;
      ...
      /// binary path
      rc = utilBuildFullPath( rootPath, SDBTP_NAME, OSS_MAX_PATHSIZE,
                              sdbtpPathName ) ;
      if ( SDB_OK != rc )
      {
      ...
      rc = utilCatPath( dialogFile, OSS_MAX_PATHSIZE, SDBTPART_LOG_FILE_NAME ) ;
      if ( SDB_OK != rc )
      {
      ...
      // first check
      rc = utilGetServiceByConfigPath( configPath, SDBTP_CFG_FILE_NAME,
           ...
           serviceExists( svcname.c_str(), info ) )
      {
         ossPrintf( "Success: %s(%s) is already started (%d)"OSS_NEWLINE,
                    utilDBTypeStr( SDB_TYPE_TP ), svcname.c_str(),
                    info._pid ) ;
         goto done ;
      }

      // start node
      buildListArgs( sdbtpPathName, force, configPath, options, runCmd ) ;
      ...
      if ( SDB_OK != tmpRC )
      {
         ...
         ossPrintf( "Error: Start %s(%s) failed, rc: %d(%s)"OSS_NEWLINE,
                    utilDBTypeStr( SDB_TYPE_TP ), svcname.c_str(), tmpRC,
                    getErrDesp( rc ) ) ;
         goto error ;
      }
      ...
      else
      {
         ...
         ossPrintf( "Error: Start %s(%s) failed, rc: %d(%s)"OSS_NEWLINE,
                    utilDBTypeStr( SDB_TYPE_TP ), svcname.c_str(), rc,
