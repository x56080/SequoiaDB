/*******************************************************************************


   Copyright (C) 2011-2018 SequoiaDB Ltd.

   This program is free software: you can redistribute it and/or modify
   it under the terms of the GNU Affero General Public License as published by
   the Free Software Foundation, either version 3 of the License, or
   (at your option) any later version.

   This program is distributed in the hope that it will be useful,
   but WITHOUT ANY WARRANTY; without even the implied warranty of
   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
   GNU Affero General Public License for more details.

   You should have received a copy of the GNU Affero General Public License
   along with this program.  If not, see <http://www.gnu.org/licenses/>.

   Source File Name = liteCacheConsole.h

   Descriptive Name =

   Dependencies: N/A

   Restrictions: N/A

   Change Activity:
   defect Date        Who Description
   ====== =========== === ==============================================
          09/08/2020  WY  Initial Draft

   Last Changed =

******************************************************************************/

#ifndef VESSEL_LITE_CACHE_CONSOLE_H_
#define VESSEL_LITE_CACHE_CONSOLE_H_

#include "vessel/liteCache.h"

namespace engine
{
namespace vessel
{
   class liteCacheConsole : public SDBObject
   {
      public:
         liteCacheConsole();
         ~liteCacheConsole();

         liteCacheConsole(const liteCacheConsole &) = delete;
         liteCacheConsole &operator=(const liteCacheConsole &) = delete;

      public:
         void fini();
         INT32 init32KBCache(const liteCacheOptions &o);

      public:
         INT32 allocate(requestContext *context,
                        const GLOBAL_PAGE_ID &gpid,
                        const liteCacheAllocateOptions &options,
                        liteCacheTuple &tuple);

         INT32 allocateToReset(requestContext *context,
                               const GLOBAL_PAGE_ID &gpid,
                               liteCacheTuple &tuple);

      private:
         liteCache *getCache(UINT32 pageSize);
      private:
         liteCache _32KBCache;
         liteCache _64KBCache;
         
   };//class liteCacheConsole
}//namespace vessel
}//namespace engine

#endif//VESSEL_LITE_CACHE_CONSOLE_H_