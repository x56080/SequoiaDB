...
namespace engine
{
                      ...
                      SDB_DPSCB * dpsCB, INT16 w )
   {
      ...
      try
      {
         ...
         BSONObjBuilder updateBuild( CAT_BUILDER_INIT_SIZE ) ;
         BSONObjBuilder sub( updateBuild.subobjStart("$addtoset") ) ;
         ...
         sub1.append( newCLObj ) ;
         sub1.done() ;

         sub.done() ;

         BSONObjBuilder setBuilder( updateBuild.subobjStart( "$set" ) ) ;
         ...
         updator = updateBuild.obj() ;
      }
      catch ( exception &e )
      {
         PD_LOG( PDERROR, "Failed to build set updator, occur exception %s",
                 e.what() ) ;
                      ...
                      BOOLEAN *pAddNewIdx, BSONObj *pIndexObj )
   {
      ...
      rc = catFormatIndexInfo( collectionName, indexDef, cb, obj ) ;
      PD_RC_CHECK( rc, PDERROR,
                               ...
                               INT16 w )
   {
      ...
      SDB_DMSCB *dmsCB = krcb->getDMSCB() ;
      SDB_DPSCB *dpsCB = krcb->getDPSCB() ;
      BSONObj dummy, updator, match ;

      match = BSON( CAT_TASKID_NAME << (INT64)taskID ) ;
      ...
      updator = BSON( "$set" <<
                      BSON( FIELD_NAME_STATUS << CLS_TASK_STATUS_FINISH <<
                            ...
                            FIELD_NAME_RESULTCODE << resultCode <<
                            FIELD_NAME_RESULTCODEDESC << getErrDesp(resultCode) <<
                            FIELD_NAME_ENDTIMESTAMP << timeStr ) ) ;

      rc = rtnUpdate( CAT_TASK_INFO_COLLECTION, match, updator, dummy, 0,
                      cb, dmsCB, dpsCB, w ) ;
      PD_RC_CHECK( rc, PDERROR, "Update task[%llu] result code to [%d] failed, "
                   "rc: %d", taskID, resultCode, rc ) ;
   done:
                                    ...
                                    _pmdEDUCB *cb, INT16 w )
   {
      ...
      if ( Object == ele.type() )
      {
         ...
         rc = catCreateAutoIncSequence( objSeq, autoIncSet, cb, w ) ;
         if( SDB_OK != rc )
         {
            goto error ;
         }
         ...
         /// builder new auto-inc sequence obj
         {
            ...
            while( itr.more() )
            {
               ...
               if ( 0 == ossStrcmp( e.fieldName(), CAT_SEQUENCE_CURRENT_VALUE ) ||
                    ...
                    0 == ossStrcmp( e.fieldName(), FIELD_NAME_CL_UNIQUEID ) )
               {
                  /// skip
                  continue ;
               }
                                         ...
                                         BSONObj &catalog, pmdEDUCB *cb )
   {
      ...
      try
      {
         ...
         if ( UTIL_INVALID_DS_UID != dsID )
         {
            ...
            rc = catBuildCatalogRecord( cb, clInfo, mask, 0, grpList, catalog, 1 ) ;
