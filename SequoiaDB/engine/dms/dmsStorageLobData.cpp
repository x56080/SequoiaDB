   ...
   */
   ...
   INT32 _dmsStorageLobData::extend( INT64 len )
   {
      ...
      do
      {
         ...
         {
            PD_LOG( PDWARNING, "failed to extend file:%d", rc ) ;
            goto error ;
         }
         else
         {
   ...
   // PD_TRACE_DECLARE_FUNCTION ( SDB_DMSSTORAGELOBDATA_FILETRUNCATE, "_dmsStorageLobData::_fileTruncate" )
   INT32 _dmsStorageLobData::_fileTruncate( UINT64 truncateSize )
   {
      INT32 rc = SDB_OK ;
      PD_TRACE_ENTRY( SDB_DMSSTORAGELOBDATA_FILETRUNCATE ) ;

      truncateSize += sizeof( _dmsStorageUnitHeader ) ;
      if ( ( truncateSize - sizeof( _dmsStorageUnitHeader ) ) % getSegmentSize() != 0 )
      {
         rc = SDB_INVALIDARG ;
         PD_LOG( PDERROR, "giving truncate file size[%lld] is invalid, rc:%d",
         truncateSize, rc ) ;
         goto error ;
      }

      rc = ossTruncateFile( &_file, truncateSize ) ;
      if ( rc )
      {
      ...
      _fileSz = truncateSize ;

   done:
      PD_TRACE_EXITRC( SDB_DMSSTORAGELOBDATA_FILETRUNCATE, rc ) ;
      return rc ;
   error:
      goto done ;
   }

   // PD_TRACE_DECLARE_FUNCTION ( SDB_DMSSTORAGELOBDATA_FILEFALLOCATE, "_dmsStorageLobData::_fileFallocate" )
   INT32 _dmsStorageLobData::_fileFallocate( UINT32 mode, UINT64 offset, UINT64 size )
   {
      INT32 rc = SDB_OK ;
      PD_TRACE_ENTRY( SDB_DMSSTORAGELOBDATA_FILEFALLOCATE ) ;

      offset += sizeof( _dmsStorageUnitHeader ) ;
      rc = ossFallocate( &_file, mode, offset, size ) ;
      if ( rc )
      {
         PD_LOG( PDERROR, "failed to fallocate file[%s], rc:%d",
                 _fileName.c_str(), rc ) ;
         goto error ;
      }

   done:
      PD_TRACE_EXITRC( SDB_DMSSTORAGELOBDATA_FILEFALLOCATE, rc ) ;
      return rc ;
   error:
      goto done ;
   }
