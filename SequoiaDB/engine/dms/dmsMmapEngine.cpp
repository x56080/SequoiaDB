...
namespace engine
{
   ...
   */
                                   ...
                                   DMS_SU_DESCRIPTOR & desc )
   {
      ...
      INT32 pageSize = options.dataPageSize;
      INT32 lobPageSize = options.lobdPageSize;
      ...
      /// add collctionspace
      rc = addCollectionSpace( pCollectionSpace, 1, su, cb, options.dpsCB, TRUE, desc );
      if ( rc )
      {
   ...
   void _dmsMmapEngine::_CSCBRelease ( dmsStorageUnitID suID,
                                   OSS_LATCH_MODE lockType )
   {
      if ( EXCLUSIVE == lockType )
      {
   ...
   INT32 _dmsMmapEngine::_CSCBNameRemoveP1 ( const CHAR *pName,
                                         _pmdEDUCB *cb,
                                         SDB_DPSCB *dpsCB )
   {
  ...
  error :
                                   ...
                                   const bson::BSONObj &adjunct )
   {
      ...
      if ( DMS_STORAGE_CAPPED != su->type() &&
           OSS_BIT_TEST( o.attributes, DMS_MB_ATTR_CAPPED ) )
      {
      ...
      rc = su->data()->addCollection( clShortName, &collectionID, clUniqueID, o.attributes, cb,
                                       o.dpsCB, 0, o.sysCall, o.compressor, &logicalID, o.extOptions,
      ...
      {
         rc = su->createIndex( clShortName, shardIdxDef, cb, o.dpsCB, TRUE, NULL,
      ...
      PD_LOG( PDEVENT, "Create collection[name: %s, id: %llu] succeed, "
              ...
              attrStr, o.attributes,
              utilCompressType2String( (UINT8)o.compressor ), o.compressor,
                                   ...
                                   const dmsRemoveCLOptions &o )
   {
      ...
      if ( UTIL_UNIQUEID_NULL != o.clUniqueID )
      {
         ...
         PD_CHECK( mbContext->mb()->_clUniqueID == o.clUniqueID,
                   SDB_DMS_NOTEXIST, error, PDWARNING,
                   "Collection %s with unique ID %llu had been dropped, "
                   "current unique ID is %llu", clFullName,
                   o.clUniqueID, mbContext->mb()->_clUniqueID ) ;
      }

      rc = su->data()->dropCollection( clShortName, cb, o.dpsCB, TRUE, mbContext, o.recycleOptions );
      if ( rc )
      {
   ...
   void _dmsMmapEngine::suUnlock ( dmsStorageUnitID suID,
                               OSS_LATCH_MODE lockType )
   {
                                             ...
                                             DMS_SU_DESCRIPTOR &desc )
   {
      ...
      if (SDB_OK == rc )
      {
         ...
         desc = makeSharedPtrFromPool<dmsSuDescriptor>( getEngineType(), pName, csUniqueID, suLID );
         // write dps
         if ( dpsCB )
         {
   ...
   INT32 _dmsMmapEngine::_delCollectionSpaceP1 ( const CHAR *pName, _pmdEDUCB *cb,
                                             SDB_DPSCB *dpsCB,
                                             BOOLEAN removeFile )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 _dmsMmapEngine::_delCollectionSpaceP1Cancel ( const CHAR *pName, _pmdEDUCB *cb,
                                                   SDB_DPSCB *dpsCB )
   {
      INT32 rc = SDB_OK ;
      IDmsExtDataHandler *extHandler = NULL ;
   ...
   INT32 _dmsMmapEngine::_delCollectionSpaceP2 ( const CHAR *pName, _pmdEDUCB *cb,
                                             SDB_DPSCB *dpsCB,
                                             BOOLEAN removeFile,
                                             dmsDropCSOptions *options )
   {
   ...
   INT32 _dmsMmapEngine::dropCollectionSpaceP1 ( const CHAR *pName, _pmdEDUCB *cb,
                                             SDB_DPSCB *dpsCB )
   {
   ...
   INT32 _dmsMmapEngine::dumpInfo( MON_CL_SIM_LIST &collectionList,
                               BOOLEAN sys )
   {
   ...
   void _dmsMmapEngine::clearSUCaches ( const MON_CS_SIM_LIST &monCSList,
                                    UINT32 mask )
   {
   ...
   void _dmsMmapEngine::changeSUCaches ( const MON_CS_SIM_LIST &monCSList,
                                     UINT32 mask )
   {
