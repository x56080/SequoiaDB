...
//               }
   ...
   void _dmsSchemaWriter::_setColRecordOffset( UINT16 columnID, UINT16 offset )
   {
      INT32* slot = (INT32 *)
   ...
   INT32 _dmsSchemaHashWriter::addColumnItem( const CHAR *name, UINT16 columnID )
   {
      ...
      while ( TRUE )
      {
         item = (INT32 *)_offset2Ptr( nextItemOffset ) ;
   ...
   INT32 _dmsInternalSchemaWriter::save( dmsMBContext *context )
   {
      INT32 rc = SDB_OK ;
