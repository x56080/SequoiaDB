...
class _mongoDatabaseCommand : public _mongoCommand
{
   ...
   protected:
      ...
      virtual INT32 _postFirstBatch( BSONObjBuilder &cursorBuilder ) { return SDB_OK ; }
...
class _mongoCollectionCommand : public _mongoCommand
{
   ...
   public:
      ...
      virtual INT32 _postFirstBatch( BSONObjBuilder &cursorBuilder ) ;
...
class _mongoDeleteCommand : public _mongoCollectionCommand
{
   MONGO_DECLARE_CMD_AUTO_REGISTER()
   public:
      _mongoDeleteCommand() : _hasBuildMsgVec( FALSE ),
                              _hasProcessAllMsg( FALSE ), _msgIndex( 0 ),
                              _deletedNum( 0 )
                              {}
...
class _mongoUpdateCommand : public _mongoCollectionCommand
{
   MONGO_DECLARE_CMD_AUTO_REGISTER()
   public:
      _mongoUpdateCommand() : _isUpsert( FALSE ), _hasBuildMsgVec( FALSE ),
                              _hasProcessAllMsg( FALSE ), _msgIndex( 0 ),
                              _insertedNum( 0 ), _updatedNum( 0 ),
                              _modifiedNum( 0 )
                              {}
   ...
   private:
      ...
      BSONObj _idObj ;
      vector<BSONElement> _msgVec ;
      ossPoolMap<INT32, BSONObj> _idObjMap ;
} ;
...
class _mongoCreateIdxCommand : public _mongoCollectionCommand
{
   MONGO_DECLARE_CMD_AUTO_REGISTER()
   public:
      _mongoCreateIdxCommand() : _hasBuildMsgVec( FALSE ),
                                 _hasProcessAllMsg( FALSE ), _msgIndex( 0 )
                                 { _msgVec.clear() ; }
...
{
   enum { STATE_NONE, STATE_LIST, STATE_DROP } ;
   ...
   public:
      ...
      virtual BOOLEAN hasProcessAllMsg() const ;
      virtual BOOLEAN needProcessByEngine() const ;

   protected:
      INT32             _pushIndex( const CHAR *pIndexName,
                                    BOOLEAN ignoreSys,
