/*******************************************************************************
   ...
   Source File Name = omConfigMariaDB.cpp
...
namespace engine
{
   ...
   INT32 OmMariaDBNode::_init( const BSONObj& bsonNode,
   ...
   INT32 OmMariaDBNode::_setServiceName( const string& serviceName )
   {
      INT32 rc = SDB_OK ;
      ...
      if ( rc )
      {
         PD_LOG( PDERROR, "failed to add used port to node, port=%s",
                 serviceName.c_str() );
         goto error ;
      }
   ...
   INT32 OmMariaDBNode::_setDBPath( const string& dbPath, OmHost& host )
   {
      INT32 rc = SDB_OK ;
   ...
   OmMariaDBConfTemplate::OmMariaDBConfTemplate()
   {
   }
   ...
   OmMariaDBConfProperties::OmMariaDBConfProperties()
   {
   }
   ...
   bool OmMariaDBConfProperties::isPrivateProperty( const string& name ) const
   {
   ...
   bool OmMariaDBConfProperties::isAllPropertySet()
   {
   ...
   public:
      ...
      {
         if ( OM_BUSINESS_SEQUOIASQL_MARIADB !=
   ...
   public:
      int operator() ( const OmHost *host1, const OmHost *host2 )
      {
         ...
         INT32 roleNum1 = host1->count( _byMariaDBBuz ) ;
         INT32 roleNum2 = host2->count( _byMariaDBBuz ) ;
   ...
   private:
      byMariaDBBuz _byMariaDBBuz ;
   } ;

   class getBestDiskForMariaDB
   {
   public:
      getBestDiskForMariaDB( const OmHost &host ) : _host ( host )
      {
      }
   ...
   OmMariaDBConfigBuilder::OmMariaDBConfigBuilder(
   ...
   INT32 OmMariaDBConfigBuilder::_build( BSONArray& nodeConfig )
   {
      INT32 rc = SDB_OK ;
      ...
      try
      {
         ...
         {
            OmMariaDBNode* node = NULL ;
            BSONObjBuilder builder ;

            node = dynamic_cast<OmMariaDBNode*>( *iter ) ;
            if( NULL == node )
            {
               rc = SDB_SYS ;
               PD_LOG_MSG( PDERROR, "failed to get node info" ) ;
               goto error ;
            }
   ...
   void OmMariaDBConfigBuilder::_setLocal()
   {
   ...
   INT32 OmMariaDBConfigBuilder::_createNode()
   {
      INT32 rc = SDB_OK ;
      OmMariaDBNode* node = NULL ;
      OmHost* host = NULL ;
      ...
      host = _cluster.chooseHost( getBestHostForMariaDB() ) ;
      ...
      disk = host->chooseDisk( getBestDiskForMariaDB( *host ) ) ;
      if ( NULL == disk )
      {
         rc = SDB_SYS ;
         PD_LOG_MSG( PDERROR, "create node failed: no disk, hostname=%s",
      ...
      if ( rc )
      {
         PD_LOG( PDERROR, "failed to get service port, hostname=%s",
                 host->getHostName().c_str() ) ;
         goto error ;
      }
      ...
      if ( rc )
      {
         PD_LOG( PDERROR, "failed to get db path, hostname=%s",
                 host->getHostName().c_str() ) ;
         goto error ;
      }

      node = SDB_OSS_NEW OmMariaDBNode() ;
      if ( NULL == node )
      {
         rc = SDB_OOM ;
      ...
      if ( SDB_OK != rc )
      {
         PD_LOG( PDERROR, "failed to set db path, hostname=%s",
                 host->getHostName().c_str() ) ;
         goto error ;
      }
      ...
      if ( SDB_OK != rc )
      {
         PD_LOG( PDERROR, "failed to set service name, service name=%s",
                 serviceName.c_str() ) ;
         goto error ;
      }
   ...
   INT32 OmMariaDBConfigBuilder::_getServiceName( const OmHost& host,
                                                   string& serviceName )
   {
      INT32 rc = SDB_OK ;
   ...
   #define OM_DBPATH_MARIADB "mariadb"
   INT32 OmMariaDBConfigBuilder::_getDBPath( OmHost& host,
      ...
      do 
      {
         ...
         if ( FALSE == isRootPath )
         {
            ss << OM_BUSINESS_SEQUOIASQL << OSS_FILE_SEP
               << OM_DBPATH_MARIADB << OSS_FILE_SEP ;
         }
   ...
   INT32 OmMariaDBConfigBuilder::_check( BSONObj& bsonConfig )
   {
      INT32 rc = SDB_OK ;
   ...
   INT32 OmMariaDBConfigBuilder::_checkAndAddNode( const BSONObj& bsonNode )
   {
      INT32 rc = SDB_OK ;
      OmMariaDBNode* node = NULL ;
      OmHost* host = NULL ;
   ...
   bool OmMariaDBConfigBuilder::_isServiceNameUsed( const OmHost& host,
                                                     const string& serviceName )
   {
