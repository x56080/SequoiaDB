/*******************************************************************************
   ...
   Source File Name = qgmOptiUpdate.cpp
...
namespace engine
{
   INT32 qgmOptiUpdate::_checkPrivileges( ISession *session ) const
   {
      INT32 rc = SDB_OK;
...

      {
         ...
         rc = session->checkPrivilegesForActionsOnExact( _collection.toString().c_str(),
                                                         actions );
