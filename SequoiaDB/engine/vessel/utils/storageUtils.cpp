...
namespace vessel
{
                                  ...
                                  bitmapVector &bitmap)
   {
      ...
      if (OSS_UNLIKELY(0 == impPtr ||
                       0 == bitmap.getBitsCapcity()))
      {
         rc = SDB_INVALIDARG;
         goto error;
      }
      ...
      {
         BOOLEAN alreadyNoFree = FALSE;
         const idMapSlot *slot = getIdMapSlot(impPtr, i);
         if (OSS_UNLIKELY(NULL == slot))
         {
            PD_LOG(PDERROR, "failed to get slot[%d]", i);
            rc = SDB_VESSEL_INTERNAL_ERR;
            goto error;
         }
         ...
         rc = bitmap.setNoFree(slot->pid, alreadyNoFree);
         if (SDB_OK != rc)
         {
            PD_LOG(PDERROR, "failed to set bitmap:%d", rc);
            goto error;
         }

         if (alreadyNoFree)
         {
            PD_LOG(PDERROR, "physical page[%d] already set as no free", slot->pid);
            rc = SDB_VESSEL_INTERNAL_ERR;
            goto error;
         }
      }
   ...
   {
      INT32 rc = SDB_OK;
      PAGE_ID baseLpid = INVALID_PAGE_ID;

      if (OSS_UNLIKELY(INVALID_PAGE_ID == pid ||
                       0 == impPtr ||
                       0 == lpids.getBitsCapcity() ||
                       0 == ppids.getBitsCapcity()))
      {
         rc = SDB_INVALIDARG;
         goto error;
      }

      baseLpid = pid * ID_MAP_PAGE_CAPACITY;
      for (UINT32 i = 0; i < ID_MAP_PAGE_CAPACITY; ++i)
      {
         BOOLEAN alreadyNoFree = FALSE;
         const idMapSlot *slot = getIdMapSlot(impPtr, i);
         ...
         rc = lpids.setNoFree(baseLpid + i, alreadyNoFree);
         if (SDB_OK != rc)
         {
            PD_LOG(PDERROR, "failed to set bitmap:%d", rc);
            goto error;
         ...
         if (alreadyNoFree)
         {
            PD_LOG(PDERROR, "logical page[%d] already set as no free", baseLpid + i);
            rc = SDB_VESSEL_INTERNAL_ERR;
            goto error;
         }

         alreadyNoFree = FALSE;
         rc = ppids.setNoFree(slot->pid, alreadyNoFree);
         if (SDB_OK != rc)
         {
            PD_LOG(PDERROR, "failed to set bitmap:%d", rc);
            goto error;
