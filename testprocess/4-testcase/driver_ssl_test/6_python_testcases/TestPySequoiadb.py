"""
   ...
   Source File Name = TestPySequoiadb.py
...
def initialize():
   ...
   try:
      sdb = client(hostname,service,ssl=True)
      #print 'connection'
...
class TestPySequoiadb(object):
   ...
   def test_user(self):
      ...
      try:
         ...
         #use new user login
         sdb_new = client(hostname,service,username,password,ssl=True)
   ...
   def test_partition(self):
      try:
         ...
         temp_sdb = client(hostname,data_port,ssl=True)
   ...
   def test_cursor(self):
      try:
         test_sdb1 = client(hostname,service,ssl=True)
         test_sdb2 = client(hostname,service,ssl=True)
