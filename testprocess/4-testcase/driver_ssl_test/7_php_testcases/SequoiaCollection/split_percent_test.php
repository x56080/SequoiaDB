/*******************************************************************************
   ...
   Source File Name = split_percent_test.php
...
*******************************************************************************/
<?php
class split_percent_test extends PHPUnit_Framework_TestCase
{
	...
	* @depends test_whether_cluster
	*/
	public function test_beforesplit_insert()
	{
	   $args_array = func_get_args() ;
	   $sdb = $args_array[0] ;
	   $cl = $args_array[1] ;
	...
	/**
	* @depends testconnect
	* @depends test_beforesplit_insert
	* @depends testgetSourceGroupName
	* @depends testgetOtherDataGroups
	* @depends test_whether_cluster
	*/
	public function test_splitCL_percent()
	{
		$args_array = func_get_args() ;
		print_r( $args_array ) ;
		$sdb = $args_array[0] ;
		...
		if( $isCluster )
		{
   		$this->assertNotEquals( 2, count( $otherDataGroups ) );
   		
   		$Percent = 100 ;
   		$Percent_PerGroup = $Percent / ( count( $otherDataGroups ) + 1 ) ;
   		for( $i = 0; $i < count( $otherDataGroups ) ; ++$i )
   		{
   			echo $Percent_PerGroup.' ' ;
   			$cl->split( $sourceGroupName,  $otherDataGroups[$i], $Percent_PerGroup ) ;
   			$array = $sdb->getError() ;
   			$this->assertEquals( 0, $array['errno'] ) ;
   		}
   		return $cl ;
