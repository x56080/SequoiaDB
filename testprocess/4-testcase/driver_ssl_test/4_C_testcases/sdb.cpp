...
TEST(sdb,sdbConnect_with_usr)
{
   ...
   rc = sdbSecureConnect ( HOST, SERVER, USERDEF, PASSWDDEF, &connection ) ;
   ASSERT_EQ( SDB_OK, rc ) ;
   // Remove a user
...
TEST(sdb,sdbConnect_with_several_address)
{
   ...
   const CHAR* connArr[10] = {"192.168.20.35:12340",
                              ...
                              "localhost:11810"} ;
   // connect to database
   rc = sdbSecureConnect1 ( connArr, 10, USER, PASSWD, &connection ) ;
   ASSERT_EQ( SDB_OK, rc ) ;
...
TEST(sdb, sdbIsClose)
{
   ...
   // connect to database
   rc = sdbSecureConnect ( HOST, SERVER, USER, PASSWD, &connection ) ;
   rc = sdbSecureConnect ( HOST, SERVER, USER, PASSWD, &connection1 ) ;
   ASSERT_EQ( SDB_OK, rc ) ;
   // get cs
   rc = getCollectionSpace ( connection,
                             COLLECTION_SPACE_NAME,
