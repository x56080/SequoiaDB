/*******************************************************************************
   ...
   Source File Name = right_split.php
...
$cl = "" ;
$str = "" ;
$order = empty ( $_POST['order'] ) ? ( empty ( $_GET['order'] ) ? "" : $_GET['order'] ) : $_POST['order'] ;
$groupname = empty ( $_POST['groupname'] ) ? ( empty ( $_GET['groupname'] ) ? "" : $_GET['groupname'] ) : $_POST['groupname'] ;
$nodename = empty ( $_POST['nodename'] ) ? ( empty ( $_GET['nodename'] ) ? "" : $_GET['nodename'] ) : $_POST['nodename'] ;
$csname = empty ( $_POST['csname'] ) ? ( empty ( $_GET['csname'] ) ? "" : $_GET['csname'] ) : $_POST['csname'] ;
$clname = empty ( $_POST['clname'] ) ? ( empty ( $_GET['clname'] ) ? "" : $_GET['clname'] ) : $_POST['clname'] ;
$surl = empty ( $_SESSION['surl'] ) ? $_SERVER['HTTP_REFERER'] : $_SESSION['surl'] ;
if ( $order == "split" )
{
	$source = empty ( $_POST['source'] ) ? ( empty ( $_GET['source'] ) ? "" : $_GET['source'] ) : $_POST['source'] ;
	$dest = empty ( $_POST['dest'] ) ? ( empty ( $_GET['dest'] ) ? "" : $_GET['dest'] ) : $_POST['dest'] ;
	$condition = empty ( $_POST['condition'] ) ? ( empty ( $_GET['condition'] ) ? "" : $_GET['condition'] ) : $_POST['condition'] ;
	if ( $csname != "" && $clname != "" )
	{
		...
		if ( $arr_1['errno'] == 0 )
		{
			...
			if ( $arr_1['errno'] == 0 )
			{
				$cl->split ( $source, $dest, $condition ) ;
				$arr_1 = $db->getError() ;
			}
		}
	}
...
<div class="topb">
	<div class="path">
		...
		{
	    	$rurl = $_SESSION['surl'] ;
		    echo '<script type="text/javascript" language="javascript">window.parent.frames["right"].location.href="'.$rurl.'"; </script>' ;
		...
		?>
         ...
         --> <a href="#" class="pathlink"><img src="../images/db.png" width="15px">&nbsp;切分</a>
	</div>
	<div class="clear"></div>
...
if ( $arr_1  != "" )
{
	if ( $arr_1['errno'] == 0 )
	{
	    $rurl = empty ( $_POST['rurl'] ) ? ( empty ( $_GET['rurl'] ) ? "" : $_GET['rurl'] ) : $_POST['rurl'] ;
		echo '<script type="text/javascript" language="javascript">window.parent.frames["right"].location.href="'.$rurl.'"; </script>' ;
	}
	else
	{
		php_div_s_3($width-250,"margin-left:15px;margin-top:15px;");
		echo '<img src="../images/icons/exclamation.png" width="15px" border="0"> 数据切分失败 错误码: '.$arr_1['errno'] ;
		php_div_e_3();
	}
}
if ( $str != "" )
...
<div style="width:<?php echo $width-206;?>px;margin-top:20px;">
    <div>
        <form action="right_split.php" method="post" class="send">
        ...
        <input type="hidden" name="order" value="split" />
        <?php
		$temp_str = "在" ;
		if ( $csname != "" && $clname != "" )
		{
			$temp_str = $temp_str."<b>".$csname.".".$clname."</b> 切分数据" ;
		}
        ...
        <div style="padding:12px;">
			<?php
                ...
                if ( !empty ( $fromGroup ) )
                {
                    ...
                    {
						if ( $fromGroupArr["Name"] == $csname.".".$clname )
						{
							if ( array_key_exists ( "CataInfo", $fromGroupArr ) )
							{
								echo json_encode ( $fromGroupArr ) ;
							}
							break ;
						}
                    }
            ...
            <table width="500" border="0" cellpadding="0" cellspacing="7">
              <tr>
                <td>source group name  </td>
                <td><input name="source" type="text" /></td>
              </tr>
              <tr>
                <td>dest group name  </td>
              ...
              <tr>
                <td></td>
                <td><div align="right"><input name="" type="submit" value=" 切  分 " /></div></td>
              </tr>
            </table>
        </div>
        <?php
