/*******************************************************************************
   ...
   Source File Name = right_insert.php
...
if ( $order == "insert" )
{
	$record = empty ( $_POST['record'] ) ? ( empty ( $_GET['record'] ) ? "" : $_GET['record'] ) : $_POST['record'] ;
...
<div class="topb">
	<div class="path">
		...
		?>
         ...
         --> <a href="#" class="pathlink"><img src="../images/db.png" width="15px">&nbsp;插入数据</a>
	</div>
	<div class="clear"></div>
...
if ( $arr_1  != "" )
{
	...
	else
	{
		...
		echo '<img src="../images/icons/exclamation.png" width="15px" border="0"> 数据插入失败 错误码: '.$arr_1['errno'] ;
		php_div_e_3();
	}
}
if ( $str != "" )
...
<div style="width:<?php echo $width-206;?>px;margin-top:20px;">
    <div>
        <form action="right_insert.php" method="post" class="send">
		...
		if ( $csname != "" && $clname != "" )
		{
			$temp_str = $temp_str."<b>".$csname.".".$clname."</b> 中插入数据" ;
		}
        php_div_s_1($width-250,250,$temp_str,"margin-top:15px;margin-left:15px;");
        ?>
        <div style="padding:12px;">
            <table width="500" border="0" cellpadding="0" cellspacing="7">
              <tr>
                <td>record  </td>
                <td><textarea name="record" rows="8" cols="100"></textarea></td>
              </tr>
              <tr>
                <td></td>
                <td><div align="right"><input name="" type="submit" value=" 插 入 " /></div></td>
              </tr>
            </table>
        </div>
        <?php
