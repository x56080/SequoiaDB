...
USAGE = '''%prog add -t <sdb|mysql> [--inst=INSTNAME] [--path=INSTALL_DIR]
       %prog del -t <sdb|mysql> [--inst=INSTNAME] [--path=INSTALL_DIR]
       %prog list
       %prog start
       %prog stop
       %prog --help'''
...
#                                 dest="version", help="show version")
    ...
    def __parse_option(self):
        ...
        if self.log_type != LOG_TYPE_SDB and self.log_type != LOG_TYPE_MYSQL \
            ...
            print("[ERROR] Invalid log type, Please use 'sdb' or 'mysql' " \
                  "instead.")
            return 1
