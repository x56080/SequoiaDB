...
(function(){
   ...
   //控制器
   sacApp.controllerProvider.register( 'Data.MySQL.Data.Ctrl', function( $scope, $location, SdbSignal, SdbSwap, SdbFunction, SdbRest ){
      ...
      if( clusterName == null || moduleType != 'sequoiasql-mysql' || $scope.ModuleName == null || SdbSwap.dbName == null || SdbSwap.tbName == null )
      {
   ...
   //操作 控制器
   sacApp.controllerProvider.register( 'Data.MySQL.Data.Operate.Ctrl', function( $scope, $location, SdbSwap, SdbSignal, SdbRest ){
      ...
      $scope.ShowQueryWindow = function(){
         if( fieldList.length == 0 )
         {
            _IndexPublic.createInfoModel( $scope, $scope.pAutoLanguage( "当前数据表没有字段，无法进行该操作，是否前往表结构页面？" ), $scope.pAutoLanguage( '确定' ), function(){
               $location.path( '/Data/SequoiaSQL/MySQL/Operate/Structure' ).search( { 'r': new Date().getTime() } ) ;
            } ) ;
         }
         else
         {
   ...
   //结果栏
   sacApp.controllerProvider.register( 'Data.MySQL.Data.Result.Ctrl', function( $scope, SdbSignal ){
