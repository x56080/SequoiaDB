...
(function(){
   ...
   //主控制器
   sacApp.controllerProvider.register( 'Data.MySQL.Database.Ctrl', function( $scope, $location, SdbFunction, $timeout, SdbRest, SdbPromise, SdbSignal, SdbSwap, Loading ){
      var clusterName    = SdbFunction.LocalData( 'SdbClusterName' ) ;
      var moduleType     = SdbFunction.LocalData( 'SdbModuleType' ) ;
      $scope.ModuleName  = SdbFunction.LocalData( 'SdbModuleName' ) ;
      if( clusterName == null || moduleType != 'sequoiasql-mysql' || $scope.ModuleName == null )
      {
      ...
      $scope.GotoData = function( tableName ){
         SdbFunction.LocalData( 'MysqlDbName', $scope.CurrentDbName ) ;
         SdbFunction.LocalData( 'MysqlTbName', tableName ) ;
         $location.path( '/Data/SequoiaSQL/MySQL/Operate/Data' ).search( { 'r': new Date().getTime() } ) ;
      }
   } ) ;

   //弹窗操作控制器
   sacApp.controllerProvider.register( 'Data.MySQL.Database.Window.Ctrl', function( $scope, $timeout, SdbFunction, SdbSignal, SdbSwap, Loading ){
      //高度偏移量
