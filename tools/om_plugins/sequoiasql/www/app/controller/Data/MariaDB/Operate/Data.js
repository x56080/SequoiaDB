...
(function(){
   ...
   //控制器
   sacApp.controllerProvider.register( 'Data.MariaDB.Data.Ctrl', function( $scope, $location, SdbSignal, SdbSwap, SdbFunction, SdbRest ){
      var clusterName = SdbFunction.LocalData( 'SdbClusterName' ) ;
      var moduleType = SdbFunction.LocalData( 'SdbModuleType' ) ;
      $scope.ModuleName = SdbFunction.LocalData( 'SdbModuleName' ) ;
      SdbSwap.dbName = SdbFunction.LocalData( 'MariaDBDbName' ) ;
      SdbSwap.tbName = SdbFunction.LocalData( 'MariaDBTbName' ) ;
      SdbSwap.tbType = SdbFunction.LocalData( 'MariaDBTbType' ) ;
      if( clusterName == null || moduleType != 'sequoiasql-mariadb' || $scope.ModuleName == null || SdbSwap.dbName == null || SdbSwap.tbName == null )
      {
      ...
      SdbSignal.on( 'exec_sql', function( data ){
         ...
         var data = { 'Sql': execSql, 'DbName': SdbSwap.dbName, 'Type': 'mysql' } ;
   ...
   //操作 控制器
   sacApp.controllerProvider.register( 'Data.MariaDB.Data.Operate.Ctrl', function( $scope, $location, SdbSwap, SdbSignal, SdbRest ){
         ...
         SdbRest.DataOperationV2( '/sql', data, {
            'success': function( result ){
               ...
               $.each( fieldList, function( index, fieldInfo ){
                  ...
                  var defaultVal = '' ;
                  if ( disabled === false && fieldInfo['COLUMN_DEFAULT'] === 'NULL' )
                  {
                  }
                  else
                  {
                     defaultVal = fieldInfo['COLUMN_DEFAULT'] ;
                  }
                  var insertLine = {
                     ...
                     'Value'  : { 'type': 'string', 'value': defaultVal }
                  } ;
      ...
      $scope.ShowQueryWindow = function(){
         if( fieldList.length == 0 )
         {
            _IndexPublic.createInfoModel( $scope, $scope.pAutoLanguage( "当前数据表没有字段，无法进行该操作，是否前往表结构页面？" ), $scope.pAutoLanguage( '确定' ), function(){
               $location.path( '/Data/SequoiaSQL/MariaDB/Operate/Structure' ).search( { 'r': new Date().getTime() } ) ;
            } ) ;
         }
         else
         {
   ...
   //结果栏
   sacApp.controllerProvider.register( 'Data.MariaDB.Data.Result.Ctrl', function( $scope, SdbSignal ){
   ...
   //sql语句输入框
   sacApp.controllerProvider.register( 'Data.MariaDB.Data.InputBox.Ctrl', function( $scope, SdbSwap, SdbSignal ){
   ...
   //表格
   sacApp.controllerProvider.register( 'Data.MariaDB.Data.Table.Ctrl', function( $scope, $timeout, SdbFunction, SdbSwap, SdbSignal ){
