...
function show_help() {
    cat << EOF
Usage:
    sdb_stop_maintenance.sh [command] [options]
...
Example:
    sdb_stop_maintenance.sh -c config/config.js
    sdb_stop_maintenance.sh -l GuangZhou
    sdb_stop_maintenance.sh --check
    sdb_stop_maintenance.sh -f target

Notes:
    By default, all MaintenanceMode will be disabled
...
function main() {
    ...
    # 构建参数列表，用 ; 分隔
    local cmd="var scriptDir=\"$SCRIPT_DIR\"; var mode=\"stop_maintenance\""
