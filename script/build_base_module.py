...
class CompileBaseModuleMgr:
   ...
   def download_jdk(self, jdk_version):
      ...
      if jdk_version == 'jdk7' and OS_ARCH != 'aarch64':
   ...
   def compile_java_driver(self, db_version):
      ...
      # reset version
      # v3.4 have jre8 suffix
      self.set_pom_version(db_version+'.jre8', java_dir)
   ...
   def compile_java7_driver(self, db_version):
      print_log('Begine compile java7 driver')
      java_dir = os.path.join(self.root_dir, 'driver/java7')
      # reset version
      # v3.4 have jre7 suffix
      self.set_pom_version(db_version+'.jre7', java_dir)
      # need to install java driver for next compile
      compile_java_cmd = 'mvn clean install -Dmaven.test.skip=true -Dmaven.javadoc.skip=true'
      # use jdk compiel java7 in arm, use for compile spark
      if OS_ARCH != 'aarch64':
         self.run_in_dir(compile_java_cmd, java_dir, self.jdk7_env)
      else:
         self.run_in_dir(compile_java_cmd, java_dir, self.jdk_env)
   ...
   def compile_spark(self, db_version):
      ...
      spark_cmd = 'mvn clean package -Dsequoiadb.driver.version={}'.format(db_version+'.jre7')
      self.set_pom_version(db_version, spark_dir)
      if OS_ARCH != 'aarch64':
         self.run_in_dir(spark_cmd, spark_dir, self.jdk7_env)
      else:
   ...
   def compile_spark3(self, db_version):
      ...
      spark3_cmd = 'mvn clean package -Dmaven.test.skip=true -Dsequoiadb.driver.version={}'.format(db_version+'.jre8')
   ...
   def compile_flink(self, db_version):
      ...
      flink_cmd = 'mvn clean package -Dmaven.test.skip=true -Dsequoiadb.driver.version={}'.format(db_version+'.jre8')
